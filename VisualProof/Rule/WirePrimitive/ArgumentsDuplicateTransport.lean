...
/-- Contracting the adjacent coordinate introduced by duplication recovers
the complete source argument vector. -/
theorem eraseTargetArguments_exact
    {source : CheckedDiagram definitions}
    {wire : source.val.WireId}
    {position : Nat}
    (forward : AppliedArgDuplicate source wire position) :
    ConcreteWirePrimitive.eraseAt forward.targetArgumentList (position + 1) =
      forward.sourceArgumentList := by
  rw [AppliedArgDuplicate.targetArgumentList, forward.targetArguments_exact]
  exact eraseAt_insertAt_of_le forward.sourceArgumentList (position + 1)
    (forward.sourceArgumentList[position]?.getD .iota) (by
      have := forward.positionValid
      omega)

/-- The transport's insertion equation is the same typed duplication
evidence accepted by the semantic checker. -/
theorem semanticDuplication_exact
    {source : CheckedDiagram definitions}
    {wire : source.val.WireId}
    {position : Nat}
    (forward : AppliedArgDuplicate source wire position) :
    ArgumentsSemantics.TypedArguments.duplicatedArguments
        forward.sourceArgumentList forward.duplicationEvidence.position =
      forward.argumentResult.targetArguments :=
  forward.duplicationEvidence.targetExact

/-- The suffix isomorphism identifies the inverse contraction's source argument
vector with the exact duplication target vector. -/
theorem inverseSourceArguments_exact
    {planned real : CheckedDiagram definitions}
    {forwardWire : planned.val.WireId}
    {position : Nat}
    (forward : AppliedArgDuplicate planned forwardWire position)
    {backwardWire : real.val.WireId}
    (backward : AppliedArgContract real backwardWire position)
...
/-- Duplication followed by its checked contraction restores the planned argument
vector exactly. -/
theorem inverseTargetArguments_exact
  ...
  calc
    backward.targetArgumentList =
        ConcreteWirePrimitive.eraseAt backward.sourceArgumentList (position + 1) :=
      backward.targetArguments_exact
    _ = ConcreteWirePrimitive.eraseAt forward.targetArgumentList (position + 1) := by
...
/-- Region carrier of the transported duplication/contraction pair. -/
...
/-- Node carrier of the transported duplication/contraction pair. -/
...
/-- Exact head-only wire carrier of the transported duplication/contraction pair. -/
...
theorem inverseTransport_targetNode
  ...
  have backwardImage : backward.nodeEquiv backwardNode =
    ...
    unfold AppliedArgContract.nodeEquiv
  ...
  have forwardImage : forward.nodeEquiv plannedNode =
    ...
    unfold AppliedArgDuplicate.nodeEquiv
      ConcreteWirePrimitive.ArgumentResult.nodeEquiv
...
/-- Complete node-table law for the transported inverse contraction/extension
carrier. -/
...
/-- Every rebuilt contraction argument endpoint maps at the same restored
...
theorem inverseTransportEndpointMap_argument
    ...
    let realPosition := restorePosition (position + 1) index
    let realAttachment :=
  ...
  have insertedGet := insertAt_getElem?_restorePosition plannedSite.arguments
    (position + 1) index ((plannedSite.arguments[position]?).getD forwardWire)
    (by rw [plannedSiteLength]; exact
      Nat.succ_le_iff.mpr forward.positionValid)
    plannedArgumentBound
  rw [plannedAttachmentGet] at insertedGet
  have forwardTargetBound : restorePosition (position + 1) index <
      forward.targetArgumentList.length := by
    rw [AppliedArgDuplicate.targetArgumentList,
      forward.targetArguments_exact,
      insertAt_length_of_le forward.sourceArgumentList (position + 1) (forward.sourceArgumentList[position]?.getD .iota)
        (Nat.succ_le_iff.mpr forward.positionValid), ← plannedSiteLength]
    unfold restorePosition
    split <;> omega
  have forwardOwner := forward.generatedArgument_endpointOwner_of_selected
    plannedPosition (restorePosition (position + 1) index) forwardTargetBound
  ...
  have sourceLength : backward.sourceArgumentList.length =
    ...
    rw [sourceArguments, AppliedArgDuplicate.targetArgumentList,
      forward.targetArguments_exact,
      insertAt_length_of_le forward.sourceArgumentList (position + 1) (forward.sourceArgumentList[position]?.getD .iota)
        (Nat.succ_le_iff.mpr forward.positionValid)]
  have realPositionBound : restorePosition (position + 1) index <
      backwardSite.arguments.length := by
    rw [backwardSiteLength, sourceLength]
    unfold restorePosition
    split <;> omega
  let realAttachment :=
    (backwardSite.arguments[restorePosition (position + 1) index]?).getD backwardWire
  have realAttachmentGet :
      backwardSite.arguments[restorePosition (position + 1) index]? =
        some realAttachment := by
    unfold realAttachment
    rw [List.getElem?_eq_getElem realPositionBound]
    simp
  have erasedGet :
      (ConcreteWirePrimitive.eraseAt backwardSite.arguments (position + 1))[index]? =
        some realAttachment := by
    rw [← eraseAt_getElem?_restorePosition backwardSite.arguments (position + 1)
      index]
  ...
  have middleOwner := targetIso.atom_owner_forward real.property
    forward.target.property backwardNodeData
      (backwardSite.argument_owner (restorePosition (position + 1) index)
        realPositionBound)
...
private theorem contractGeneratedHead_endpointOwner
    {source : CheckedDiagram definitions}
    {wire : source.val.WireId}
    {position : Nat}
...
private theorem contractGeneratedArgument_bound
    {source : CheckedDiagram definitions}
    {wire : source.val.WireId}
    {position : Nat}
    (applied : AppliedArgContract source wire position)
  ...
  simpa [AppliedArgContract.targetArgumentList] using required

private theorem contractGeneratedIdentity_not_required
    {source : CheckedDiagram definitions}
    {wire : source.val.WireId}
    {position : Nat}
    (applied : AppliedArgContract source wire position)
...
theorem inverseTransportEndpointMap_mem
  ...
  · have generated : realNode ∈
    ...
    cases port with
    | head =>
        have generatedOwner := contractGeneratedHead_endpointOwner backward site
    ...
    | arg index =>
        have indexBound := contractGeneratedArgument_bound backward site index
          required
        have transported := forward.inverseTransportEndpointMap_argument
          backward targetIso wireExact site index indexBound
        let realAttachment :=
          (backwardSite.arguments[restorePosition (position + 1) index]?).getD
            backwardWire
        let plannedAttachment :=
          (plannedSite.arguments[index]?).getD forwardWire
    ...
    | identity index =>
        exact (contractGeneratedIdentity_not_required backward site index
          required).elim
    ...
    | head =>
        ...
        exact ConcreteDiagram.endpointOwner?_incident backward.target.val
          ⟨backward.targetNode site, .head⟩ backward.targetWire
            (contractGeneratedHead_endpointOwner backward site)
    | arg index =>
        have plannedBound : index < plannedSite.arguments.length := by
...
duplication, its checked inverse contraction, and the supplied suffix isomorphism. -/
