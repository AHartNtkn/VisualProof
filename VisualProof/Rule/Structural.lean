...
comparing compiler-produced intrinsic regions.  They do not form a rule
receipt or accept semantic evidence.
-/
...
private def intrinsicRegionsEqual
    (left right : Region definitions ctx) : Bool :=
  decide (left = right)

open IntrinsicEquality in
private theorem intrinsicRegionsEqual_sound
...
def checkVacuous
    ...
    Except StructuralError (CheckedVacuous input) := by
  match siteAccepted : compileSite? plain input.site with
  | none => exact .error .siteCompilationFailed
  | some siteCompiled =>
      if exact :
