import VisualProof.Rule.Soundness.Modal.EliminationRootCompiler
...
noncomputable def rootContextSimulation
  ...
  · intro atRoot focused allowed recurse recurseAt sourceItems targetItems
    ...
    change trace.sourceDiagram.root = trace.targetIndex targetWellFormed
      at focused
    have targetAtRoot : trace.target = input.root := by
      calc
        trace.target = trace.origin (trace.targetIndex targetWellFormed) :=
...
theorem interfaceTransport_transportBoundary
    ...
    (sourceRoot : ∀ wire, wire ∈ boundary →
      (input.wires wire).scope = input.root) :
    (doubleCutElimInterfaceTransport hraw).transportBoundary boundary =
      some (boundary.map
        (Fin.cast (doubleCutElimRaw?_wireCount hraw).symm)) := by
  ...
  intro wire member
  unfold doubleCutElimInterfaceTransport InterfaceTransport.byWireCount
