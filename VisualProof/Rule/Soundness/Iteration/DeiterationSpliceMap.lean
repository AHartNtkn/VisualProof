...
private theorem deiterationMaterial_mem_iff
  ...
  · intro membership
    ...
    have targetMaterial := (Concrete.Splice.Input.PlugLayout.materialRegions_survives_iff
    ...
    have sourceMaterial := (deiterationPattern_material_iff input selection witness region).mp
      targetMaterial
    have sourceSurvives := (Concrete.Splice.Input.PlugLayout.materialRegions_survives_iff
  ...
  · intro membership
    ...
    have sourceMaterial := (Concrete.Splice.Input.PlugLayout.materialRegions_survives_iff
    ...
    have targetMaterial := (deiterationPattern_material_iff input selection witness region).mpr
      sourceMaterial
    have targetSurvives := (Concrete.Splice.Input.PlugLayout.materialRegions_survives_iff
  ...
  FiniteEquiv.restrictLists occurrence.diagram.wires
    ...
    target.plugLayout.internalWires.enumeration_nodup (by
      ...
      · intro membership
        ...
        have targetInternal := (Concrete.Splice.Input.PlugLayout.internalWires_survives_iff
        ...
        exact (SurvivorDomain.mem_enumeration source.plugLayout.internalWires wire).mpr
          ((Concrete.Splice.Input.PlugLayout.internalWires_survives_iff
            source.plugLayout wire).mpr sourceInternal)
      · intro membership
        have sourceSurvives := (SurvivorDomain.mem_enumeration
          source.plugLayout.internalWires wire).mp membership
        have sourceInternal := (Concrete.Splice.Input.PlugLayout.internalWires_survives_iff
        ...
        exact (SurvivorDomain.mem_enumeration target.plugLayout.internalWires
          (occurrence.diagram.wires wire)).mpr
            ((Concrete.Splice.Input.PlugLayout.internalWires_survives_iff target.plugLayout
...
noncomputable def deiterationQuotientEquiv
  ...
  (iterationQuotientWireEquiv
    ...
    (deiterationReinsertTarget input selection)).trans
      (Concrete.Splice.Decomposition.originalQuotientWireEquiv
...
theorem deiterationQuotientEquiv_origin
  ...
  exact (Concrete.Splice.Decomposition.originalQuotientWireEquiv
...
theorem deiterationBinderTarget_alignment
  ...
  calc
    ...
    _ = domains.regions.origin
      ...
      symm
      change domains.regions.origin
          (Concrete.Splice.Decomposition.originalBinderTarget
            (deiterationDecomposition input selection) targetIndex) = _
      unfold Concrete.Splice.Decomposition.originalBinderTarget
...
noncomputable def deiterationOutputRegionEquiv
    ...
    FiniteEquiv
      ...
      (Fin (Concrete.Splice.Decomposition.originalFragmentInput
...
private theorem binderRegion_eq_body_of_no_proxy
  ...
  have lookupNone : layout.proxyIndex? region = none := by
    unfold Concrete.Splice.Input.PlugLayout.proxyIndex?
    ...
    | some found =>
        ...
        rw [Concrete.Splice.Input.PlugLayout.proxies, List.mem_map] at member
        obtain ⟨index, _, equality⟩ := member
        exact False.elim (notProxy index equality.symm)
  unfold Concrete.Splice.Input.PlugLayout.binderRegion
  rw [lookupNone]
