...
theorem mapRouteToFinalFocus
    ...
    ∃ finalPath,
      ∃ finalRoute : Concrete.Splice.RegionRoute elimTrace.sourceDiagram
  ...
  | @here region =>
      ...
      let finalWitness :
          { candidate : Concrete.Splice.RegionRoute elimTrace.sourceDiagram
        ...
        rw [focusEq]
        exact ⟨.here _, Concrete.Splice.RegionRoute.HasCutDepth.here
  ...
  | @step start child target rest hparent position hposition tail ih =>
      ...
      obtain ⟨finalPosition, finalPositionLookup⟩ := indexOf?_complete
        ((Concrete.Elaboration.mem_localOccurrences_child
          ...
          (copyTrace.finalRegionMap elimTrace finalWellFormed child)).2
            finalParent)
      let finalRoute := Concrete.Splice.RegionRoute.step finalParent finalPosition
      ...
      | cut parent =>
          ...
          subst parent
          have sourceDepth :
              (Concrete.Splice.RegionRoute.step hparent position hposition tail).HasCutDepth
                (tailNatural + 1) :=
...
theorem bubble_parent_concreteCutDepth
  ...
  let parentView := Classical.choice
    (Concrete.Splice.siteView_complete input payload.parent)
  let bubbleView := Classical.choice
    (Concrete.Splice.siteView_complete input bubble)
  ...
  let lastRoute : Concrete.Splice.RegionRoute input.val payload.parent bubble
      [position.val] :=
    Concrete.Splice.RegionRoute.step bubbleParent position positionLookup (.here bubble)
  have lastDepth : lastRoute.HasCutDepth 0 := by
    exact Concrete.Splice.RegionRoute.HasCutDepth.bubble
      ...
      payload.bubble_eq (Concrete.Splice.RegionRoute.HasCutDepth.here bubble)
  ...
  have pathEq := VisualProof.Concrete.Splice.Input.RegionRoute.path_unique
...
theorem finalAllowed_root
  ...
  · have finalRootFocus : elimTrace.sourceDiagram.root =
    ...
    have normalizedDepth : normalizedRoute.HasCutDepth depth :=
      normalized.property
    let here : Concrete.Splice.RegionRoute elimTrace.sourceDiagram
        (elimTrace.targetIndex finalWellFormed)
        (elimTrace.targetIndex finalWellFormed) [] := .here _
    have pathEq :=
      VisualProof.Concrete.Splice.Input.RegionRoute.path_unique
    ...
    have hereCastDepth : hereCast.HasCutDepth 0 :=
      by
        simpa [hereCast] using
          (Concrete.Splice.RegionRoute.HasCutDepth.here
