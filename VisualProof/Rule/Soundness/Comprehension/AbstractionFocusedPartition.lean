...
def selectedAt
    ...
    (occurrences : List (OperationAbstractionOccurrence input))
    (region : Fin input.val.regionCount) :
    List (Concrete.Elaboration.LocalOccurrence input.val.regionCount
      input.val.nodeCount) :=
  ...
  | child child =>
      ...
      have leftSelected : child ∈
        ...
        ((occurrences.get left).selection.mem_selectedRegions child).2
          ⟨child, leftDirect, Concrete.Diagram.Encloses.refl input.val child⟩
      have rightSelected : child ∈
        ...
        ((occurrences.get right).selection.mem_selectedRegions child).2
          ⟨child, rightDirect, Concrete.Diagram.Encloses.refl input.val child⟩
...
theorem localOccurrences_perm_focusedPartition
  ...
  · rw [List.nodup_append]
    refine ⟨(Concrete.Elaboration.localOccurrences_nodup input.val region).filter
    ...
    have survives := (mem_survivingSources trace
      (Concrete.Elaboration.localOccurrences input.val region) survivor).1
      survivorMember |>.2
    rw [selectedAt_deleted trace payload region survivor selectedMember] at survives
    contradiction
  · exact Concrete.Elaboration.localOccurrences_nodup input.val region
  · intro occurrence
    constructor
    · intro member
      rcases List.mem_append.mp member with survivor | selected
      · exact (mem_survivingSources trace
          (Concrete.Elaboration.localOccurrences input.val region) occurrence).1
            survivor |>.1
...
theorem selectedAt_wrap_subset
    ...
    occurrence ∈ VisualProof.Rule.ModalSoundness.selectedOccurrences input.val wrap := by
  obtain ⟨index, anchor, selected⟩ :=
...
theorem selectedOccurrences_perm_focusedPartition
  ...
  · rw [List.nodup_append]
    ...
    subst selected
    have survives := (mem_survivingSources trace
      (VisualProof.Rule.ModalSoundness.selectedOccurrences input.val wrap) survivor).1
      survivorMember |>.2
  ...
  · intro occurrence
    ...
    · intro member
      rcases List.mem_append.mp member with survivor | selected
      · exact (mem_survivingSources trace
          (VisualProof.Rule.ModalSoundness.selectedOccurrences input.val wrap) occurrence).1
            survivor |>.1
    ...
    · intro selected
      ...
      | some target =>
          ...
          apply Or.inl
          exact (mem_survivingSources trace
            (VisualProof.Rule.ModalSoundness.selectedOccurrences input.val wrap) occurrence).2
