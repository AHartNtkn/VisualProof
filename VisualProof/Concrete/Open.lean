...
abbrev CheckedOpen :=
  { d : OpenDiagram // d.WellFormed  }

namespace Diagram

/-- The derived open view of a closed concrete diagram has no boundary. -/
def asOpen (d : Diagram) : OpenDiagram where
  diagram := d
  boundary := []

@[simp] theorem asOpen_diagram (d : Diagram) :
    d.asOpen.diagram = d := rfl

@[simp] theorem asOpen_boundary (d : Diagram) :
    d.asOpen.boundary = [] := rfl

theorem asOpen_wellFormed (d : Diagram)
    (hwf : d.WellFormed ) :
...
def asOpen (checked : Checked ) :
    CheckedOpen  :=
  ⟨checked.val.asOpen, checked.val.asOpen_wellFormed checked.property⟩

@[simp] theorem asOpen_val (checked : Checked ) :
...
-/
def exposedWires (d : OpenDiagram) :
    List (Fin d.diagram.wireCount) :=
  d.boundary.eraseDups

@[simp] theorem mem_exposedWires (d : OpenDiagram)
    (wire : Fin d.diagram.wireCount) :
    wire ∈ d.exposedWires ↔ wire ∈ d.boundary := by
  simp [exposedWires]

theorem exposedWires_nodup (d : OpenDiagram) :
    d.exposedWires.Nodup :=
  eraseDups_nodup d.boundary

@[simp] theorem boundary_get_mem_exposedWires (d : OpenDiagram)
...
affect it.
-/
def boundaryClass (d : OpenDiagram)
...
theorem boundaryClass_lookup (d : OpenDiagram)
    (position : Fin d.boundary.length) :
...
theorem boundaryClass_sound (d : OpenDiagram)
    (position : Fin d.boundary.length) :
...
theorem boundaryClass_complete (d : OpenDiagram)
    (position : Fin d.boundary.length)
...
theorem boundaryClass_eq_iff (d : OpenDiagram)
...
theorem boundaryClass_surjective (d : OpenDiagram) :
...
theorem hiddenWires_nodup (d : OpenDiagram) :
    d.hiddenWires.Nodup :=
  (exactScopeWires_nodup d.diagram d.diagram.root).filter _

theorem exposedWires_hiddenWires_disjoint (d : OpenDiagram) :
...
theorem rootWires_nodup (d : OpenDiagram) :
...
@[simp] theorem asOpen_exposedWires (d : Diagram) :
    d.asOpen.exposedWires = [] := rfl

@[simp] theorem asOpen_hiddenWires (d : Diagram) :
...
@[simp] theorem asOpen_rootWires (d : Diagram) :
