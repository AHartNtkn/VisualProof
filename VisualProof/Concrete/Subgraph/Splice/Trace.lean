  ...
  | fuel + 1, region =>
      if region = d.root then some []
        ...
        | some parent => do
            ...
            let position ← indexOf? (Elaboration.localOccurrences d parent)
              (.child region)
            pure (prior ++ [position.val])

def regionPath? (checked : Checked )
...
inductive RegionPath (d : Diagram) :
  ...
  | child {start parent child prior}
      ...
      (hposition : indexOf? (Elaboration.localOccurrences d parent)
...
inductive RegionRoute (d : Diagram) :
  ...
  | step {start child target rest}
      ...
      (position : Fin (Elaboration.localOccurrences d start).length)
      (hposition : indexOf? (Elaboration.localOccurrences d start)
        (.child child) = some position)
...
private theorem regionRoute_complete_of_climb
  ...
  | zero =>
      simp only [Diagram.climb_zero, Option.some.injEq] at hclimb
      subst target
      exact ⟨[], ⟨.here start⟩⟩
  | succ steps ih =>
      simp only [Diagram.climb] at hclimb
      split at hclimb
      · contradiction
      · rename_i parent hparent
        obtain ⟨path, ⟨route⟩⟩ := ih parent hclimb
        obtain ⟨position, hposition⟩ := indexOf?_complete
          ((Elaboration.mem_localOccurrences_child d parent target).2
            hparent)
...
theorem regionRoute_complete_of_encloses
    (d : Diagram) (start target : Fin d.regionCount)
...
inductive RegionRoute.HasCutDepth {d : Diagram} :
...
structure Region.ContextPath.CompilerLeaf
  ...
  inheritedWires : Elaboration.WireContext diagram
  inheritedLength : inheritedWires.length = witness.toFocus.holeWires
  binders : Elaboration.BinderContext diagram
  ...
  itemsComputation :
    Elaboration.compileOccurrencesWith?  diagram
        (Elaboration.compileRegion?  diagram fuel)
        (inheritedWires.extend target) binders
        (Elaboration.localOccurrences diagram target) = some items
  ...
  bodyComputation : witness.toFocus.body =
    Region.castWiresEq inheritedLength
      (Elaboration.finishRegion diagram inheritedWires target items)
...
def Region.ContextPath.CompilerLeaf.atFocus
    {diagram : Diagram} {target : Fin diagram.regionCount}
    {body : Region  outer rels} {path : List Nat}
...
def Region.ContextPath.CompilerLeaf.hereOfItemsComputation
    ...
    (target : Fin diagram.regionCount)
    (inheritedWires : Elaboration.WireContext diagram)
    (binders : Elaboration.BinderContext diagram rels)
    (fuel : Nat)
    ...
    (bindersCover : binders.Covers target)
    (binderEnumeration :
      Elaboration.BinderContext.Enumeration diagram binders target) :
    Region.ContextPath.CompilerLeaf diagram target
      (.here (Elaboration.finishRegion diagram inheritedWires target
        items)) where
...
theorem Region.ContextPath.CompilerLeaf.inherited_mem_iff
  ...
  rw [Elaboration.WireContext.extend, List.nodup_append]
    at extendedNodup
  constructor
  · intro hinherited
    ...
    intro hscope
    have hlocal : wire ∈ Elaboration.exactScopeWires diagram site :=
      (Elaboration.mem_exactScopeWires diagram site wire).2 hscope
    exact extendedNodup.2.2 wire hinherited wire hlocal rfl
  · rintro ⟨hvisible, hnotLocal⟩
    have hextended : wire ∈ leaf.inheritedWires.extend site :=
      (leaf.wiresExact.mem_iff wire).2 hvisible
    rw [Elaboration.WireContext.extend, List.mem_append] at hextended
    exact hextended.resolve_right fun hlocal =>
      hnotLocal ((Elaboration.mem_exactScopeWires diagram site wire).1
        hlocal)
...
def Region.ContextPath.CompilerLeaf.canonicalBodyItems
    ...
    ItemSeq
      (outer + (Elaboration.exactScopeWires diagram site).length)
      rels :=
  (state.items.castWiresEq
    ...
    (congrArg
      (fun inherited => inherited +
        (Elaboration.exactScopeWires diagram site).length)
...
inductive Region.ContextPath.OpenCompilerLeaf
  ...
  | root
      ...
      OpenCompilerLeaf checked checked.val.diagram.root
        (.here (Elaboration.finishRoot checked.val.exposedWires
...
inductive CompilerTrace (diagram : Diagram) :
  ...
  | cut
      ...
      {position : Fin
        (Elaboration.localOccurrences diagram start).length}
      {hposition : indexOf?
        (Elaboration.localOccurrences diagram start) (.child child) =
          some position}
      ...
      (localWiresCanonical : localWires =
        (Elaboration.exactScopeWires diagram start).length)
...
def compilerLeafHereCastWiresEq
    {diagram : Diagram} {start : Fin diagram.regionCount}
      ...
      exact fun index =>
        ih (Fin.cast (congrArg List.length inherited).symm
          (Fin.cast
            ...
            (Fin.castAdd
              (Elaboration.exactScopeWires diagram start).length index)))
  ...
  | @cut start child target rest hparent position hposition tail outer
      ...
      calc
        ...
        _ = state.inheritedWires.get index := by
          simp [extendedIndex, Elaboration.WireContext.extend]
  | @bubble start child target rest hparent position hposition tail outer
      localWires arity rels items focus childBody atIndex isBubble nested state
      localWiresCanonical itemsCanonical childState childKind inherited binders
      fuel tailTrace ih =>
...
theorem binderEnumeration_lookup_exact
    ...
    (enumeration : Elaboration.BinderContext.Enumeration diagram
      binders region)
...
theorem CompilerTrace.binder_lookup_outerRelation
  ...
  | @bubble start child target rest hparent position hposition tail outer
      ...
      let lifted : Theory.RelVar (bubbleArity :: rels) arity :=
        Elaboration.BinderContext.liftVar bubbleArity relation
      have childLookup : childState.binders binder = some ⟨arity, lifted⟩ := by
        rw [binders,
          Elaboration.BinderContext.push_other state.binders
...
structure OpenRootCompilerState (checked : CheckedOpen )
...
inductive OpenCompilerTrace (checked : CheckedOpen ) :
  ...
  | cut
      ...
      {position : Fin (Elaboration.localOccurrences
...
structure CompilerTraceResult
    ...
    {rels : Theory.RelCtx}
    (context : Elaboration.WireContext checked.val)
    (binders : Elaboration.BinderContext checked.val rels)
...
theorem compileRegion_route_context_complete
    ...
    (hcompile : Elaboration.compileRegion?  checked.val fuel
  ...
  | @step start child target rest hparent position hposition tail ih =>
      ...
      | succ fuel =>
          ...
          cases hitems : Elaboration.compileOccurrencesWith?
              ...
              (Elaboration.localOccurrences checked.val start) with
          | none => simp [hitems] at hcompile
          | some items =>
              simp [hitems] at hcompile
              subst body
              obtain ⟨itemFocus, hitemFocus, hitemCompiled⟩ :=
                compiledOccurrence_focus checked.val
                  (Elaboration.compileRegion?  checked.val fuel)
                  (context.extend start) rels binders
                  (Elaboration.localOccurrences checked.val start) items
                  (.child child) position hitems hposition
              let wireEq :=
                Elaboration.WireContext.length_extend context start
              ...
              | cut parent =>
                  ...
                  subst parent
                  simp only [Elaboration.compileOccurrenceWith?, hchild]
                    at hitemCompiled
                  cases hchildBody : Elaboration.compileRegion?
...
theorem openRootWires_exact
  ...
  · intro wire
    constructor
    · intro hmem
      have hscope := (OpenDiagram.mem_rootWires_iff
    ...
    · intro hencloses
      have hscope := Elaboration.encloses_sheet_eq
              ...
              | some childBody =>
                  ...
                  have castInherited : castState.inheritedWires =
                    ...
                    simpa [castState]
                      using childResult.inherited_eq
                  have castBinders : castState.binders =
                      Elaboration.BinderContext.empty.push child arity := by
                    simpa [castState]
                      using childResult.binders_eq
                  have castFuel : castState.fuel + 1 =
                      checked.val.diagram.regionCount := by
...
lookup.
-/
structure SiteView (checked : Checked )
  ...
  result : CompilerTraceResult checked route
    ([] : Elaboration.WireContext checked.val)
    Elaboration.BinderContext.empty (checked.val.regionCount + 1)
    checked.elaborate
...
structure OpenSiteView (checked : CheckedOpen )
...
theorem checkedRootItems_complete (checked : Checked ) :
    ∃ items : ItemSeq
        (Elaboration.exactScopeWires checked.val
      ...
      Elaboration.compileOccurrencesWith?  checked.val
        ...
        (Elaboration.localOccurrences checked.val checked.val.root) =
          some items := by
  obtain ⟨body, hroot, _⟩ := Checked.elaborate_computation checked
  simp only [Elaboration.compileRoot?] at hroot
  cases hitems : Elaboration.compileOccurrencesWith?
      checked.val
      (Elaboration.compileRegion?  checked.val
        checked.val.regionCount)
      (Elaboration.exactScopeWires checked.val checked.val.root)
      Elaboration.BinderContext.empty
      (Elaboration.localOccurrences checked.val checked.val.root) with
...
theorem checkedOpenRootItems_complete
  ...
  | some items =>
      refine ⟨items, ?_⟩
      simpa only [OpenDiagram.rootWires] using hitems

theorem contextFocusAtRegion_complete (checked : Checked )
    (region : Fin checked.val.regionCount) :
    ∃ path, regionPath? checked region = some path ∧
...
def replaceOpenBody (diagram : VisualProof.Diagram.OpenDiagram arity)
    ...
    VisualProof.Diagram.OpenDiagram arity where
...
theorem splice_climb_prefix_exists {d : Diagram}
  ...
  | succ first ih =>
      ...
      | succ second =>
          ...
          | none => simp [Diagram.climb, hparent] at hfinish
          | some parent =>
              have htail : d.climb second parent = some finish := by
                simpa [Diagram.climb, hparent] using hfinish
              obtain ⟨middle, hmiddle⟩ :=
                ih (Nat.le_of_succ_le_succ hle) htail
              exact ⟨middle, by
                simpa [Diagram.climb, hparent] using hmiddle⟩

theorem splice_climb_cancel_prefix {d : Diagram}
  ...
  | succ first ih =>
      ...
      | succ second =>
          cases hparent : (d.regions start).parent? with
          | none => simp [Diagram.climb, hparent] at hfirst
          | some parent =>
              have hfirstTail : d.climb first parent = some middle := by
                simpa [Diagram.climb, hparent] using hfirst
              have hsecondTail : d.climb second parent = some finish := by
                simpa [Diagram.climb, hparent] using hsecond
...
theorem BinderSpine.TerminalBodyContract.hiddenWires_eq_nil_of_nonempty
  ...
  intro wire hhidden
  have hidden := (OpenDiagram.mem_hiddenWires openDiagram wire).mp hhidden
  have notBoundary : wire ∉ openDiagram.boundary := by
    intro hboundary
    exact hidden.2 ((OpenDiagram.mem_exposedWires openDiagram wire).2
      hboundary)
...
theorem BinderSpine.TerminalBodyContract.nonterminal_exactScopeWires_eq_nil
    ...
    Elaboration.exactScopeWires openDiagram.diagram
      (spine.proxy proxy) = [] := by
  apply List.eq_nil_iff_forall_not_mem.mpr
  intro wire hwire
  have scope := (Elaboration.mem_exactScopeWires openDiagram.diagram
...
theorem BinderSpine.enclosing_proxy_aux
  ...
  | 0, bound, ancestor, hencloses => by
      have parent :
        ...
        simp [CRegion.parent?]
      rcases Elaboration.encloses_direct_child parent hencloses with
        heq | hroot
      · right
...
theorem Region.ContextPath.CompilerLeaf.inherited_mem_iff_exposed
  ...
  have extendedNodup := leaf.wiresExact.nodup
  rw [Elaboration.WireContext.extend,
  ...
  have notLocal : wire ∈ leaf.inheritedWires →
    ...
    have hlocal : wire ∈ Elaboration.exactScopeWires
        checked.val.diagram (spine.proxy terminal) :=
      (Elaboration.mem_exactScopeWires checked.val.diagram
  ...
  · intro hinherited
    ...
    · by_cases hexposed : wire ∈ checked.val.exposedWires
      ...
      · have hhidden : wire ∈ checked.val.hiddenWires :=
          (OpenDiagram.mem_hiddenWires checked.val wire).2
            ⟨hroot, hexposed⟩
  ...
  · intro hexposed
    have rootScope := checked.property.boundary_is_root_scoped wire
      ((OpenDiagram.mem_exposedWires checked.val wire).1 hexposed)
