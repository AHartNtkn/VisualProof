import VisualProof.Concrete.Subgraph.Splice.Input.Semantics.FocusedEnvironment
...
theorem checkedDiagram_regions_eq
    (left right : Checked ) (h : left = right)
...
theorem checkedDiagram_regions_rename_eq
    ...
    right.val.regions (Fin.cast (congrArg
      (fun checked : Checked  => checked.val.regionCount) h)
      region) =
      (left.val.regions region).rename
        (FiniteEquiv.finCast (congrArg
          (fun checked : Checked  => checked.val.regionCount) h)) := by
  cases h
...
theorem checkedDiagram_nodes_rename_eq
    ...
    right.val.nodes (Fin.cast (congrArg
      (fun checked : Checked  => checked.val.nodeCount) h)
      node) =
...
theorem checkedDiagram_endpointOccurs_eq
    ...
    left.val.EndpointOccurs wire endpoint ↔
      right.val.EndpointOccurs
        (Fin.cast (congrArg
          (fun checked : Checked  => checked.val.wireCount) h)
          wire)
...
def castLocalOccurrence
    ...
    Elaboration.LocalOccurrence left.val.regionCount left.val.nodeCount →
      Elaboration.LocalOccurrence right.val.regionCount
        right.val.nodeCount
...
theorem checkedDiagram_localOccurrences_eq
    (left right : Checked ) (h : left = right)
    (region : Fin left.val.regionCount) :
    Elaboration.localOccurrences right.val
        (Fin.cast (congrArg
          (fun checked : Checked  => checked.val.regionCount) h)
          region) =
      (Elaboration.localOccurrences left.val region).map
        (castLocalOccurrence
          left right
          (congrArg
            (fun checked : Checked  => checked.val.regionCount) h)
          (congrArg
            (fun checked : Checked  => checked.val.nodeCount) h)) := by
  cases h
  have regionCast :
      (Fin.cast (congrArg
        (fun checked : Checked  => checked.val.regionCount)
  ...
  have nodeCast :
      (Fin.cast (congrArg
        (fun checked : Checked  => checked.val.nodeCount)
...
theorem coalescedScope_eq_of_nonSite_wire
  ...
  have sourceEnclosesTarget :
    ...
    exact transported
  exact Elaboration.checked_encloses_antisymm target.frame.property
...
noncomputable def coalescedLocalWireEquiv
    ...
    (regionNe : region ≠ source.site) :
    FiniteEquiv
      (Fin (Elaboration.exactScopeWires source.coalesceFrameRaw
        region).length)
      (Fin (Elaboration.exactScopeWires target.coalesceFrameRaw
        (Fin.cast presentation.frameRegionCountEq region)).length) :=
  listEmbeddingEquiv presentation.quotientMap
    (Elaboration.exactScopeWires source.coalesceFrameRaw region)
    (Elaboration.exactScopeWires target.coalesceFrameRaw
      (Fin.cast presentation.frameRegionCountEq region))
    (Elaboration.exactScopeWires_nodup _ _)
    (Elaboration.exactScopeWires_nodup _ _)
    (fun quotient member =>
      (Elaboration.mem_exactScopeWires _ _ _).2
        (presentation.quotientMap_coalescedScope sourceAdmissible
          targetAdmissible region regionNe quotient
          ((Elaboration.mem_exactScopeWires _ _ _).1 member)))
    (fun quotient member => by
      ...
      exact ⟨sourceQuotient,
        (Elaboration.mem_exactScopeWires _ _ _).2 sourceScope,
        mapped⟩)
    (fun left leftMember right rightMember mapped =>
      presentation.quotientMap_injective_at_nonSite region regionNe left right
        ((Elaboration.mem_exactScopeWires _ _ _).1 leftMember)
        ((Elaboration.mem_exactScopeWires _ _ _).1 rightMember)
        mapped)
...
theorem coalescedLocalWireEquiv_spec
    ...
    (Elaboration.exactScopeWires target.coalesceFrameRaw
      ...
      presentation.quotientMap
        ((Elaboration.exactScopeWires source.coalesceFrameRaw
          region).get index) := by
...
theorem outputRootWireEquivOfNested_related
  ...
  · obtain ⟨wire, sourceGet, targetGet⟩ :=
      presentation.outputRootExposedWireEquivOfNested_related
        ...
        exposed
    refine ⟨wire, ?_, ?_⟩
    · simpa [sourceOpen, OpenDiagram.rootWires] using sourceGet
    · dsimp only [sourceOpen]
      rw [extendWireEquiv_outer]
      simpa [targetOpen, OpenDiagram.rootWires] using targetGet
  · obtain ⟨wire, sourceGet, targetGet⟩ :=
...
noncomputable def regularFrameLocalWireEquiv
    ...
    (regionNe : region ≠ source.site) :
    FiniteEquiv
      (Fin (Elaboration.exactScopeWires source.plugLayout.plugRaw
        (source.plugLayout.frameRegion region)).length)
      (Fin (Elaboration.exactScopeWires target.plugLayout.plugRaw
...
theorem regularFrameLocalWireEquiv_related
  ...
  let sourceQuotient :=
    (Elaboration.exactScopeWires source.coalesceFrameRaw region).get
      sourceCoalesced
  let wire := source.wireQuotient.origin sourceQuotient
  have sourceQuotientEq : source.quotientWire wire = sourceQuotient :=
    source.quotientWire_wireQuotient_origin sourceQuotient
