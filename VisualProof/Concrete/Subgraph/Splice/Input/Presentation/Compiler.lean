import VisualProof.Concrete.Subgraph.Splice.Input.Presentation.Core
...
theorem regularChildFrameRegion
  ...
  · intro childParent
    have member : Elaboration.LocalOccurrence.child
      ...
      (Elaboration.mem_localOccurrences_child _ _ _).2 childParent
    ...
    | child frameChild =>
        exact False.elim
          (source.plugLayout.frameRegion_ne_materialRegion frameChild
            materialChild
            (Elaboration.LocalOccurrence.child.inj mapped))
...
private theorem filter_patternNodes_eq_nil
  ...
  have member : Elaboration.LocalOccurrence.node
  ...
  | node frameNode =>
      exact layout.frameNode_ne_patternNode frameNode patternNode
        (Elaboration.LocalOccurrence.node.inj mapped)
...
private theorem filter_materialChildren_eq_nil
  ...
  have member : Elaboration.LocalOccurrence.child
      (layout.materialRegion materialChild) ∈
        Elaboration.localOccurrences layout.plugRaw
          (layout.frameRegion region) :=
    (Elaboration.mem_localOccurrences_child _ _ _).2
      (decide_eq_true_iff.mp selected)
  obtain ⟨original, _, mapped⟩ :=
    layout.frameSemanticOccurrences_complete region hne _ member
  cases original with
...
theorem localOccurrences_frameRegion
        ...
        fun child => decide
          ((layout.plugRegion child).parent? =
            some (layout.frameRegion region))).map
              Elaboration.LocalOccurrence.child =
    ((((VisualProof.Data.Finite.allFin input.frame.val.nodeCount).filter
      fun node => decide ((input.frame.val.nodes node).region = region)).map
        Elaboration.LocalOccurrence.node ++
      ((VisualProof.Data.Finite.allFin input.frame.val.regionCount).filter
      fun child => decide
        ((input.frame.val.regions child).parent? = some region)).map
          Elaboration.LocalOccurrence.child).map
  ...
  calc
    _ =
        ((List.filter
          (fun node => decide
            ((layout.plugNode node).region = layout.frameRegion region))
          ((VisualProof.Data.Finite.allFin input.frame.val.nodeCount).map
            layout.frameNode)).map Elaboration.LocalOccurrence.node ++
        (List.filter
          (fun node => decide
            ((layout.plugNode node).region = layout.frameRegion region))
          ((VisualProof.Data.Finite.allFin
      ...
      congr 1
      · exact (congrArg (List.map Elaboration.LocalOccurrence.node)
            (List.filter_append
              (p := fun node => decide
                ((layout.plugNode node).region = layout.frameRegion region))
              ((VisualProof.Data.Finite.allFin
      ...
      · exact (congrArg (List.map Elaboration.LocalOccurrence.child)
            (List.filter_append
              (p := fun child => decide
                ((layout.plugRegion child).parent? =
                  some (layout.frameRegion region)))
    ...
    _ = _ := by
      ...
      change
        List.map (Elaboration.LocalOccurrence.node ∘ layout.frameNode)
            sourceNodes ++
          List.map (Elaboration.LocalOccurrence.child ∘
            layout.frameRegion) sourceChildren =
        List.map layout.mapFrameOccurrence
          (List.map Elaboration.LocalOccurrence.node sourceNodes ++
            List.map Elaboration.LocalOccurrence.child sourceChildren)
      calc
        _ = List.map (layout.mapFrameOccurrence ∘
              Elaboration.LocalOccurrence.node) sourceNodes ++
            List.map (layout.mapFrameOccurrence ∘
              Elaboration.LocalOccurrence.child) sourceChildren := by
...
def occurrenceMap (presentation : TwoInputPresentation source target)
    ...
    Elaboration.LocalOccurrence source.plugLayout.plugRaw.regionCount
        source.plugLayout.plugRaw.nodeCount →
      Elaboration.LocalOccurrence target.plugLayout.plugRaw.regionCount
...
theorem occurrenceMap_mapFrameOccurrence
  ...
  | child child =>
      change Elaboration.LocalOccurrence.child
          (presentation.regionMap
...
theorem localOccurrences_map
  ...
  change Elaboration.localOccurrences target.coalesceFrameRaw
      targetFrame =
    (Elaboration.localOccurrences source.coalesceFrameRaw frame).map
      (castLocalOccurrence source.frame target.frame
        presentation.frameRegionCountEq presentation.frameNodeCountEq)
      at frameOccurrences
  rw [frameOccurrences]
  calc
    _ = List.map (target.plugLayout.mapFrameOccurrence ∘
          castLocalOccurrence source.frame target.frame
            presentation.frameRegionCountEq presentation.frameNodeCountEq)
        (Elaboration.localOccurrences source.coalesceFrameRaw frame) :=
      List.map_map
    _ = List.map
        (presentation.occurrenceMap (source.plugLayout.frameRegion frame)
          regular ∘ source.plugLayout.mapFrameOccurrence)
        (Elaboration.localOccurrences source.coalesceFrameRaw frame) := by
...
def relationMap
  ...
  subst targetRels
  exact Elaboration.identityRelationRenaming sourceRels
...
theorem binders_empty
    (presentation : TwoInputPresentation source target) :
    presentation.BinderRelated Elaboration.BinderContext.empty
...
def binderWitness_empty
    (presentation : TwoInputPresentation source target) :
    presentation.BinderWitness Elaboration.BinderContext.empty
      Elaboration.BinderContext.empty where
...
theorem binders_push_frameRegion
  ...
  · subst frame
    rw [Elaboration.BinderContext.push_self,
  ...
  · have sourceNe :
    ...
    have targetNe :
      ...
      exact values
    rw [Elaboration.BinderContext.push_other _ arity sourceNe,
      Elaboration.BinderContext.push_other _ arity targetNe,
      related frame]
...
private def depthAllowed
    (siteDirection direction : Elaboration.SimulationDirection)
    (depth : Nat) : Prop :=
...
inductive ContextWitness
    ...
    Elaboration.WireContext source.plugLayout.plugRaw →
      Elaboration.WireContext target.plugLayout.plugRaw → Type
  | root :
  ...
  | @rootFocusedChild rootFocused child sourceParent targetParent =>
      ...
      have targetMember :
          Elaboration.LocalOccurrence.child
            ...
            Elaboration.localOccurrences target.plugLayout.plugRaw
              (target.plugLayout.frameRegion target.site) :=
        (Elaboration.mem_localOccurrences_child _ _ _).2 targetParent
      have rawMember :
          Elaboration.LocalOccurrence.child target.site ∈
            Elaboration.localOccurrences target.coalesceFrameRaw
              target.site :=
      ...
      have rawParent :
          (target.frame.val.regions target.site).parent? =
            some target.site := by
        exact (Elaboration.mem_localOccurrences_child _ _ _).1 rawMember
      exact False.elim
        (Elaboration.checked_direct_child_not_encloses_parent
          target.frame.property rawParent
          (Diagram.Encloses.refl target.frame.val target.site))
...
def contextIndexRelation
    ...
    (targetContext : Elaboration.WireContext
      target.plugLayout.plugRaw) :
    Elaboration.ContextIndexRelation sourceContext.length
...
root context. -/
def rootExposedIndex (openDiagram : OpenDiagram)
...
def rootHiddenIndex (openDiagram : OpenDiagram)
...
@[simp] theorem rootEnvironment_rootExposedIndex
    ...
    (index : Fin openDiagram.exposedWires.length) :
    Elaboration.rootEnvironment openDiagram.exposedWires
        openDiagram.hiddenWires outer localEnv
        (rootExposedIndex openDiagram index) =
      outer index := by
  unfold Elaboration.rootEnvironment
...
theorem contextIndexRelation_of_resolved_frame_port
    ...
    (targetIndex : Fin targetContext.length)
    (sourceResolved :
      Elaboration.resolvePort? source.plugLayout.plugRaw sourceContext
        (source.plugLayout.frameNode node) port = some sourceIndex)
    (targetResolved :
      Elaboration.resolvePort? target.plugLayout.plugRaw targetContext
        (target.plugLayout.frameNode
  ...
  obtain ⟨sourcePlugWire, sourceOccurs, sourceGet⟩ :=
    Elaboration.resolvePort?_sound sourceResolved
  obtain ⟨targetPlugWire, targetOccurs, targetGet⟩ :=
    Elaboration.resolvePort?_sound targetResolved
...
theorem focusedFrameOccurrence_itemSimulation_of_compiled
    ...
    (member : occurrence ∈
      Elaboration.localOccurrences source.coalesceFrameRaw source.site)
    ...
    Elaboration.ItemSimulation model  direction
      (presentation.contextIndexRelation sourceContext targetContext)
  ...
  | node node =>
      have sourceNodeCompiled :
        ...
        simpa [Elaboration.compileOccurrenceWith?,
      ...
      apply Elaboration.compileNode?_itemSimulation_of_related_ports
        ...
        (Elaboration.identityRelationRenaming rels)
        (source.plugLayout.frameNode node)
        (target.plugLayout.frameNode
          (Fin.cast presentation.frameNodeCountEq node))
