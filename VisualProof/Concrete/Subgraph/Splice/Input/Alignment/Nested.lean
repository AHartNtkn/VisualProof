...
noncomputable def PlugLayout.canonicalToFramePreparedWire
    ...
    {sourceItems : ItemSeq  (sourceOuter + localWires) rels}
...
theorem PlugLayout.pairedRouteChild_eq
  ...
  have hsourceRaw :=
    VisualProof.Concrete.Splice.Input.RegionRoute.encloses sourceTail
  ...
  have htarget := VisualProof.Concrete.Splice.Input.RegionRoute.encloses
    targetTail (plugRaw_wellFormed input layout hadmissible)
  ...
  rcases Diagram.enclosingRegions_comparable htarget hsource' with
      htargetSource | hsourceTarget
  · rcases Elaboration.encloses_direct_child sourceMappedParent
        htargetSource with heq | hcycle
    · exact heq
    · exact False.elim
        (Elaboration.checked_direct_child_not_encloses_parent
          (plugRaw_wellFormed input layout hadmissible) targetParent' hcycle)
  · rcases Elaboration.encloses_direct_child targetParent'
        hsourceTarget with heq | hcycle
    · exact heq.symm
    · exact False.elim
        (Elaboration.checked_direct_child_not_encloses_parent
          (plugRaw_wellFormed input layout hadmissible) sourceMappedParent
          hcycle)
...
noncomputable def PlugLayout.pairedCompilerTraceContextIso
    ...
    (sourceTrace : @CompilerTrace  input.coalesceFrameRaw start
      sourceEnd sourcePath sourceOuter rels sourceBody sourceRoute
      sourceWitness sourceState)
    (targetTrace : @CompilerTrace  layout.plugRaw targetStart
      ...
      | @cut _ targetChild _ targetRest targetParent targetPosition
          ...
          have hmappedPosition :
            ...
            exact VisualProof.Data.Finite.indexOf?_get_eq_some_of_nodup
              (Elaboration.localOccurrences_nodup _ _) _
      ...
      case refine_3 =>
          ...
          have childBinderSpec : ∀ {arity}
            ...
            have hlookup := layout.frameRelationLookup_bubbleChild hadmissible
              ...
              Elaboration.BinderContext.empty sourceEnumeration
