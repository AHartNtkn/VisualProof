...
theorem compileHostNode_at_site
    ...
    Elaboration.compileNode?  layout.plugRaw
...
theorem compilePatternNode_at_site
    ...
    Elaboration.compileNode?  layout.plugRaw
      ...
      (Elaboration.compileNode?  input.pattern.val.diagram
...
theorem compilePatternRootNode_at_site
    ...
    Elaboration.compileNode?  layout.plugRaw
      ...
      (Elaboration.compileNode?  input.pattern.val.diagram
        ...
        Elaboration.BinderContext.empty node).map
          (fun item : Item
...
noncomputable def compilePatternRootNode_at_seam_iso
    ...
    (hsource : Elaboration.compileNode?
      input.pattern.val.diagram
      (input.pattern.val.exposedWires ++ input.pattern.val.hiddenWires)
      Elaboration.BinderContext.empty node = some sourceItem)
    (htarget : Elaboration.compileNode?  layout.plugRaw
      (outputLeaf.inheritedWires.extend (layout.frameRegion input.site))
      outputLeaf.binders (layout.patternNode node) = some targetItem) :
    ItemIso
...
noncomputable def compilePatternNode_at_material_iso
    ...
    (region : Fin input.pattern.val.diagram.regionCount)
    (hregion : input.binderSpine.IsMaterialRegion region)
    (sourceOuter : Elaboration.WireContext
      input.pattern.val.diagram)
...
noncomputable def compilePatternRegion_at_material
    ...
    (hsource : Elaboration.compileRegion?
  ...
  | succ sourceFuel ih =>
      ...
      | zero => simp [Elaboration.compileRegion?] at htarget
      | succ targetFuel =>
            ...
            | node node =>
                ...
                exact layout.compilePatternNode_at_material_iso  input
                  ...
                  (by simpa [sourceExtended,
                    Elaboration.compileOccurrenceWith?] using hsourceItem)
                  (by simpa [targetExtended, mapPatternOccurrence,
                    Elaboration.compileOccurrenceWith?] using htargetItem)
            | child child =>
                have hparent :=
                ...
                | sheet => simp [Elaboration.compileOccurrenceWith?, hchild]
                    at hsourceItem
                | cut parent =>
      ...
      | succ targetFuel =>
          ...
          have hoccurrence : ∀
            ...
            | child child =>
                ...
                have hchildPosition :
                  ...
                  · exact Or.inl (Elaboration.checked_encloses_trans
                      (input.coalesceFrameRaw_wellFormed hadmissible)
                      hbelow hparentChild)
                  · exact Or.inr fun hchildSite => haway
                      (Elaboration.checked_encloses_trans
                        (input.coalesceFrameRaw_wellFormed hadmissible)
          ...
          cases hsourceItems : Elaboration.compileOccurrencesWith?
              ...
              (Elaboration.compileRegion?  input.coalesceFrameRaw
                sourceFuel) sourceExtended sourceBinders
              (Elaboration.localOccurrences input.coalesceFrameRaw region) with
          | none => simp [sourceExtended, hsourceItems] at hsource
          | some sourceItems =>
              simp [sourceExtended, hsourceItems] at hsource
              subst sourceBody
...
theorem RegionRoute.encloses
  ...
  | here => exact Diagram.Encloses.refl _ _
  | @step start child target rest hparent position hposition tail ih =>
      have hdirect : d.Encloses start child := by
        ...
        simp [Diagram.climb, hparent]
      exact Elaboration.checked_encloses_trans hwf hdirect ih
...
theorem RegionRoute.distinctSibling_away
  ...
  rcases Diagram.enclosingRegions_comparable
      hsiblingTarget hchildTarget with hsiblingChild | hchildSibling
  · rcases Elaboration.encloses_direct_child childParent
        hsiblingChild with heq | hcycle
    · exact hne heq
    · exact Elaboration.checked_direct_child_not_encloses_parent
        hwf siblingParent hcycle
  · rcases Elaboration.encloses_direct_child siblingParent
...
noncomputable def compileFrameOccurrence_away_from_site
  ...
  | child child =>
      ...
      have hchildNeSite : child ≠ input.site := by
        intro heq
        subst child
        exact haway (Diagram.Encloses.refl _ _)
      have hparent :=
...
theorem compileFrameSiblings_targetCoordinates
  ...
  have hsourcePreparedLength : sourcePrepared.length =
      (Elaboration.localOccurrences input.coalesceFrameRaw region).length := by
    simpa [sourcePrepared] using hsourceLength
  have htargetPreparedLength : targetPrepared.length =
      (Elaboration.localOccurrences layout.plugRaw
        (layout.frameRegion region)).length := by
      ...
      have hsiblingParent :=
        (Elaboration.mem_localOccurrences_child _ _ _).1
          (show Elaboration.LocalOccurrence.child sibling ∈
            Elaboration.localOccurrences input.coalesceFrameRaw region by
              rw [← hsibling]
