import VisualProof.Concrete.Subgraph.Splice.Input.Layout.HostTransport
...
theorem terminalBody_inherited_mem_iff_exposed
  ...
  have notLocal : wire ∈ patternLeaf.inheritedWires →
    ...
    have hlocal : wire ∈ Elaboration.exactScopeWires
      ...
      (Elaboration.mem_exactScopeWires input.pattern.val.diagram
  ...
  · intro hinherited
    ...
    · by_cases hexposed : wire ∈ input.pattern.val.exposedWires
      ...
      · have hhidden : wire ∈ input.pattern.val.hiddenWires :=
          (OpenDiagram.mem_hiddenWires input.pattern.val wire).2
            ⟨hroot, hexposed⟩
  ...
  · intro hexposed
    have rootScope := input.pattern.property.boundary_is_root_scoped wire
      ((OpenDiagram.mem_exposedWires input.pattern.val wire).1 hexposed)
...
theorem patternSeamWireMapOfNonempty_spec
  ...
  · have hprepared : layout.patternSeamPreparedWireOfNonempty hadmissible host
    ...
    rw [hmap]
    rw [Elaboration.WireContext.extend_get_local]
...
theorem patternRootSeamWireMapOfEmpty_spec
  ...
  · have hmap :
    ...
    have hhidden : input.pattern.val.hiddenWires.get hidden ∉
        input.pattern.val.exposedWires :=
      (OpenDiagram.mem_hiddenWires input.pattern.val
...
noncomputable def patternMaterialWireIndexMap
  ...
  fun index =>
    ...
    Classical.choose (Elaboration.WireContext.lookup?_complete
...
theorem patternMaterialWireIndexMap_spec
  ...
  exact Classical.choose_spec (Elaboration.WireContext.lookup?_complete
    ((targetExact.mem_iff
...
def root (layout : PlugLayout input)
    (targetBinders : Elaboration.BinderContext
      layout.plugRaw targetRels) :
    PatternBinderWitness layout
      Elaboration.BinderContext.empty targetBinders where
  ...
  · intro hasArity
    ...
    change (targetBinders.push (layout.bodyRegion child) childArity)
      ...
      some ⟨arity,
        RelationRenaming.lift relationMap childArity
          ⟨tail.succ, hasArity⟩⟩
    rw [Elaboration.BinderContext.push_other targetBinders childArity hne,
