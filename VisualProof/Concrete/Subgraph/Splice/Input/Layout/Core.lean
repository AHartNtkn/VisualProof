...
def exposedPosition (_layout : PlugLayout input)
  ...
  (indexOf? input.pattern.val.boundary
    (input.pattern.val.exposedWires.get external)).get (by
      ...
      exact (OpenDiagram.mem_exposedWires _ _).1
        (List.get_mem _ _))
...
def mapFrameOccurrence (layout : PlugLayout input) :
    Elaboration.LocalOccurrence input.coalesceFrameRaw.regionCount
        input.coalesceFrameRaw.nodeCount →
      Elaboration.LocalOccurrence layout.plugRaw.regionCount
...
def mapPatternOccurrence (layout : PlugLayout input) :
    Elaboration.LocalOccurrence input.pattern.val.diagram.regionCount
        input.pattern.val.diagram.nodeCount →
      Elaboration.LocalOccurrence layout.plugRaw.regionCount
        layout.plugRaw.nodeCount
...
def semanticSiteOccurrences (layout : PlugLayout input) :
    List (Elaboration.LocalOccurrence layout.plugRaw.regionCount
      layout.plugRaw.nodeCount) :=
  (Elaboration.localOccurrences input.coalesceFrameRaw input.site).map
...
theorem mapFrameOccurrence_mem_localOccurrences (layout : PlugLayout input)
  ...
  | node node =>
      ...
      rw [Elaboration.mem_localOccurrences_node,
        Elaboration.mem_localOccurrences_node]
  ...
  | child child =>
      simp only [mapFrameOccurrence]
      rw [Elaboration.mem_localOccurrences_child,
        Elaboration.mem_localOccurrences_child]
...
theorem patternInternalWire_scope_material_or_bodyContainer
  ...
  · right
    have hnotBoundary : wire ∉ input.pattern.val.boundary := by
      intro hboundary
      exact hinternal
        ((OpenDiagram.mem_exposedWires input.pattern.val wire).2
          hboundary)
...
theorem mapPatternOccurrence_mem_localOccurrences_of_mem
    ...
    (occurrence : Elaboration.LocalOccurrence
      ...
      input.pattern.val.diagram.nodeCount)
    (hmem : occurrence ∈ Elaboration.localOccurrences
  ...
  | node node =>
      have hregion :=
        (Elaboration.mem_localOccurrences_node _ _ _).1 hmem
  ...
  | child child =>
      have hparent :=
        (Elaboration.mem_localOccurrences_child _ _ _).1 hmem
...
theorem semanticSiteOccurrences_subset (layout : PlugLayout input) :
    ∀ occurrence, occurrence ∈ layout.semanticSiteOccurrences →
      occurrence ∈ Elaboration.localOccurrences layout.plugRaw
        (layout.frameRegion input.site) := by
  ...
  | node node =>
      ...
      · intro hmem
        ...
        · apply List.mem_append_right
          ...
          exact (Elaboration.mem_localOccurrences_node _ _ _).2 hbody
  | child child =>
      revert hmem
      ...
      · let original := layout.materialRegions.origin materialChild
        ...
        | cut parent =>
            ...
            · exact (Elaboration.mem_localOccurrences_child _ _ _).2
                (hparentBody ▸ hparent)
            · exact congrArg Elaboration.LocalOccurrence.child
...
theorem mapFrameOccurrence_injective (layout : PlugLayout input) :
  ...
  · exact congrArg Elaboration.LocalOccurrence.node
      (layout.frameNode_injective
        (Elaboration.LocalOccurrence.node.inj heq))
  · contradiction
  · contradiction
  · exact congrArg Elaboration.LocalOccurrence.child
      (layout.frameRegion_injective
        (Elaboration.LocalOccurrence.child.inj heq))

def frameSemanticOccurrences (layout : PlugLayout input)
    (region : Fin input.coalesceFrameRaw.regionCount) :
    List (Elaboration.LocalOccurrence layout.plugRaw.regionCount
      layout.plugRaw.nodeCount) :=
  (Elaboration.localOccurrences input.coalesceFrameRaw region).map
    layout.mapFrameOccurrence

theorem frameSemanticOccurrences_complete (layout : PlugLayout input)
    (region : Fin input.coalesceFrameRaw.regionCount)
    (hne : region ≠ input.site) :
    ∀ occurrence, occurrence ∈ Elaboration.localOccurrences
        layout.plugRaw (layout.frameRegion region) →
      ∃ original, original ∈ Elaboration.localOccurrences
...
theorem frameSemanticOccurrences_nodup (layout : PlugLayout input)
  ...
  exact (Elaboration.localOccurrences_nodup
...
noncomputable def frameOccurrenceEquiv (layout : PlugLayout input)
    ...
    (hne : region ≠ input.site) :
    FiniteEquiv
      (Fin (Elaboration.localOccurrences input.coalesceFrameRaw
        region).length)
      (Fin (Elaboration.localOccurrences layout.plugRaw
        (layout.frameRegion region)).length) :=
  listEmbeddingEquiv layout.mapFrameOccurrence
    (Elaboration.localOccurrences input.coalesceFrameRaw region)
    (Elaboration.localOccurrences layout.plugRaw
      (layout.frameRegion region))
    (Elaboration.localOccurrences_nodup _ _)
    (Elaboration.localOccurrences_nodup _ _)
    (fun occurrence hmem =>
...
theorem frameOccurrenceEquiv_spec (layout : PlugLayout input)
    (region : Fin input.coalesceFrameRaw.regionCount)
    (hne : region ≠ input.site)
    (index : Fin (Elaboration.localOccurrences
...
def frameSemanticWires (layout : PlugLayout input)
  ...
  (Elaboration.exactScopeWires input.coalesceFrameRaw region).map
    layout.frameWire

theorem frameSemanticWires_subset (layout : PlugLayout input)
    (region : Fin input.coalesceFrameRaw.regionCount) :
    ∀ wire, wire ∈ layout.frameSemanticWires region →
      wire ∈ Elaboration.exactScopeWires layout.plugRaw
        (layout.frameRegion region) := by
  intro wire hmem
  obtain ⟨source, hsource, rfl⟩ := List.mem_map.mp hmem
  have hscope :=
    (Elaboration.mem_exactScopeWires input.coalesceFrameRaw region
      source).1 hsource
  rw [Elaboration.mem_exactScopeWires]
...
theorem frameSemanticWires_complete (layout : PlugLayout input)
    (region : Fin input.coalesceFrameRaw.regionCount)
    (hne : region ≠ input.site) :
    ∀ wire, wire ∈ Elaboration.exactScopeWires layout.plugRaw
        (layout.frameRegion region) →
  ...
  · intro hmem
    ...
    rw [Elaboration.mem_exactScopeWires] at hmem
...
theorem frameSemanticWires_nodup (layout : PlugLayout input)
  ...
  exact (Elaboration.exactScopeWires_nodup
...
noncomputable def frameLocalWireEquiv (layout : PlugLayout input)
  ...
  listEmbeddingEquiv layout.frameWire
    (Elaboration.exactScopeWires input.coalesceFrameRaw region)
    (Elaboration.exactScopeWires layout.plugRaw
      (layout.frameRegion region))
    (Elaboration.exactScopeWires_nodup _ _)
    (Elaboration.exactScopeWires_nodup _ _)
...
theorem frameLocalWireEquiv_spec (layout : PlugLayout input)
    (region : Fin input.coalesceFrameRaw.regionCount)
    (hne : region ≠ input.site)
    (index : Fin (Elaboration.exactScopeWires
      input.coalesceFrameRaw region).length) :
    (Elaboration.exactScopeWires layout.plugRaw
      (layout.frameRegion region)).get
...
@[simp] theorem Elaboration.WireContext.extend_get_outer
    (context : Elaboration.WireContext diagram)
    (region : Fin diagram.regionCount) (index : Fin context.length) :
    (context.extend region).get
        (Fin.cast (Elaboration.WireContext.length_extend context region).symm
          (Fin.castAdd
            (Elaboration.exactScopeWires diagram region).length index)) =
      context.get index := by
  simp [Elaboration.WireContext.extend]

@[simp] theorem Elaboration.WireContext.extend_get_local
    (context : Elaboration.WireContext diagram)
    (region : Fin diagram.regionCount)
    (index : Fin (Elaboration.exactScopeWires diagram region).length) :
    (context.extend region).get
        (Fin.cast (Elaboration.WireContext.length_extend context region).symm
          (Fin.natAdd context.length index)) =
      (Elaboration.exactScopeWires diagram region).get index := by
  simp [Elaboration.WireContext.extend]
...
noncomputable def frameExtendedWireMap (layout : PlugLayout input)
  ...
  fun index =>
    Fin.cast
      (Elaboration.WireContext.length_extend targetContext
      ...
      (Fin.addCases
        ...
        (Fin.cast
          (Elaboration.WireContext.length_extend sourceContext region)
          index))
    ...
    layout.frameSourceExtendedWireMap region sourceContext targetContext outerMap =
      extendWireRenaming outerMap
          (Elaboration.exactScopeWires input.coalesceFrameRaw
            region).length ∘
        Fin.cast
          (Elaboration.WireContext.length_extend sourceContext region) := by
  funext index
  let split := Fin.cast
    (Elaboration.WireContext.length_extend sourceContext region) index
  have hrecover : Fin.cast
      (Elaboration.WireContext.length_extend sourceContext region).symm
      split = index := by
    apply Fin.ext
    rfl
  rw [← hrecover]
...
theorem frameFinishRegion_renameWires_renameRelations
    ...
    ((Elaboration.finishRegion input.coalesceFrameRaw sourceContext region
        items).renameWires outerMap).renameRelations relationMap =
      .mk (Elaboration.exactScopeWires input.coalesceFrameRaw
          region).length
        ((items.renameWires
          (layout.frameSourceExtendedWireMap region sourceContext targetContext
            outerMap)).renameRelations relationMap) := by
  simp only [Elaboration.finishRegion, Region.renameWires,
...
theorem frameExtendedWireMap_spec (layout : PlugLayout input)
  ...
  · have hmap : layout.frameExtendedWireMap region hne sourceContext
          ...
          (Fin.cast
            (Elaboration.WireContext.length_extend sourceContext
              region).symm
            (Fin.castAdd
              (Elaboration.exactScopeWires input.coalesceFrameRaw
                region).length outer)) =
    ...
    rw [hmap]
    simpa only [Elaboration.WireContext.extend_get_outer] using
      outerSpec outer
  · have hmap : layout.frameExtendedWireMap region hne sourceContext
          targetContext outerMap
          (Fin.cast
      ...
      simp [frameExtendedWireMap]
    rw [hmap]
    simpa only [Elaboration.WireContext.extend_get_local] using
      layout.frameLocalWireEquiv_spec region hne localIndex

theorem mapPatternOccurrence_injective_on_body
    (layout : PlugLayout input) :
    ∀ left, left ∈ Elaboration.localOccurrences
        input.pattern.val.diagram input.binderSpine.bodyContainer →
      ∀ right, right ∈ Elaboration.localOccurrences
        input.pattern.val.diagram input.binderSpine.bodyContainer →
  ...
  | child leftChild =>
      ...
      | child rightChild =>
          have leftParent :=
            (Elaboration.mem_localOccurrences_child _ _ _).1 hleft
          have rightParent :=
            (Elaboration.mem_localOccurrences_child _ _ _).1 hright
          ...
          have rightMaterial := directChildOfBody_material input rightChild
            rightParent
          have hregions := Elaboration.LocalOccurrence.child.inj heq
          ...
          simpa only [materialIndex, SurvivorDomain.origin_index] using
            congrArg Elaboration.LocalOccurrence.child horigins
...
theorem semanticSiteOccurrences_nodup (layout : PlugLayout input) :
  ...
  · let bodyOccurrences := Elaboration.localOccurrences
      input.pattern.val.diagram input.binderSpine.bodyContainer
    have mappedNodup : ∀ items : List
        (Elaboration.LocalOccurrence
          input.pattern.val.diagram.regionCount
          input.pattern.val.diagram.nodeCount),
        items.Nodup →
...
def siteOccurrenceEquiv (layout : PlugLayout input) :
  ...
  FiniteEquiv.restrictLists
    (FiniteEquiv.refl (Elaboration.LocalOccurrence
...
theorem bodyInternalOriginalWires_mem_exactScopeWires_iff
    ...
    (hnonempty : input.binderSpine.proxyCount ≠ 0)
    (wire : Fin input.pattern.val.diagram.wireCount) :
    wire ∈ Elaboration.exactScopeWires input.pattern.val.diagram
        input.binderSpine.bodyContainer ↔
  ...
  · intro hlocal
    ...
    exact input.binderSpine.proxy_ne_root (input.terminalProxy hnonempty)
      himpossible
  · intro hmember
    exact (Elaboration.mem_exactScopeWires _ _ wire).2
...
def bodyInternalExactEquiv
    ...
    FiniteEquiv (Fin layout.bodyInternalCarriers.length)
      (Fin (Elaboration.exactScopeWires
...
theorem bodyInternalExactEquiv_spec
  ...
  have targetNodup := Elaboration.exactScopeWires_nodup
    input.pattern.val.diagram input.binderSpine.bodyContainer
...
theorem bodyInternalOriginalWires_mem_hiddenWires_iff
    ...
    wire ∈ input.pattern.val.hiddenWires ↔
      wire ∈ layout.bodyInternalOriginalWires := by
  rw [OpenDiagram.mem_hiddenWires,
...
def semanticSiteWires (layout : PlugLayout input) :
    List (Fin layout.plugRaw.wireCount) :=
  (Elaboration.exactScopeWires input.coalesceFrameRaw input.site).map
      layout.frameWire ++
...
theorem semanticSiteWires_subset (layout : PlugLayout input) :
  ...
  · obtain ⟨source, hsource, rfl⟩ := List.mem_map.mp hframe
    have hscope :=
      (Elaboration.mem_exactScopeWires _ _ _).1 hsource
...
theorem semanticSiteWires_complete (layout : PlugLayout input) :
  ...
  refine Fin.addCases (m := input.wireQuotient.count)
    ...
    (fun internal => ?_) wire
  · intro hmem
    have hsource : frame ∈ Elaboration.exactScopeWires
        input.coalesceFrameRaw input.site := by
      apply (Elaboration.mem_exactScopeWires _ _ _).2
      rw [Elaboration.mem_exactScopeWires] at hmem
...
theorem semanticSiteWires_nodup (layout : PlugLayout input) :
  ...
  change ((Elaboration.exactScopeWires input.coalesceFrameRaw
    input.site).map layout.frameWire ++
...
theorem siteLocalWireEquivOfNonempty_host_spec
  ...
  simp [FiniteEquiv.finCast, semanticSiteWires,
    extendWireEquiv]
  have hmap : index.val < ((Elaboration.exactScopeWires
...
theorem siteLocalWireEquivOfNonempty_pattern_spec
  ...
  let offset := (Elaboration.exactScopeWires
  ...
  have hright : ((Elaboration.exactScopeWires
  ...
  let leftIndex : Fin layout.bodyInternalCarriers.length :=
    ⟨offset - ((Elaboration.exactScopeWires input.coalesceFrameRaw
...
theorem material_climb_body_and_plug_site (layout : PlugLayout input) :
  ...
  | succ fuel ih =>
      ...
      | none => simp [Diagram.climb, hparent] at hclimb
      | some parent =>
          ...
          · obtain ⟨steps, horiginal, hplug⟩ :=
            ...
            refine ⟨1 + steps, ?_, ?_⟩
            · exact Elaboration.climb_add (by
                simp [Diagram.climb, hparent]) horiginal
          ...
          | some parent =>
              ...
              have hparentMaterial :
                ...
                · have hparentBody := nonmaterial_parent_eq_bodyContainer
                  ...
                  have hcycleRoot := Elaboration.climb_add
                    htail hbodyRoot
                  rw [hparentBody] at hcycleRoot
                  have hunique :=
                    Elaboration.ParentTraversal.climb_to_root_steps_unique
                      input.pattern.val.diagram
...
theorem material_climb_steps_le_count (layout : PlugLayout input)
  ...
  have pathIndex_injective : Function.Injective pathIndex := by
    ...
    have hfirstRoot := Elaboration.climb_add hfirstSuffix hbodyRoot
    have hsecondRoot := Elaboration.climb_add hsecondSuffix hbodyRoot
...
theorem frame_climb (layout : PlugLayout input) :
  ...
  | succ steps ih =>
      ...
      | sheet =>
          simp [Diagram.climb, hregion, CRegion.parent?] at hclimb
      | cut parent =>
          have htail : input.frame.val.climb steps parent = some finish := by
            simpa [Diagram.climb, hregion] using hclimb
          simp only [Diagram.climb, plugRaw,
  ...
  · intro hclimb
    ...
    | zero =>
        have heq : layout.frameRegion start = layout.frameRegion finish :=
          Option.some.inj hclimb
        simpa [Diagram.climb, layout.frameRegion_injective heq]
    | succ steps ih =>
        cases hregion : input.frame.val.regions start with
        | sheet =>
            simp [Diagram.climb, plugRaw, plugRegion_frameRegion,
              hregion, mapFrameRegion, CRegion.parent?] at hclimb
        | cut parent =>
            have htail : layout.plugRaw.climb steps
                (layout.frameRegion parent) =
                  some (layout.frameRegion finish) := by
              simpa [Diagram.climb, plugRaw, plugRegion_frameRegion,
                hregion, mapFrameRegion, CRegion.parent?] using hclimb
            simpa [Diagram.climb, hregion] using ih parent htail
        | bubble parent arity =>
            have htail : layout.plugRaw.climb steps
                (layout.frameRegion parent) =
                  some (layout.frameRegion finish) := by
...
theorem checked_encloses_antisymm
    {diagram : Diagram} (hroot : diagram.RootIsSheet)
  ...
  have hcycle : diagram.climb (leftSteps.val + rightSteps.val) right =
      some right := Elaboration.climb_add hleft hright
  have hcycleRoot := Elaboration.climb_add hcycle htoRoot
  have hlength :=
  ...
  rw [hzero] at hleft
  simpa [Diagram.climb] using hleft.symm
...
theorem plugRaw_all_regions_reach_root (layout : PlugLayout input) :
  ...
  · let original := layout.materialRegions.origin material
    ...
    have hplugRoot := Elaboration.climb_add hmaterialSite hframeRoot
...
theorem plugRaw_encloses_trans (layout : PlugLayout input)
  ...
  have hcomposed := Elaboration.climb_add hsecond hfirst
  have htoRoot := Elaboration.climb_add hcomposed hroot
  have hbound :=
    Elaboration.ParentTraversal.climb_to_root_steps_le_regionCount
...
theorem frame_encloses_iff (layout : PlugLayout input)
  ...
  · rintro ⟨steps, hsteps⟩
    ...
    have htoRoot := Elaboration.climb_add hframe hroot
    have hbound :=
      Elaboration.ParentTraversal.climb_to_root_steps_le_regionCount
