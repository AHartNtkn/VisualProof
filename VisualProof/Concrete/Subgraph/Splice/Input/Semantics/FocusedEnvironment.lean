...
theorem exactContextValue_eq
  ...
  have chosenWire : context.get chosen = wire :=
    Elaboration.WireContext.lookup?_sound chosenLookup
  have chosenEq : chosen = index := by
    exact (Elaboration.WireContext.lookup?_unique exact.nodup
...
noncomputable def siteQuotientEnvironment
    (input : Input )
    (context : Elaboration.WireContext input.plugLayout.plugRaw)
    (exact : context.Exact
...
noncomputable def focusedLocalEnvironmentOfEmpty
    ...
    Fin (Elaboration.exactScopeWires input.plugLayout.plugRaw
  ...
  fun index =>
    ...
    Fin.addCases
      (fun frame =>
        values ((Elaboration.exactScopeWires
...
@[simp] theorem focusedLocalEnvironmentOfEmpty_frame
    ...
    (values : input.wireQuotient.Carrier → D)
    (hiddenEnv : Fin input.pattern.val.hiddenWires.length → D)
    (frame : Fin (Elaboration.exactScopeWires
...
theorem focusedExtendedEnvironment_frameWire_eq
    ...
    Elaboration.extendedEnvironment context
  ...
  let split : Fin (context.length +
      (Elaboration.exactScopeWires layout.plugRaw region).length) :=
    Fin.cast
      (Elaboration.WireContext.length_extend context region) index
  have recover :
      Fin.cast
          (Elaboration.WireContext.length_extend context region).symm
          split =
        index := by
    apply Fin.ext
    rfl
  ...
  · have outerWire :
      ...
      exact
        (PlugLayout.Elaboration.WireContext.extend_get_outer
          context region outer).symm.trans canonicalWire
    unfold Elaboration.extendedEnvironment
    simp only [Function.comp_apply]
    have castEq :
        Fin.cast
            (Elaboration.WireContext.length_extend context
              (input.plugLayout.frameRegion input.site))
            (Fin.cast
              (Elaboration.WireContext.length_extend context region).symm
              (Fin.castAdd
  ...
  · have localWire :
        (Elaboration.exactScopeWires layout.plugRaw region).get
            localIndex =
          layout.frameWire quotient := by
      simpa only [region,
    ...
    have quotientLocal :
        coalescedQuotient ∈ Elaboration.exactScopeWires
          input.coalesceFrameRaw input.site := by
      have targetLocal :
          layout.frameWire quotient ∈
            Elaboration.exactScopeWires layout.plugRaw region := by
        rw [← localWire]
        exact List.get_mem _ localIndex
      have targetScope :=
        (Elaboration.mem_exactScopeWires layout.plugRaw region
    ...
    let mapped :=
      layout.siteLocalWireEquivOfEmpty hzero
        (Fin.castAdd input.pattern.val.hiddenWires.length frame)
    have mappedWire :
        (Elaboration.exactScopeWires layout.plugRaw region).get mapped =
          layout.frameWire quotient := by
    ...
    have localEq : localIndex = mapped := by
      ...
      exact (List.getElem_inj
        (Elaboration.exactScopeWires_nodup layout.plugRaw region)).mp
    ...
    have targetValue :
        ...
        apply Fin.ext
        exact congrArg Fin.val frameGet)
    simpa [Elaboration.extendedEnvironment, region, extendWireEnv] using
      targetValue
...
theorem patternRootItems_of_pattern_denote
  ...
  unfold Elaboration.finishRoot at substituted
...
theorem focusedExtendedEnvironment_patternRoot_eq
    ...
    (outerValues : ∀ quotient index,
      outputLeaf.inheritedWires.get index =
          input.plugLayout.frameWire quotient →
        outerEnv index = values quotient) :
    Elaboration.extendedEnvironment outputLeaf.inheritedWires
        (input.plugLayout.frameRegion input.site) outerEnv
        (focusedLocalEnvironmentOfEmpty input hzero values hiddenEnv) ∘
  ...
  · let rootIndex : Fin
    ...
    simp [split, rootIndex, Elaboration.extendedEnvironment,
