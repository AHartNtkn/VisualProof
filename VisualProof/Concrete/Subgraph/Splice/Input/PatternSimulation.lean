import VisualProof.Concrete.Subgraph.Splice.Input.Presentation.Transport
...
abbrev PatternContextWitness
    ...
    (targetContext : Elaboration.WireContext layout.plugRaw) : Type :=
...
def patternContextIndexRelation
  ...
  | some indexMap =>
      Elaboration.ContextIndexRelation.forwardMap indexMap
...
theorem patternLocalSelection
        ...
        | .forward => ∀ sourceLocal,
            ∃ targetLocal,
              (Elaboration.ContextIndexRelation.forwardMap
                ...
                (Elaboration.extendedEnvironment sourceContext region
  ...
  rw [Elaboration.ContextIndexRelation.environmentsAgree_forwardMap]
    at outerAgrees
  ...
  | forward =>
      ...
      apply (Elaboration.ContextIndexRelation.environmentsAgree_forwardMap
        _ _ _).2
      funext index
      let split := Fin.cast
      ...
      · simpa [Elaboration.extendedEnvironment,
  ...
  | backward =>
      ...
      · simpa [Elaboration.extendedEnvironment,
          materialExtendedWireMap, split, Function.comp_def, extendWireEnv] using
          congrFun outerAgrees outer
      · simp [Elaboration.extendedEnvironment,
...
noncomputable def patternConcreteSemanticSimulation
    ...
    Elaboration.ConcreteSemanticSimulation
      input.pattern.val.diagram layout.plugRaw model  where
  source_wellFormed := input.pattern.property.diagram_well_formed
  target_wellFormed := layout.plugRaw_wellFormed  input hadmissible
  ...
  localOccurrences_map := by
    ...
    | child child =>
        have parent :=
          (Elaboration.mem_localOccurrences_child _ _ _).1 member
  ...
  binders_push := by
    ...
    · subst binder
      rw [Elaboration.BinderContext.push_self] at sourceLookup
      ...
      simp [layout.binderRegion_material child hchild,
        ...
        Elaboration.BinderContext.head, RelationRenaming.lift]
    · rw [Elaboration.BinderContext.push_other sourceBinders arity
          ...
          have targetNe : layout.binderRegion binder ≠
            ...
            simpa [patternSimulationRegionMap, hchild] using
              layout.binderRegion_ne_bodyRegion_of_ne_material binder child
                hchild hbinder
          rw [Elaboration.BinderContext.push_other targetBinders arity
            targetNe]
  ...
  localTransport := by
    ...
    rcases atRegion with ⟨hregion, indexMap, witnessEq, indexSpec, memberSpec⟩
    subst witness
    refine Elaboration.directionalLocalTransport_of_agreement direction
      sourceContext targetContext region
      (layout.patternSimulationRegionMap region)
      (Elaboration.ContextIndexRelation.forwardMap indexMap)
      (Elaboration.ContextIndexRelation.forwardMap
        (layout.patternExtendedWireMap sourceContext targetContext region
          hregion indexMap)) model
  ...
  nodeSemantic := by
    ...
    subst context
    have targetNodeEq := Elaboration.LocalOccurrence.node.inj mapped
    subst targetNode
    ...
    · intro port sourceIndex targetIndex sourceResolved targetResolved
      have resolved := Elaboration.resolvePort?_map_of_occurrence
...
theorem compilePatternRegion_denote_at_material
    ...
    (targetEnumeration : Elaboration.BinderContext.Enumeration
...
theorem compilePatternRootOccurrence_at_site_simulation
  ...
  | node node =>
      ...
      simp only [Elaboration.compileOccurrenceWith?] at hsource
      change Elaboration.compileNode?  layout.plugRaw
      ...
      · intro region binder arity relation sourceShape sourceLookup
        simp [Elaboration.BinderContext.empty] at sourceLookup
      · exact hsource
      · exact htarget
  | child child =>
      have parentRoot :=
      ...
      | cut parent =>
          ...
          cases sourceResult : Elaboration.compileRegion?
              input.pattern.val.diagram input.pattern.val.diagram.regionCount
              child sourceContext Elaboration.BinderContext.empty with
          | none =>
              simp [Elaboration.compileOccurrenceWith?, childKind,
          ...
          | some sourceBody =>
              ...
              | some targetBody =>
                  ...
                  have bodies := layout.compilePatternRegion_denote_at_material
                    hadmissible model  direction.flip
      ...
      | bubble parent arity =>
          ...
          have targetKindAtSite : layout.plugRaw.regions
            ...
            simpa only [layout.bodyRegion_bodyContainer] using targetKind
          let sourcePushed := Elaboration.BinderContext.empty.push
            child arity
...
theorem focusedPatternOccurrence_itemSimulation
    ...
    let occurrenceIndex := Fin.cast
      (Elaboration.compileOccurrencesWith?_length
        (Elaboration.compileRegion?
          input.pattern.val.diagram input.pattern.val.diagram.regionCount)
        (input.pattern.val.exposedWires ++ input.pattern.val.hiddenWires)
        Elaboration.BinderContext.empty pattern.computation)
      patternIndex
  ...
  have sourceGet := Elaboration.compileOccurrencesWith?_get
    (Elaboration.compileRegion?  input.pattern.val.diagram
      input.pattern.val.diagram.regionCount)
    (input.pattern.val.exposedWires ++ input.pattern.val.hiddenWires)
    Elaboration.BinderContext.empty pattern.computation occurrenceIndex
  ...
  exact layout.compilePatternRootOccurrence_at_site_simulation hadmissible
    outputWitness outputLeaf hzero model  direction
    ((Elaboration.localOccurrences input.pattern.val.diagram
      input.pattern.val.diagram.root).get occurrenceIndex)
...
theorem denoteFocusedItems_of_patternRootItems_and_frame
    ...
    (frameDenotes :
      ∀ frameIndex : Fin (Elaboration.localOccurrences
          input.coalesceFrameRaw input.site).length,
        let occurrenceIndex :=
          layout.siteOccurrenceEquiv
        ...
        let itemIndex := Fin.cast
          ...
          occurrenceIndex
        denoteItem model  env relEnv (outputLeaf.items.get itemIndex)) :
  ...
  have patternLength :=
    Elaboration.compileOccurrencesWith?_length
      ...
      (input.pattern.val.exposedWires ++ input.pattern.val.hiddenWires)
      Elaboration.BinderContext.empty pattern.computation
  apply (denoteItemSeq_iff_get model  env relEnv outputLeaf.items).mpr
  intro targetIndex
  ...
  · let occurrenceIndex : Fin
    ...
    obtain ⟨actualIndex, actualIndexVal, simulation⟩ :=
      layout.focusedPatternOccurrence_itemSimulation hadmissible outputWitness
        outputLeaf hzero model  .forward patternIndex
    ...
    have renamedDenotes :=
      (denoteItem_renameRelations model
        (emptyRelationRenaming outputWitness.toFocus.holeRels)
...
theorem denotePatternRootItems_of_denoteFocusedItems
  ...
  have htargetLength :=
    Elaboration.compileOccurrencesWith?_length
      ...
      outputLeaf.binders outputLeaf.itemsComputation
  have hbody := input.binderSpine.body_eq_root_of_empty hzero
  apply (denoteItemSeq_iff_get (relCtx := []) model
    (env ∘ layout.patternRootWireIndexMap hadmissible hzero
      outputWitness outputLeaf)
  ...
  have simulation := layout.compilePatternRootOccurrence_at_site_simulation
    hadmissible outputWitness outputLeaf hzero model  .backward
    ((Elaboration.localOccurrences input.pattern.val.diagram
      input.pattern.val.diagram.root).get patternOccurrenceIndex)
  ...
  have targetDenotes :=
    (denoteItemSeq_iff_get model  env relEnv outputLeaf.items).mp
  ...
  have relationAgrees : RelEnv.Agrees
    ...
    simpa using RelEnv.pullback_agrees
      (emptyRelationRenaming outputWitness.toFocus.holeRels) relEnv
  exact (denoteItem_renameRelations model
    (emptyRelationRenaming outputWitness.toFocus.holeRels)
    (PUnit.unit : RelEnv model.Carrier []) relEnv relationAgrees
    (env ∘ layout.patternRootWireIndexMap hadmissible hzero
      outputWitness outputLeaf)
...
theorem pattern_denote_of_denoteFocusedItems
  ...
  apply pattern_denote_of_patternRootItems input hadmissible outputWitness
    outputLeaf hzero model  env fallback
  exact input.plugLayout.denotePatternRootItems_of_denoteFocusedItems input
    hadmissible outputWitness outputLeaf hzero model  env relEnv denotes
