...
def IsRepeat (pattern : OpenDiagram)
...
theorem indexOf?_not_repeat (pattern : OpenDiagram)
...
position wire as their sole free input. -/
...
def materializedBoundaryWire (pattern : OpenDiagram)
    (position : Fin pattern.boundary.length) :
...
theorem raw_boundary_injective (pattern : OpenDiagram)
...
theorem raw_boundary_nodup (pattern : OpenDiagram)
...
theorem boundary_length {pattern : CheckedOpen }
    {originalSpine : BinderSpine pattern.val.diagram}
    (certificate : Certificate pattern originalSpine) :
...
theorem boundary_nodup {pattern : CheckedOpen }
    {originalSpine : BinderSpine pattern.val.diagram}
    (certificate : Certificate pattern originalSpine) :
