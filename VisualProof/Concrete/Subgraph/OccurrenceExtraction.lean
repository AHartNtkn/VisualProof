...
known to be the canonical extraction of one exact host selection. Both sides
of the seam name the same ordered
...
structure ExtractionSpecialization
  ...
  bodyContainer_alignment :
    Fin.cast (congrArg Diagram.regionCount
      (congrArg OpenDiagram.diagram pattern_eq))
...
private def openIsoOfEq {source target : OpenDiagram}
    (equality : source = target) : OpenIso source target := by
  subst target
  exact OpenIso.refl source

private theorem openIsoOfEq_regions
    {source target : OpenDiagram} (equality : source = target)
    (region : Fin source.diagram.regionCount) :
    (openIsoOfEq equality).diagram.regions region =
      Fin.cast (congrArg Diagram.regionCount
        (congrArg OpenDiagram.diagram equality)) region := by
  subst target
  rfl
...
private theorem openIsoOfEq_nodes
    ...
    (openIsoOfEq equality).diagram.nodes node =
      Fin.cast (congrArg Diagram.nodeCount
        (congrArg OpenDiagram.diagram equality)) node := by
  subst target
  rfl
...
private theorem openIsoOfEq_wires
    ...
    (openIsoOfEq equality).diagram.wires wire =
      Fin.cast (congrArg Diagram.wireCount
        (congrArg OpenDiagram.diagram equality)) wire := by
  subst target
  rfl
  ...
  have extractedEncloses :
      (problem.host.val.extractDiagramRaw specialization.extractedSelection
        specialization.extractedLayout).Encloses
          specialization.extractedLayout.bodyContainer
          (specialization.extractedLayout.materialRegion material) :=
    Diagram.extractDiagramRaw_bodyContainer_encloses_materialRegion
      ...
      (Fin.addCases
        ...
        (fun material =>
          ((embedding.properRegionsEquiv.symm material).content problem).origin
            problem))
      (Fin.cast (by
        simp only [occurrenceFragment, Diagram.extractOpenRaw,
          Diagram.extractDiagramRaw, FragmentLayout.regionCount,
...
private theorem regionForward_root
  ...
  simp [regionForward, occurrenceFragment, occurrenceLayout,
    Diagram.extractOpenRaw, Diagram.extractDiagramRaw,
    FragmentLayout.root]
...
private theorem regionForward_fragmentParent_of_mappedOwner
  ...
  · obtain ⟨content, sourceOrigin, contentProper, targetMap⟩ := proper
    ...
    obtain ⟨index, indexTarget, fragmentParent⟩ :=
      Diagram.fragmentParent_selectedRegion problem.host
...
private theorem regionsEquiv_proxy_eq
  ...
  obtain ⟨sourceParent, sourceBinder⟩ :=
    Diagram.extractedBinderSpine_target_region problem.host
...
noncomputable def nodeForward
  ...
  fun node => (embedding.nodesEquiv.symm (Fin.cast (by
    simp [occurrenceFragment, Diagram.extractOpenRaw,
      Diagram.extractDiagramRaw, FragmentLayout.nodeCount]) node)).origin
        problem
...
private theorem regionForward_fragmentBinder_of_atomValid
  ...
  · obtain ⟨proxy, rfl, rfl⟩ := external
    ...
    rw [← binderEq,
      Diagram.fragmentBinder_externalBinder problem.host
...
private theorem required_port_is_covered
  ...
  | atom region binder =>
      ...
      | sheet =>
          simp [Diagram.RequiresPort, nodeKind, binderKind] at required
      | cut parent =>
          simp [Diagram.RequiresPort, nodeKind, binderKind] at required
      | bubble parent arity =>
          simp only [nodeKind, binderKind] at covered
          rw [Diagram.requiresPort_atom_bubble_iff diagram node port
            region binder parent arity nodeKind binderKind] at required
          obtain ⟨index, rfl⟩ := required
          exact covered index
  | identity region arity =>
      simp only [nodeKind] at covered
      rw [Diagram.requiresPort_identity_iff diagram node port region
...
private theorem renamedEndpoints_nodup
  ...
  exact Diagram.extractDiagramRaw_endpoints_are_nodup problem.host
...
private theorem internal_endpoint_mem_iff
  ...
  · intro member
    ...
    have origin := Diagram.fragmentEndpoint?_origin
    ...
    have sourceMember : sourceEndpoint ∈
      ...
      (problem.host.val.mem_extractDiagramRaw_internalWire_endpoints_iff
        ...
        sourceEndpoint).2 ⟨_, hostMember,
          Diagram.fragmentEndpoint_selectedNode embedding.selection
...
private theorem requiresPort_transport
  ...
  unfold Diagram.RequiresPort at required ⊢
...
private theorem boundary_endpoint_mem_iff
  ...
  · intro member
    ...
    have sourceRequired : specialization.occurrenceFragment.diagram.RequiresPort
        sourceEndpoint.node sourceEndpoint.port :=
      Diagram.extractDiagramRaw_endpoints_are_valid problem.host
        embedding.selection specialization.occurrenceLayout
        (specialization.occurrenceLayout.boundaryWire boundary)
...
theorem boundary_map
  ...
  have lengths :
    ...
    rw [List.length_map]
    simpa [occurrenceFragment, Diagram.extractOpenRaw,
      Diagram.extractBoundaryRaw] using
  ...
  let sourcePosition : Fin embedding.selection.touchingWires.length :=
    ⟨index, by
      simpa [occurrenceFragment, Diagram.extractOpenRaw,
        Diagram.extractBoundaryRaw] using sourceBound⟩
  ...
  have boundaryGet :
    ...
    simp [occurrenceFragment, Diagram.extractOpenRaw,
      Diagram.extractBoundaryRaw, sourcePosition, sourceBoundaryIndex]
...
noncomputable def concreteOccurrenceEquiv
    {embedding : OpenOccurrenceEmbedding problem}
    (specialization : ExtractionSpecialization embedding) :
    OccurrenceEquiv specialization.occurrenceFragment.diagram
