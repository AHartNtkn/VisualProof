...
import VisualProof.Concrete.Subgraph.Reindex
...
private theorem climb_prefix_exists {d : Diagram}
...
private theorem climb_cancel_prefix {d : Diagram}
...
theorem downward {d : Diagram} (hwf : d.WellFormed )
  ...
  exact ⟨root, hroot,
    Elaboration.checked_encloses_trans hwf hrootAncestor hencloses⟩

end SelectionRequest.SelectsRegion

/-- Dense survivor receipts shared by frame construction and decomposition. -/
structure FrameDomains (d : Diagram) (selection : CheckedSelection d) where
...
private theorem parent_encloses (d : Diagram)
...
theorem parent_survives (host : Checked )
  ...
  · right
    ...
    · subst region
      ...
      unfold Diagram.RootIsSheet at hsheet
...
theorem atomBinder_survives (host : Checked )
    (selection : CheckedSelection host.val)
    (domains : FrameDomains host.val selection)
...
theorem incidentWire_survives (host : Checked )
    (selection : CheckedSelection host.val)
    (domains : FrameDomains host.val selection)
...
private def frameRegion (d : Diagram)
...
private def frameNode (d : Diagram)
    (selection : CheckedSelection d) (domains : FrameDomains d selection)
...
private def frameEndpoints (d : Diagram)
    (selection : CheckedSelection d) (domains : FrameDomains d selection)
...
private def frameWire (d : Diagram)
    (selection : CheckedSelection d) (domains : FrameDomains d selection)
...
-/
def removeRaw (d : Diagram) (selection : CheckedSelection d)
    (domains : FrameDomains d selection := {}) : Diagram where
...
theorem removeRaw_root_is_sheet
  ...
  unfold RootIsSheet
  have hreindexed := Diagram.removeRaw_region_reindexed host selection domains
    domains.root
...
theorem removeRaw_climb
  ...
  | succ steps ih =>
      ...
      | some parent =>
          ...
          rw [Diagram.removeRaw_parent host selection domains hstart hparent]
          exact hframeTail
...
theorem removeRaw_climb_to_root_steps_lt_regionCount
  ...
  let pathSurvives (index : Fin (steps + 1)) :
    ...
    Exists.choose (Diagram.removeRaw_climb host selection domains
  ...
  have embed_injective : Function.Injective embed := by
    ...
    have hremaining :=
      Elaboration.ParentTraversal.climb_to_root_steps_unique host.val
...
theorem removeRaw_all_regions_reach_root
  ...
  have hbound :=
    Diagram.removeRaw_climb_to_root_steps_lt_regionCount
      host selection domains (domains.regions.origin_survives region) hhostClimb
  obtain ⟨hroot, hframeClimb⟩ :=
    Diagram.removeRaw_climb host selection domains
...
theorem removeRaw_encloses
  ...
  have hdescendantRoot :
      host.val.climb (steps.val + rootSteps.val) descendant =
        some host.val.root :=
    Elaboration.climb_add hclimb hancestorRoot
  have hbound :=
    Diagram.removeRaw_climb_to_root_steps_lt_regionCount
      host selection domains hdescendant hdescendantRoot
  obtain ⟨hfinish, hframeClimb⟩ :=
    Diagram.removeRaw_climb host selection domains hdescendant hclimb
  refine ⟨⟨steps.val, by
    change steps.val < domains.regions.count + 1
    omega⟩, ?_⟩
...
theorem removeRaw_atom_binders_are_bubbles
  ...
  | identity region arity =>
      rw [← hindex,
        Diagram.removeRaw_identity host selection domains hsurvives hnode]
      trivial
  | atom region binder =>
      ...
      rw [← hindex,
        Diagram.removeRaw_atom host selection domains hsurvives hnode]
      simp only
      rw [Diagram.removeRaw_bubble host selection domains
...
theorem removeRaw_atom_binders_enclose
  ...
  | atom region binder =>
      ...
      rw [← hindex,
        Diagram.removeRaw_atom host selection domains hsurvives hnode]
      exact Diagram.removeRaw_encloses host selection domains
...
theorem removeRaw_requiresPort_iff
  ...
  | identity region arity =>
      ...
      unfold Diagram.RequiresPort
      rw [Diagram.removeRaw_identity host selection domains hsurvives hnode]
      simp only [hnode]
  | atom region binder =>
      have hbubble := host.property.atom_binders_are_bubbles original
      simp only [hnode] at hbubble
      ...
      rw [domains.nodes.origin_index original hsurvives]
      unfold Diagram.RequiresPort
      rw [Diagram.removeRaw_atom host selection domains hsurvives hnode]
      simp only [hnode]
      rw [Diagram.removeRaw_bubble host selection domains
        hbinderSurvives hkind]
      simp only
...
theorem mem_removeRaw_wire_endpoints_iff
  ...
  rw [Diagram.removeRaw_wire_endpoints host selection domains]
...
theorem reindexEndpoint?_some_injective
    ...
    first = second := by
  rw [Diagram.reindexEndpoint?_origin domains hfirst,
    Diagram.reindexEndpoint?_origin domains hsecond]
...
theorem removeRaw_endpoints_are_nodup
  ...
  exact filterMap_nodup_of_some_injective
    ...
    (fun hfirst hsecond =>
      Diagram.reindexEndpoint?_some_injective domains hfirst hsecond)
...
theorem removeRaw_wire_endpoints_are_disjoint
  ...
  obtain ⟨firstOriginal, hfirstMember, hfirstMapped⟩ :=
    (Diagram.mem_removeRaw_wire_endpoints_iff host selection domains
      first endpoint).1 hfirst
  obtain ⟨secondOriginal, hsecondMember, hsecondMapped⟩ :=
    (Diagram.mem_removeRaw_wire_endpoints_iff host selection domains
      second endpoint).1 hsecond
  have hsame := Diagram.reindexEndpoint?_some_injective domains
...
theorem removeRaw_required_ports_are_covered
  ...
  | identity region arity =>
      ...
      exact Diagram.removeRaw_endpointOccurs_of_surviving
...
theorem removeRaw_endpoints_are_valid
  ...
  have horigin := Diagram.reindexEndpoint?_origin domains hreindex
  rw [horigin] at hvalid
  exact (Diagram.removeRaw_requiresPort_iff host selection domains
...
theorem removeRaw_wire_scopes_enclose
  ...
  obtain ⟨originalEndpoint, horiginalMember, hreindex⟩ :=
    (Diagram.mem_removeRaw_wire_endpoints_iff host selection domains
      wire endpoint).1 hendpoint
  have horiginal := Diagram.reindexEndpoint?_origin domains hreindex
  ...
  rw [Diagram.removeRaw_wire_scope host selection domains wire]
  ...
  | atom region binder =>
      ...
      rw [← hnodeIndex,
        Diagram.removeRaw_atom host selection domains hnodeSurvives hnode]
      exact Diagram.removeRaw_encloses host selection domains
  ...
  | identity region arity =>
      ...
      have hencloses : host.val.Encloses
        ...
        simpa only [hnode, CNode.region] using hhostEncloses
      rw [← hnodeIndex,
        Diagram.removeRaw_identity host selection domains hnodeSurvives hnode]
      exact Diagram.removeRaw_encloses host selection domains
        hscopeSurvives hregion hencloses
...
theorem removeRaw_wellFormed
  ...
  root_is_sheet := Diagram.removeRaw_root_is_sheet host selection domains
  only_root_is_sheet :=
    Diagram.removeRaw_only_root_is_sheet host selection domains
  all_regions_reach_root :=
    Diagram.removeRaw_all_regions_reach_root host selection domains
  atom_binders_are_bubbles :=
    Diagram.removeRaw_atom_binders_are_bubbles host selection domains
  atom_binders_enclose :=
    Diagram.removeRaw_atom_binders_enclose host selection domains
  endpoints_are_valid :=
    Diagram.removeRaw_endpoints_are_valid host selection domains
  endpoints_are_nodup :=
    Diagram.removeRaw_endpoints_are_nodup host selection domains
  wire_endpoints_are_disjoint :=
    Diagram.removeRaw_wire_endpoints_are_disjoint host selection domains
  required_ports_are_covered :=
    Diagram.removeRaw_required_ports_are_covered host selection domains
  wire_scopes_enclose :=
    Diagram.removeRaw_wire_scopes_enclose host selection domains

@[simp] theorem removeRaw_regionCount (d : Diagram)
    (selection : CheckedSelection d) (domains : FrameDomains d selection) :
    (d.removeRaw selection domains).regionCount = domains.regions.count := rfl

@[simp] theorem removeRaw_nodeCount (d : Diagram)
    (selection : CheckedSelection d) (domains : FrameDomains d selection) :
    (d.removeRaw selection domains).nodeCount = domains.nodes.count := rfl

@[simp] theorem removeRaw_wireCount (d : Diagram)
    (selection : CheckedSelection d) (domains : FrameDomains d selection) :
    (d.removeRaw selection domains).wireCount = domains.wires.count := rfl

@[simp] theorem removeRaw_root (d : Diagram)
    (selection : CheckedSelection d) (domains : FrameDomains d selection) :
    (d.removeRaw selection domains).root = domains.root := rfl

/-- Run the sole concrete well-formedness authority on the computed frame. -/
def removeChecked (host : Checked )
    (selection : CheckedSelection host.val)
    (domains : FrameDomains host.val selection := {}) :
