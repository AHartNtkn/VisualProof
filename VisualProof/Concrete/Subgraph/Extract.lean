...
private theorem selectsRegion_downward {d : Diagram}
...
theorem Diagram.enclosingRegions_comparable {d : Diagram}
...
@[simp] theorem mem_anchorChainInnerFirst (selection : CheckedSelection d)
  ...
  simp [anchorChainInnerFirst, Diagram.Encloses]
  ...
  | atom region candidate =>
      ...
      · obtain ⟨child, hchild, hchildEncloses⟩ := hsubtree
        ...
        · have hparent := selection.property.childRoots_direct child hchild
          rcases Elaboration.encloses_direct_child hparent hbinderChild with
            heq | hbinderAnchor
          · subst binder
            exact False.elim (hnotSelected
              ((selection.mem_selectedRegions child).2
                ⟨child, hchild, Diagram.Encloses.refl host.val child⟩))
          · exact hbinderAnchor
        · exact False.elim (hnotSelected
...
structure FragmentLayout (d : Diagram) (selection : CheckedSelection d) where
...
-/
structure BinderSpine (diagram : Diagram) where
  proxyCount : Nat
...
/-- Canonical dense enumeration of the material regions in a binder spine. -/
...
-/
structure TerminalBodyContract (openDiagram : OpenDiagram)
...
private def proxyArity (d : Diagram)
...
private def fragmentProxyRegion (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection)
...
private def fragmentMaterialRegion (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection)
...
private def fragmentRegion (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection) :
...
private def fragmentNode (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection)
...
private def fragmentEndpoints (d : Diagram)
...
private def fragmentInternalWire (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection)
...
private def fragmentBoundaryWire (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection)
    (index : Fin layout.boundaryWireCount) :
    CWire layout.regionCount layout.nodeCount :=
...
private def fragmentWire (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection) :
...
theorem touchingWire_scope_encloses_anchor
  ...
  · obtain ⟨child, hchild, hchildNode⟩ := hsubtree
    ...
    · have hparent := selection.property.childRoots_direct child hchild
      rcases Elaboration.encloses_direct_child hparent hscopeChild with
...
def extractDiagramRaw (d : Diagram) (selection : CheckedSelection d)
    (layout : FragmentLayout d selection := {}) : Diagram where
...
def extractBoundaryRaw (d : Diagram) (selection : CheckedSelection d)
    (layout : FragmentLayout d selection := {}) :
    List (Fin (d.extractDiagramRaw selection layout).wireCount) :=
  List.ofFn layout.boundaryWire

def extractOpenRaw (d : Diagram) (selection : CheckedSelection d)
    (layout : FragmentLayout d selection := {}) : OpenDiagram where
...
@[simp] theorem extractDiagramRaw_boundaryWire_scope
    (d : Diagram) (selection : CheckedSelection d)
    (layout : FragmentLayout d selection)
    (index : Fin layout.boundaryWireCount) :
...
@[simp] theorem extractDiagramRaw_root_region (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection) :
...
theorem extractDiagramRaw_proxy_region (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection)
    (index : Fin layout.proxyCount) :
...
theorem extractDiagramRaw_materialRegion (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection)
    (index : Fin layout.materialRegionCount) :
...
def extractedBinderSpine (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection := {}) :
...
theorem extractDiagramRaw_atom_binder_bubble
  ...
  · have huses : selection.UsesExternalBinder binder :=
    ...
    unfold proxyArity Diagram.binderArity?
    rw [hget, hkind]
    rfl
...
theorem extractDiagramRaw_endpointOccurs_of_selected
  ...
  · obtain ⟨index, hindex⟩ := indexOf?_complete hinternal
    ...
    rw [hget]
    simpa only [Diagram.EndpointOccurs, original] using hoccurs
...
theorem extractDiagramRaw_externalAtom
  ...
  rw [Diagram.fragmentBinder_externalBinder host selection layout proxy]
  rfl
...
private theorem selectedRegion_ne_root
  ...
  rw [heq] at hencloses
  have hchildRoot := Elaboration.encloses_sheet_eq
  ...
  | succ steps ih =>
      ...
      | cut parent =>
          ...
          have htail : host.val.climb steps parent = some finish := by
            simpa only [Diagram.climb, hparent] using hclimb
...
theorem extractDiagramRaw_climb_selected_steps_lt_materialRegionCount
  ...
  have pathIndex_injective : Function.Injective pathIndex := by
    ...
    have hfirstRoot := Elaboration.climb_add hfirstSuffix hfinishRoot
    have hsecondRoot := Elaboration.climb_add hsecondSuffix hfinishRoot
...
theorem extractDiagramRaw_all_regions_reach_root
  ...
  · obtain ⟨index, rfl⟩ := hmaterial
    ...
    have hchildBody :
      ...
      rfl
    have hmaterialBody :=
      Elaboration.climb_add hmaterialClimb hchildBody
    have hbodyRoot :=
      host.val.extractDiagramRaw_bodyContainer_climb selection layout
    have hmaterialRoot :=
      Elaboration.climb_add hmaterialBody hbodyRoot
...
theorem extractDiagramRaw_bodyContainer_encloses_materialRegion
  ...
  refine ⟨⟨steps.val + 1, by
    simp only [extractDiagramRaw, FragmentLayout.regionCount]
    omega⟩, ?_⟩
  exact Elaboration.climb_add hmaterialClimb hchildBody
...
theorem extractDiagramRaw_proxy_encloses_materialRegion
  ...
  refine ⟨⟨steps.val + 1 + (terminal.val - proxy.val), by
    ...
    omega⟩, ?_⟩
  exact Elaboration.climb_add hmaterialBody hbodyProxy
...
@[simp] theorem extractBoundaryRaw_length (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection) :
...
theorem extractBoundaryRaw_get_injective (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection) :
...
theorem extractBoundaryRaw_root_scoped (d : Diagram)
    (selection : CheckedSelection d) (layout : FragmentLayout d selection) :
...
structure RawExtraction (host : Diagram)
    (selection : CheckedSelection host) where
  layout : FragmentLayout host selection := {}
  fragment : OpenDiagram := host.extractOpenRaw selection layout
...
theorem check_complete {host : Checked }
    {selection : CheckedSelection host.val}
    (raw : RawExtraction host.val selection) :
    ∃ extraction, raw.check = .ok extraction := by
  have hwf : raw.fragment.diagram.WellFormed  := by
    rw [raw.fragment_exact]
    exact Diagram.extractDiagramRaw_wellFormed host selection raw.layout
  unfold check
  split
...
def extractChecked (host : Checked )
    (selection : CheckedSelection host.val) :
    Except WFError (CheckedExtraction  host selection) :=
