...
def spawnLiftEndpoint {nodeCount : Nat} (endpoint : CEndpoint nodeCount) :
    CEndpoint (nodeCount + 1) :=
  { node := endpoint.node.castSucc, port := endpoint.port }

def spawnLiftWire {regionCount nodeCount : Nat}
    (wire : CWire regionCount nodeCount) :
    CWire regionCount (nodeCount + 1) :=
...
def spawnNodeRaw (input : Concrete.Diagram)
    (node : CNode input.regionCount) (scope : Fin input.regionCount)
    (portCount : Nat) (port : Fin portCount → CPort) : Concrete.Diagram where
...
def spawnNodeWireProvenance (input : Concrete.Diagram)
    (node : CNode input.regionCount) (scope : Fin input.regionCount)
...
def spawnNodeWireTransport (input : Concrete.Diagram)
    (node : CNode input.regionCount) (scope : Fin input.regionCount)
    (portCount : Nat) (port : Fin portCount → CPort) :
    WireTransport input (spawnNodeRaw input node scope portCount port) :=
  WireTransport.append input
...
theorem spawnNodeWireTransport_transportBoundary
    ...
    WireTransport.transportBoundary
        (spawnNodeWireTransport input node scope portCount port) boundary =
      some (boundary.map fun wire =>
        Fin.cast (by rfl) (Fin.castAdd portCount wire)) := by
  apply WireTransport.transportBoundary_eq_map
  intro wire hmem
  simp [spawnNodeWireTransport, WireTransport.append,
    WireTransport.rootFiltered, spawnNodeRaw, spawnLiftWire,
...
theorem spawnNodeRawOpen_exposedWires
  ...
  unfold spawnNodeRawOpen OpenDiagram.exposedWires
...
@[simp] theorem spawnNodeRaw_newNode
    (input : Concrete.Diagram) (node : CNode input.regionCount)
...
theorem spawnNodeRaw_oldEndpointOccurs_backward
  ...
  · unfold Diagram.EndpointOccurs at h
    have hwire :
        (spawnNodeRaw input node scope portCount port).wires
...
structure SpawnContextEmbedding
    ...
    (scope : Fin input.regionCount) (portCount : Nat)
    (port : Fin portCount → CPort)
    (source : Elaboration.WireContext input)
    (target : Elaboration.WireContext
...
def mapOldContext
    (input : Concrete.Diagram) (portCount : Nat)
    (source : Elaboration.WireContext input) :
...
noncomputable def ofMem
  ...
  get := by
    intro i
    exact Elaboration.WireContext.lookup?_sound
...
theorem index_eq_of_get
  ...
  have hcandidateFound :=
    Elaboration.WireContext.lookup?_unique targetNodup hfound rfl
  have hembeddingFound :=
    Elaboration.WireContext.lookup?_unique targetNodup hfound
...
theorem spawnNodeRaw_exactScopeWires_mem_old_iff
    ...
    (port : Fin portCount → CPort) (wire : Fin input.wireCount) :
    Fin.castAdd portCount wire ∈
        Elaboration.exactScopeWires
          (spawnNodeRaw input node scope portCount port) region ↔
      wire ∈ Elaboration.exactScopeWires input region := by
  constructor
  · intro hmem
    have hscope := (Elaboration.mem_exactScopeWires
      (spawnNodeRaw input node scope portCount port) region
      (Fin.castAdd portCount wire)).mp hmem
    rw [spawnNodeRaw_oldWire_scope] at hscope
    exact (Elaboration.mem_exactScopeWires input region wire).mpr hscope
  · intro hmem
    have hscope :=
      (Elaboration.mem_exactScopeWires input region wire).mp hmem
...
noncomputable def extend
  ...
  ofMem (by
    intro wire
    unfold Elaboration.WireContext.extend
    constructor
    · intro hmem
...
theorem spawnNodeRaw_compileNode?_old
    ...
    (embedding : SpawnContextEmbedding input node scope portCount port
      source target)
    (binders : Elaboration.BinderContext input rels)
    ...
    Elaboration.compileNode?
      ...
      (Elaboration.compileNode?  input source binders old).map
  ...
  have hnode : spawned.nodes old.castSucc =
      ...
      | .identity region arity =>
          .identity (id region) arity := by
    rw [spawnNodeRaw_oldNode]
    cases input.nodes old <;> rfl
  have hports : ∀ endpointPort,
      Elaboration.resolvePort? spawned target old.castSucc endpointPort =
        (Elaboration.resolvePort? input source old endpointPort).map
          embedding.index := by
    intro endpointPort
  ...
  have hbinders : ∀ region binder,
    ...
    simp
  have hmap := Elaboration.compileNode?_map
     source target binders binders old old.castSucc
...
theorem spawnNodeRaw_exactScopeWires
    ...
    Elaboration.exactScopeWires
        (spawnNodeRaw input node scope portCount port) region =
      (Elaboration.exactScopeWires input region).map
          (Fin.castAdd portCount) ++
        if region = scope then
          (allFin portCount).map (Fin.natAdd input.wireCount)
        else [] := by
  unfold Elaboration.exactScopeWires filterFin
...
theorem spawnNodeRawOpen_hiddenWires
  ...
  have hold :
      List.filter
          ...
          ((Elaboration.exactScopeWires source.diagram
...
theorem OpenDiagram.rootWires_exact
    (source : OpenDiagram) (hwf : source.WellFormed ) :
    Elaboration.WireContext.Exact source.rootWires
      source.diagram.root := by
  constructor
  · exact source.rootWires_nodup
  · intro wire
    rw [OpenDiagram.mem_rootWires_iff source hwf]
    constructor
    · intro hscope
      rw [hscope]
...
theorem spawnNodeRawOpen_boundaryClass
  ...
  rw [OpenDiagram.boundaryClass_sound]
...
theorem spawnNodeRaw_exactScopeWires_length_of_ne
    ...
    (Elaboration.exactScopeWires
      ...
      (Elaboration.exactScopeWires input region).length := by
  rw [spawnNodeRaw_exactScopeWires, if_neg hne, List.append_nil]
  exact List.length_map _
  ...
  fun index =>
    Fin.cast
      ((congrArg (fun localCount => target.length + localCount)
        ...
        (Elaboration.WireContext.length_extend target region).symm)
      (extendWireRenaming embedding.index
        (Elaboration.exactScopeWires input region).length
        (Fin.cast
          (Elaboration.WireContext.length_extend source region) index))
...
theorem spawnNodeRaw_extendedWireMapOfNe_spec
  ...
  have hrecover : Fin.cast
      (Elaboration.WireContext.length_extend source region).symm
      split = index := by
    apply Fin.ext
    rfl
  rw [← hrecover]
  ...
  · have hmap : spawnNodeRaw_extendedWireMapOfNe embedding region hne
        (Fin.cast
          (Elaboration.WireContext.length_extend source region).symm
          (Fin.castAdd
            (Elaboration.exactScopeWires input region).length outer)) =
        Fin.cast
          (Elaboration.WireContext.length_extend target region).symm
          (Fin.castAdd
            (Elaboration.exactScopeWires
    ...
    rw [hmap]
    simpa [Elaboration.WireContext.extend] using embedding.get outer
  · let hlength := spawnNodeRaw_exactScopeWires_length_of_ne input node scope
    ...
    simp [Elaboration.WireContext.extend, hlist]
...
noncomputable def spawnNodeRaw_extendedWireMapAtScope
  ...
  fun index =>
    Fin.cast
      (Elaboration.WireContext.length_extend target scope).symm
      (Fin.addCases
        ...
        (Fin.cast
          (Elaboration.WireContext.length_extend source scope) index))
...
theorem spawnNodeRaw_extendedWireMapAtScope_spec
  ...
  let split := Fin.cast
    (Elaboration.WireContext.length_extend source scope) index
  have hrecover : Fin.cast
      (Elaboration.WireContext.length_extend source scope).symm
      split = index := by
    apply Fin.ext
    rfl
  rw [← hrecover]
  ...
  · have hmap : spawnNodeRaw_extendedWireMapAtScope embedding
        (Fin.cast
          (Elaboration.WireContext.length_extend source scope).symm
          (Fin.castAdd
            (Elaboration.exactScopeWires input scope).length outer)) =
        Fin.cast
          (Elaboration.WireContext.length_extend target scope).symm
          (Fin.castAdd
            (Elaboration.exactScopeWires
  ...
  · let hlength := spawnNodeRaw_exactScopeWires_length_at_scope input node scope
    ...
    change (List.map (Fin.castAdd portCount)
      ...
      List.map (Fin.natAdd input.wireCount) (allFin portCount))[localIndex.val] =
        Fin.castAdd portCount
          (Elaboration.exactScopeWires input scope)[localIndex.val]
...
theorem spawnNodeRaw_extendWireEnv_at_scope
    ...
    (localEnv : Fin (Elaboration.exactScopeWires
      (spawnNodeRaw input node scope portCount port) scope).length → D) :
    (extendWireEnv outerEnv localEnv ∘
        Fin.cast (Elaboration.WireContext.length_extend target scope)) ∘
      ...
      extendWireEnv (outerEnv ∘ embedding.index)
        ...
        Fin.cast (Elaboration.WireContext.length_extend source scope) := by
  funext wire
  let split := Fin.cast
    (Elaboration.WireContext.length_extend source scope) wire
  have hrecover : Fin.cast
      (Elaboration.WireContext.length_extend source scope).symm
      split = wire := by
    apply Fin.ext
...
theorem spawnNodeRaw_localOccurrences
    ...
    Elaboration.localOccurrences
      ...
      (filterFin fun old => decide ((input.nodes old).region = region)).map
          (fun old => Elaboration.LocalOccurrence.node old.castSucc) ++
        (if node.region = region then
          [Elaboration.LocalOccurrence.node (Fin.last input.nodeCount)]
        else []) ++
        (filterFin fun child =>
          decide ((input.regions child).parent? = some region)).map
            Elaboration.LocalOccurrence.child := by
  unfold Elaboration.localOccurrences filterFin
  ...
  have hold :
      (List.filter
        ...
        (filterFin fun old => decide ((input.nodes old).region = region)).map
          (fun old => Elaboration.LocalOccurrence.node
...
prefix injection. -/
def spawnNodeRaw_oldOccurrence (input : Concrete.Diagram) :
    Elaboration.LocalOccurrence input.regionCount input.nodeCount →
      Elaboration.LocalOccurrence input.regionCount
        (input.nodeCount + 1)
...
theorem spawnNodeRaw_localOccurrences_old_of_ne
    ...
    Elaboration.localOccurrences
        (spawnNodeRaw input node scope portCount port) region =
      (Elaboration.localOccurrences input region).map
  ...
  rw [if_neg hnodeNe]
  unfold Elaboration.localOccurrences
...
theorem spawnNodeRaw_compileOccurrenceWith?_old
    ...
    (sourceRecurse : ∀ {rels : RelCtx},
      (region : Fin input.regionCount) →
      (context : Elaboration.WireContext input) →
      Elaboration.BinderContext input rels →
      Option (Region  context.length rels))
    (targetRecurse : ∀ {rels : RelCtx},
      (region : Fin input.regionCount) →
      (context : Elaboration.WireContext
        (spawnNodeRaw input node scope portCount port)) →
      Elaboration.BinderContext
        (spawnNodeRaw input node scope portCount port) rels →
      Option (Region  context.length rels))
    (binders : Elaboration.BinderContext input rels)
    (occurrence : Elaboration.LocalOccurrence input.regionCount
      input.nodeCount)
    (targetNodup : target.Nodup)
    (targetDisjoint :
      (spawnNodeRaw input node scope portCount port).WireEndpointsAreDisjoint)
    (hrecurse : ∀ {childRels : RelCtx}
      ...
      (childBinders : Elaboration.BinderContext input childRels),
    ...
    Elaboration.compileOccurrenceWith?
      ...
      (Elaboration.compileOccurrenceWith?  input sourceRecurse
  ...
  | child child =>
      ...
      | sheet =>
          simp [Elaboration.compileOccurrenceWith?, spawnNodeRaw,
      ...
      | cut parent =>
          ...
          | none =>
              ...
              simp only [Elaboration.compileOccurrenceWith?,
...
theorem spawnNodeRaw_finishRegion_old_of_ne
    ...
    Elaboration.finishRegion
        (spawnNodeRaw input node scope portCount port) target region
        (items.renameWires
          (spawnNodeRaw_extendedWireMapOfNe embedding region hne)) =
      (Elaboration.finishRegion input source region items).renameWires
        embedding.index := by
  unfold Elaboration.finishRegion
...
theorem regionRoute_encloses
  ...
  | here region => exact Diagram.Encloses.refl input region
  | @step start child target rest hparent position hposition tail ih =>
      exact Elaboration.checked_encloses_trans hinput
...
private theorem checked_sibling_not_encloses_descendant
  ...
  have hselectedParent : input.climb (selectedSteps.val + 1) descendant =
      some parent := by
    apply Elaboration.climb_add hselectedClimb
    simp [Diagram.climb, hselected]
  have hotherParent : input.climb (otherSteps.val + 1) descendant =
      some parent := by
    apply Elaboration.climb_add hotherClimb
    simp [Diagram.climb, hother]
  have hselectedRoot :
      input.climb ((selectedSteps.val + 1) + rootSteps.val) descendant =
        some input.root :=
    Elaboration.climb_add hselectedParent hparentRoot
  have hotherRoot :
      input.climb ((otherSteps.val + 1) + rootSteps.val) descendant =
        some input.root :=
    Elaboration.climb_add hotherParent hparentRoot
  have hsteps :=
    Elaboration.ParentTraversal.climb_to_root_steps_unique input
...
theorem spawnNodeRaw_compileRegion?_old_of_not_encloses_node
    ...
    ∀ {rels : RelCtx} (fuel : Nat) (region : Fin input.regionCount)
      ...
      Elaboration.compileRegion?
          ...
          binders =
        (Elaboration.compileRegion?  input fuel region source
  ...
  | succ fuel ih =>
      ...
      have hnodeNe : region ≠ node.region := by
        intro equality
        subst region
        exact hnotAbove (Diagram.Encloses.refl input node.region)
      ...
      have hoccurrence : ∀ occurrence,
          occurrence ∈ Elaboration.localOccurrences input region →
          Elaboration.compileOccurrenceWith?
              (spawnNodeRaw input node scope portCount port)
              (Elaboration.compileRegion?
            ...
            (Elaboration.compileOccurrenceWith?  input
              (Elaboration.compileRegion?  input fuel)
        ...
        subst occurrence
        have hparent :=
          (Elaboration.mem_localOccurrences_child input region child).mp
            hmem
        ...
        have hchildNotAbove : ¬ input.Encloses child node.region := by
          intro hchildAbove
          exact hnotAbove (Elaboration.checked_encloses_trans hinput
      ...
      have hsequence := Elaboration.compileOccurrencesWith?_map
        (Elaboration.compileRegion?  input fuel)
        (Elaboration.compileRegion?
          (spawnNodeRaw input node scope portCount port) fuel)
        sourceExtended targetExtended binders binders
        (spawnNodeRaw_oldOccurrence input) extendedEmbedding.index
        (Elaboration.localOccurrences input region) hoccurrence
      have hsequence' :
          Elaboration.compileOccurrencesWith?
            ...
            (Elaboration.compileOccurrencesWith?  input
              (Elaboration.compileRegion?  input fuel)
      ...
      cases hsourceItems : Elaboration.compileOccurrencesWith?
          input (Elaboration.compileRegion?  input fuel)
          sourceExtended binders
          (Elaboration.localOccurrences input region) with
      | none =>
          have htargetItems := hsequence'
          rw [hsourceItems] at htargetItems
          simp only [Option.map_none] at htargetItems
          change (Elaboration.compileOccurrencesWith?
            (spawnNodeRaw input node scope portCount port)
            (Elaboration.compileRegion?
              (spawnNodeRaw input node scope portCount port) fuel)
            (target.extend region) binders
            ((Elaboration.localOccurrences input region).map
              ...
              (fun current => some (Elaboration.finishRegion
                (spawnNodeRaw input node scope portCount port) target region
                current)) = none
          rw [htargetItems]
          rfl
      | some items =>
          ...
          change (Elaboration.compileOccurrencesWith?
            ...
            some (Region.renameWires embedding.index
              (Elaboration.finishRegion input source region items))
          rw [htargetItems]
...
theorem spawnNodeRaw_compileRegion?_old_of_not_encloses
  ...
  | succ fuel ih =>
      ...
      have hne : region ≠ scope := by
        intro heq
        subst region
        exact hnotAbove (Diagram.Encloses.refl input scope)
      simp only [Elaboration.compileRegion?]
...
theorem localOccurrences_split_at_child
    (input : Concrete.Diagram) (parent selected : Fin input.regionCount)
    (position : Fin
      (Elaboration.localOccurrences input parent).length)
    (hposition : indexOf? (Elaboration.localOccurrences input parent)
      (.child selected) = some position) :
    ∃ before after,
      Elaboration.localOccurrences input parent =
        before ++ .child selected :: after ∧
      Elaboration.LocalOccurrence.child selected ∉ before ∧
      Elaboration.LocalOccurrence.child selected ∉ after := by
  let occurrences := Elaboration.localOccurrences input parent
  let before := occurrences.take position.val
  let after := occurrences.drop (position.val + 1)
  have hget : occurrences[position.val] =
      Elaboration.LocalOccurrence.child selected := by
  ...
  have hdecomp : occurrences =
      before ++ Elaboration.LocalOccurrence.child selected :: after := by
    rw [← List.take_append_drop position.val occurrences, hdrop]
  have hnodup := Elaboration.localOccurrences_nodup input parent
  ...
  have hawayBefore :
      Elaboration.LocalOccurrence.child selected ∉ before := by
    intro hmem
...
theorem spawnNodeRaw_compileOccurrencesAway
    ...
    {rest : List Nat}
    (tail : Concrete.Splice.RegionRoute input selected scope rest)
    (fuel : Nat)
    (source : Elaboration.WireContext input)
    (target : Elaboration.WireContext
      (spawnNodeRaw input node scope portCount port))
    ...
    (occurrences : List (Elaboration.LocalOccurrence
      input.regionCount input.nodeCount))
    (hlocal : ∀ occurrence, occurrence ∈ occurrences →
      occurrence ∈ Elaboration.localOccurrences input parent)
    (haway : Elaboration.LocalOccurrence.child selected ∉ occurrences) :
    Elaboration.compileOccurrencesWith?
        (spawnNodeRaw input node scope portCount port)
        (Elaboration.compileRegion?
          (spawnNodeRaw input node scope portCount port) fuel)
  ...
  apply Elaboration.compileOccurrencesWith?_map
  intro occurrence hmem
  ...
  subst occurrence
  have hchildParent :=
    (Elaboration.mem_localOccurrences_child input parent child).mp
...
theorem spawnNodeRaw_compileRootOccurrencesAway
    ...
    (occurrences : List (Elaboration.LocalOccurrence
      input.regionCount input.nodeCount))
    (hlocal : ∀ occurrence, occurrence ∈ occurrences →
      occurrence ∈ Elaboration.localOccurrences input input.root)
    (haway : Elaboration.LocalOccurrence.child selected ∉ occurrences) :
    Elaboration.compileOccurrencesWith?
        (spawnNodeRaw input node scope portCount port)
        (Elaboration.compileRegion?
          (spawnNodeRaw input node scope portCount port) fuel)
        target Elaboration.BinderContext.empty
        (occurrences.map (spawnNodeRaw_oldOccurrence input)) =
      (Elaboration.compileOccurrencesWith?  input
        (Elaboration.compileRegion?  input fuel)
        source Elaboration.BinderContext.empty occurrences).map
  ...
  apply spawnNodeRaw_compileOccurrenceWith?_old input node scope portCount port
    ...
    (Elaboration.compileRegion?
      (spawnNodeRaw input node scope portCount port) fuel)
    Elaboration.BinderContext.empty occurrence htargetExact.nodup
    htarget.wire_endpoints_are_disjoint
  intro childRels child childBinders heq
  subst occurrence
  have hchildParent :=
    (Elaboration.mem_localOccurrences_child input input.root child).mp
      (hlocal (.child child) hmem)
  have hchildNe : child ≠ selected := by
    intro heq
    subst child
...
theorem spawnNodeRaw_compileRegion?_positional_unaffected
  ...
  have hwire :
    ...
    rfl
  rw [hwire] at h
  cases hsource : Elaboration.compileRegion?  input fuel
...
theorem spawnNodeRaw_compileOldOccurrencesAtSite
    ...
    Elaboration.compileOccurrencesWith?
        ...
        ((Elaboration.localOccurrences input scope).map
          (spawnNodeRaw_oldOccurrence input)) =
      (Elaboration.compileOccurrencesWith?  input
        (Elaboration.compileRegion?  input fuel)
  ...
  subst occurrence
  have hparent :=
    (Elaboration.mem_localOccurrences_child input scope child).mp hmem
  have hchildNotAbove : ¬ input.Encloses child scope :=
    Elaboration.checked_direct_child_not_encloses_parent hinput hparent
  exact spawnNodeRaw_compileRegion?_old_of_not_encloses input node scope
    portCount port
...
theorem spawnNodeRaw_compileOldOccurrencesAtNodeSite
    ...
    Elaboration.compileOccurrencesWith?
        ...
        ((Elaboration.localOccurrences input node.region).map
          (spawnNodeRaw_oldOccurrence input)) =
      (Elaboration.compileOccurrencesWith?  input
        (Elaboration.compileRegion?  input fuel)
  ...
  subst occurrence
  have hparent :=
    (Elaboration.mem_localOccurrences_child input node.region child).mp
      hmem
...
theorem spawnNodeRaw_compileOccurrencesAwayFromNode
    ...
    (hparent : (input.regions selected).parent? = some parent)
    {rest : List Nat}
    (tail : Concrete.Splice.RegionRoute input selected node.region rest)
    (fuel : Nat)
    (source : Elaboration.WireContext input)
    (target : Elaboration.WireContext
      (spawnNodeRaw input node scope portCount port))
...
theorem spawnNodeRaw_compileOldOccurrencesAtRoot
    ...
    Elaboration.compileOccurrencesWith?
        ...
        (Elaboration.compileRegion?
          (spawnNodeRaw input node scope portCount port) fuel)
        target binders
        ((Elaboration.localOccurrences input input.root).map
          (spawnNodeRaw_oldOccurrence input)) =
      (Elaboration.compileOccurrencesWith?  input
        (Elaboration.compileRegion?  input fuel)
        source binders
  ...
  have hchildNotAbove : ¬ input.Encloses child scope := by
    rw [← hroot]
    exact Elaboration.checked_direct_child_not_encloses_parent hinput
      hparent
  exact spawnNodeRaw_compileRegion?_old_of_not_encloses input node scope
    portCount port hinput htarget hnode fuel child source target embedding
    childBinders hchildNotAbove
