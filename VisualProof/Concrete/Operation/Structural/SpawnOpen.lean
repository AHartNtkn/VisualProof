import VisualProof.Concrete.Operation.Structural.SpawnTransport
...
def SpawnRootSiteReflection
  ...
  ∀ (sourceBody : Region  source.val.exposedWires.length [])
    (targetBody : Region
      ...
      []),
    Elaboration.compileRoot?  source.val.diagram
...
private theorem spawnNodeRaw_compileRoot_route_kernel
    ...
    {path : List Nat}
    (route : Concrete.Splice.RegionRoute source.val.diagram
  ...
  | @step _ child _ rest hparent position hposition tail =>
      ...
      change (Elaboration.compileOccurrencesWith?  input
        (Elaboration.compileRegion?  input input.regionCount)
        source.val.rootWires Elaboration.BinderContext.empty
        (Elaboration.localOccurrences input input.root)).bind
          (fun items => some (Elaboration.finishRoot
      ...
      change (Elaboration.compileOccurrencesWith?
        ...
        (Elaboration.localOccurrences targetOpen.diagram input.root)).bind
          (fun items => some (Elaboration.finishRoot
      ...
      cases hsourceItems : Elaboration.compileOccurrencesWith?
          input (Elaboration.compileRegion?  input
      ...
      | some sourceItems =>
        ...
        cases htargetItems : Elaboration.compileOccurrencesWith?
            ...
            (Elaboration.compileRegion?  targetOpen.diagram
              input.regionCount) targetOpen.rootWires
            Elaboration.BinderContext.empty
            (Elaboration.localOccurrences targetOpen.diagram input.root) with
        ...
        | some targetItems =>
          ...
          obtain ⟨sourceBefore, sourceFocus, sourceAfter, hsourceBefore,
            ...
            compileOccurrencesWith?_frame_split
              (Elaboration.compileRegion?  input
                input.regionCount) source.val.rootWires
              Elaboration.BinderContext.empty before after (.child child)
          ...
          | succ childFuel =>
            ...
            have hsourceExact := OpenDiagram.rootWires_exact source.val
              source.property
            have htargetOpenWf := spawnNodeRawOpen_wellFormed source node spawnScope
              portCount port htarget
            have htargetExact := OpenDiagram.rootWires_exact targetOpen
              htargetOpenWf
            ...
            | cut parent =>
              ...
              have child_is_cut : input.regions child = .cut input.root :=
                childRegion
              have headDepth :
                  (Concrete.Splice.RegionRoute.step hparent position hposition
              ...
              simp only [spawnNodeRaw_oldOccurrence,
                Elaboration.compileOccurrenceWith?] at htargetFocus
              ...
              change (Elaboration.compileRegion?
                ...
                Elaboration.BinderContext.empty).bind
            ...
            | bubble parent arity =>
              ...
              change (Elaboration.compileRegion?
                targetOpen.diagram input.regionCount child targetOpen.rootWires
                (Elaboration.BinderContext.empty.push child _)).bind
...
theorem spawnNodeRawOpen_projects
  ...
  let targetWf := spawnNodeRawOpen_wellFormed source node scope portCount port
    htarget
  let target : CheckedOpen  := ⟨targetOpen, targetWf⟩
  let boundaryLength : targetOpen.boundary.length = source.val.boundary.length :=
    by simp [targetOpen, spawnNodeRawOpen]
  obtain ⟨sourceBody, hsourceCompile, hsourceElaborate⟩ :=
    CheckedOpen.elaborate_body_computation source
  obtain ⟨targetBody, htargetCompile, htargetElaborate⟩ :=
    CheckedOpen.elaborate_body_computation target
...
theorem spawnNodeRaw_compileNode?_complete
    ...
    (binders : Elaboration.BinderContext
    ...
    ∃ item, Elaboration.compileNode?
      (spawnNodeRaw input node scope portCount port) context binders
      (Fin.last input.nodeCount) = some item := by
  apply Elaboration.compileNode?_complete hwf hwires hbinders
  rw [spawnNodeRaw_newNode]
  exact hregion

private def checkRawReceipt (input : Checked )
    (raw : Concrete.Diagram) (provenance : WireProvenance input.val raw)
  ...
  | .ok result => .ok (OperationReceipt.ofChecked input raw provenance interface
      result hcheck)
      ...
      if input.val.Encloses binder region then
        ...
        checkRawReceipt input raw
          (spawnNodeWireProvenance input.val (.atom region binder) region arity
            (fun index => .arg index))
          (spawnNodeWireTransport input.val (.atom region binder) region
...
theorem applyBoundRelationSpawn_realizes
    ...
    result.Realizes
      ...
      (spawnNodeWireProvenance input.val (.atom region binder) region arity
        (fun index => .arg index))
      (spawnNodeWireTransport input.val (.atom region binder) region arity
        (fun index => .arg index)) := by
  unfold applyBoundRelationSpawn at happly
  split at happly <;> try contradiction
  split at happly <;> try contradiction
...
theorem applyBoundRelationSpawn_success (orientation : Orientation) (input : Checked )
    (region binder : Fin input.val.regionCount) (arity : Nat)
