...
theorem spawnNodeRaw_finishRegion_site_projects
    ...
    denoteRegion model  outerEnv relEnv
      ...
      denoteRegion model  (outerEnv ∘ embedding.index) relEnv
        (Elaboration.finishRegion input source scope sourceItems) := by
  unfold Elaboration.finishRegion
  simp only [denoteRegion_mk]
  rintro ⟨localEnv, htarget⟩
  let restrictedLocal :
      Fin (Elaboration.exactScopeWires input scope).length →
        model.Carrier :=
  ...
  apply (denoteItemSeq_renameWires model
    (Fin.cast (Elaboration.WireContext.length_extend source scope))
    ...
    (Fin.natAdd target.length
      (Fin.cast
        (spawnNodeRaw_exactScopeWires_length_at_scope input node scope
          portCount port).symm
        (Fin.natAdd
          (Elaboration.exactScopeWires input scope).length fresh)))
...
theorem spawnNodeRaw_freshExtendedIndex_get
  ...
  have hvalid : target.length +
      (Elaboration.exactScopeWires input scope).length + fresh.val <
      (target ++ Elaboration.exactScopeWires
        (spawnNodeRaw input node scope portCount port) scope).length := by
    rw [List.length_append,
      spawnNodeRaw_exactScopeWires_length_at_scope]
    omega
  let listIndex : Fin (target ++ Elaboration.exactScopeWires
  ...
  have hindex : spawnNodeRaw_freshExtendedIndex input node scope portCount port
    ...
    omega
  rw [hindex]
  change (target ++ Elaboration.exactScopeWires
  ...
  simp only [show target.length +
      (Elaboration.exactScopeWires input scope).length + fresh.val -
        target.length =
      (Elaboration.exactScopeWires input scope).length + fresh.val by
        omega]
  refine (List.getElem_append_right (as :=
      ...
      (i := (Elaboration.exactScopeWires input scope).length +
...
theorem spawnNodeRaw_finishRegion_site_reflects
    ...
    (hreflect : ∀ (rawEnv : Fin (target.extend scope).length → model.Carrier),
      ...
      denoteItemSeq model  rawEnv relEnv
        ...
        denoteItemSeq model  rawEnv relEnv targetItems) :
    denoteRegion model  (outerEnv ∘ embedding.index) relEnv
        (Elaboration.finishRegion input source scope sourceItems) →
      denoteRegion model  outerEnv relEnv
        (Elaboration.finishRegion
          (spawnNodeRaw input node scope portCount port) target scope
          targetItems) := by
  ...
  let targetLocal : Fin (Elaboration.exactScopeWires
      (spawnNodeRaw input node scope portCount port) scope).length →
    ...
    denoteRegion (relCtx := []) model  outerEnv PUnit.unit
      ...
      denoteRegion (relCtx := []) model
        (outerEnv ∘ spawnNodeRawOpenExternalClass source node scope portCount
          port) PUnit.unit
        (Elaboration.finishRoot source.exposedWires source.hiddenWires
          sourceItems) := by
  unfold Elaboration.finishRoot
  simp only [denoteRegion_mk]
  rintro ⟨localEnv, htarget⟩
...
theorem spawnNodeRaw_compileRegion_site_projects
    ...
    (hsourceBody : Elaboration.compileRegion?  input
      (fuel + 1) scope source binders = some sourceBody)
    (htargetBody : Elaboration.compileRegion?
      (spawnNodeRaw input node scope portCount port)
  ...
  let sourceChildren :=
    (filterFin fun child =>
      ...
      (Elaboration.LocalOccurrence.child (nodes := input.nodeCount))
  ...
  let fresh := [Elaboration.LocalOccurrence.node
    (regions := input.regionCount) (Fin.last input.nodeCount)]
  have hsourceOccurrences :
      Elaboration.localOccurrences input scope =
  ...
  have htargetOccurrences :
    ...
    simp only [sourceNodes, sourceChildren, targetNodes, targetChildren, fresh,
      List.map_map]
    rfl
  simp only [Elaboration.compileRegion?] at hsourceBody htargetBody
  cases hsourceItemsEq : Elaboration.compileOccurrencesWith?
      input (Elaboration.compileRegion?  input fuel)
      (source.extend scope) binders
      (Elaboration.localOccurrences input scope) with
  | none => simp [hsourceItemsEq] at hsourceBody
  | some sourceItems =>
    simp [hsourceItemsEq] at hsourceBody
    subst sourceBody
    cases htargetItemsEq : Elaboration.compileOccurrencesWith?
        (spawnNodeRaw input node scope portCount port)
    ...
    | some targetItems =>
      ...
      obtain ⟨nodeItems, restItems, hnodeItems, hrestItems,
          htargetItems⟩ :=
        Elaboration.compileOccurrencesWith?_append_split
          (Elaboration.compileRegion?
            (spawnNodeRaw input node scope portCount port) fuel)
      ...
      have holdCompile :
        ...
        exact Elaboration.compileOccurrencesWith?_append
          (Elaboration.compileRegion?
            (spawnNodeRaw input node scope portCount port) fuel)
...
theorem spawnNodeRaw_compileRoot_site_projects
    ...
    (targetBody : Region
      ...
      [])
    (hsourceBody : Elaboration.compileRoot?
      source.val.diagram source.val.exposedWires source.val.hiddenWires =
        some sourceBody)
    (htargetBody : Elaboration.compileRoot?
  ...
  have hsourceOccurrences :
      Elaboration.localOccurrences input input.root =
        sourceNodes ++ sourceChildren := by
    rfl
  have htargetOccurrences :
      Elaboration.localOccurrences targetOpen.diagram input.root =
        targetNodes ++ fresh ++ targetChildren := by
    change Elaboration.localOccurrences
    ...
    rfl
  change Elaboration.compileRoot?  input
      source.val.exposedWires source.val.hiddenWires = some sourceBody
    at hsourceBody
  change Elaboration.compileRoot?  targetOpen.diagram
      targetOpen.exposedWires targetOpen.hiddenWires = some targetBody
    at htargetBody
  simp only [Elaboration.compileRoot?] at hsourceBody htargetBody
  cases hsourceItemsEq : Elaboration.compileOccurrencesWith?
      input (Elaboration.compileRegion?  input input.regionCount)
      (source.val.exposedWires ++ source.val.hiddenWires)
      Elaboration.BinderContext.empty
      (Elaboration.localOccurrences input input.root) with
  | none => simp [hsourceItemsEq] at hsourceBody
  | some sourceItems =>
    simp [hsourceItemsEq] at hsourceBody
    subst sourceBody
    cases htargetItemsEq : Elaboration.compileOccurrencesWith?
        targetOpen.diagram
        (Elaboration.compileRegion?  targetOpen.diagram
          targetOpen.diagram.regionCount)
        (targetOpen.exposedWires ++ targetOpen.hiddenWires)
        Elaboration.BinderContext.empty
        (Elaboration.localOccurrences targetOpen.diagram
    ...
    | some targetItems =>
      ...
      have holdMap := spawnNodeRaw_compileOldOccurrencesAtRoot input node scope
        ...
        Elaboration.BinderContext.empty
        (OpenDiagram.rootWires_exact source.val source.property)
        (OpenDiagram.rootWires_exact targetOpen
      ...
      have hsourceItemsRoot :
          Elaboration.compileOccurrencesWith?  input
              (Elaboration.compileRegion?  input
                input.regionCount)
              source.val.rootWires Elaboration.BinderContext.empty
              (Elaboration.localOccurrences input input.root) =
            some sourceItems := by
  ...
  have hfresh : ∀ fresh : Fin portCount,
    ...
    have hindex : spawnNodeRawOpenFreshRootIndex source node scope portCount port
      ...
      simp only [spawnNodeRawOpenFreshRootIndex, rootFresh, hiddenFresh,
        Fin.val_cast, Fin.val_natAdd, OpenDiagram.rootWires,
        List.length_append]
      rw [hexposedLength]
      omega
    rw [hindex]
...
theorem compileOccurrencesWith?_frame_split
    ...
    (before after : List (Elaboration.LocalOccurrence
      d.regionCount d.nodeCount))
    (focus : Elaboration.LocalOccurrence d.regionCount d.nodeCount)
    ...
    ∃ beforeItems focusItem afterItems,
      Elaboration.compileOccurrencesWith?  d recurse context
          binders before = some beforeItems ∧
      Elaboration.compileOccurrenceWith?  d recurse context
  ...
  obtain ⟨beforeItems, restItems, hbefore, hrest, hitemsEq⟩ :=
    Elaboration.compileOccurrencesWith?_append_split recurse context
      binders before (focus :: after) items hitems
  simp only [Elaboration.compileOccurrencesWith?] at hrest
  cases hfocus : Elaboration.compileOccurrenceWith?  d recurse
      context binders focus with
  | none => simp [hfocus] at hrest
  | some focusItem =>
      cases hafter : Elaboration.compileOccurrencesWith?  d
...
theorem finishRegion_denote_mono
    (d : Concrete.Diagram) (context : Elaboration.WireContext d)
    (region : Fin d.regionCount)
    ...
    (relEnv : RelEnv model.Carrier rels) :
    denoteRegion model  outerEnv relEnv
        (Elaboration.finishRegion d context region sourceItems) →
      denoteRegion model  outerEnv relEnv
        (Elaboration.finishRegion d context region targetItems) := by
  unfold Elaboration.finishRegion
  simp only [denoteRegion_mk]
  ...
  rw [ItemSeq.castWiresEq_eq_renameWires] at hsource ⊢
  have hsourceRaw := (denoteItemSeq_renameWires model
    (Fin.cast (Elaboration.WireContext.length_extend context region))
...
private theorem spawnNodeRaw_compileRegion_route_kernel
    ...
    (route : Concrete.Splice.RegionRoute input start scope path)
  ...
  | @cut start child targetRegion rest depth hparent position hposition tail
      ...
      cases hsourceItems : Elaboration.compileOccurrencesWith?
          ...
          (Elaboration.localOccurrences input start) with
      | none => simp [hsourceItems] at hsourceBody
      | some sourceItems =>
        simp [hsourceItems] at hsourceBody
        subst sourceBody
        cases htargetItems : Elaboration.compileOccurrencesWith?
        ...
        | some targetItems =>
          ...
          cases fuel with
          | zero =>
              simp [Elaboration.compileOccurrenceWith?, child_is_cut,
                Elaboration.compileRegion?] at hsourceFocus
          | succ childFuel =>
                ...
                | some targetChild =>
                  ...
                  have hfinish' :
                      Elaboration.finishRegion
                          ...
                          (sourceItems.renameWires
                            (embedding.extend start).index) =
                        (Elaboration.finishRegion input source start
                          sourceItems).renameWires embedding.index := by
                    rw [hwire]
                    exact hfinish
                  constructor
  ...
  | @bubble start child targetRegion rest depth arity hparent position hposition
      ...
      | some sourceItems =>
        ...
        | some targetItems =>
          ...
          cases fuel with
          | zero =>
              simp [Elaboration.compileOccurrenceWith?, child_is_bubble,
                Elaboration.compileRegion?] at hsourceFocus
          | succ childFuel =>
              simp only [Elaboration.compileOccurrenceWith?,
...
private def routeCutDepthAux (diagram : Concrete.Diagram) :
...
private theorem routeCutDepthAux_hdepth
    {input : Concrete.Diagram} {start target : Fin input.regionCount}
    {path : List Nat}
    {route : Concrete.Splice.RegionRoute input start target path}
    (hdepth : Concrete.Splice.RegionRoute.HasCutDepth route depth) (fuel : Nat) :
...
theorem regionRoute_cutDepth_unique
    ...
    {path : List Nat}
    {route : Concrete.Splice.RegionRoute input start target path}
    (left : Concrete.Splice.RegionRoute.HasCutDepth route leftDepth)
    (right : Concrete.Splice.RegionRoute.HasCutDepth route rightDepth) :
