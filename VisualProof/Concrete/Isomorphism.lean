...
structure Iso (source target : Diagram) where
...
def refl (diagram : Diagram) : Iso diagram diagram where
  regionCount_eq := rfl
  nodeCount_eq := rfl
  wireCount_eq := rfl
  regions := .refl _
...
def symm {source target : Diagram}
    (iso : Iso source target) : Iso target source where
...
def trans {first second third : Diagram}
    (left : Iso first second) (right : Iso second third) :
...
@[simp] theorem refl_regions (diagram : Diagram) :
    (refl diagram).regions = .refl _ := rfl

theorem climb_transport {source target : Diagram}
    (iso : Iso source target) (steps : Nat)
...
theorem encloses_transport {source target : Diagram}
...
theorem reachesRoot_transport {source target : Diagram}
    (iso : Iso source target) {region : Fin source.regionCount}
    (h : source.ReachesRoot region) :
    target.ReachesRoot (iso.regions region) := by
  unfold Diagram.ReachesRoot at h ⊢
  rw [<- iso.root_eq]
  exact iso.encloses_transport h

theorem endpoint_mem_transport {source target : Diagram}
    (iso : Iso source target) {wire : Fin source.wireCount}
...
theorem endpoint_mem_reflect {source target : Diagram}
    (iso : Iso source target) {wire : Fin source.wireCount}
    {endpoint : CEndpoint source.nodeCount}
...
theorem target_region_eq {source target : Diagram}
    (iso : Iso source target) (region : Fin target.regionCount) :
...
theorem target_node_eq {source target : Diagram}
    (iso : Iso source target) (node : Fin target.nodeCount) :
    target.nodes node =
...
theorem endpoint_mem_pullback {source target : Diagram}
    (iso : Iso source target) {wire : Fin target.wireCount}
...
theorem endpointOccurs_transport {source target : Diagram}
    (iso : Iso source target) {wire : Fin source.wireCount}
    {endpoint : CEndpoint source.nodeCount}
...
theorem requiresPort_transport {source target : Diagram}
    (iso : Iso source target) {node : Fin source.nodeCount}
    {port : CPort} (h : source.RequiresPort node port) :
    target.RequiresPort (iso.nodes node) port := by
  unfold Diagram.RequiresPort at h ⊢
...
theorem rootIsSheet_transport {source target : Diagram}
    (iso : Iso source target) (h : source.RootIsSheet) :
    target.RootIsSheet := by
  unfold Diagram.RootIsSheet at h ⊢
  rw [<- iso.root_eq, <- iso.regions_eq source.root, h]
  rfl

theorem onlyRootIsSheet_transport {source target : Diagram}
    (iso : Iso source target) (h : source.OnlyRootIsSheet) :
    target.OnlyRootIsSheet := by
  unfold Diagram.OnlyRootIsSheet at h ⊢
  intro region hsheet
...
theorem allRegionsReachRoot_transport {source target : Diagram}
    (iso : Iso source target) (h : source.AllRegionsReachRoot) :
    target.AllRegionsReachRoot := by
  unfold Diagram.AllRegionsReachRoot at h ⊢
  intro region
...
theorem atomBindersAreBubbles_transport {source target : Diagram}
    (iso : Iso source target) (h : source.AtomBindersAreBubbles) :
    target.AtomBindersAreBubbles := by
  unfold Diagram.AtomBindersAreBubbles at h ⊢
  intro node
...
theorem atomBindersEnclose_transport {source target : Diagram}
    (iso : Iso source target) (h : source.AtomBindersEnclose) :
    target.AtomBindersEnclose := by
  unfold Diagram.AtomBindersEnclose at h ⊢
  intro node
  have hnodes := iso.target_node_eq node
  cases hs : source.nodes (iso.nodes.symm node) with
  | identity =>
...
theorem endpointsAreValid_transport {source target : Diagram}
    (iso : Iso source target) (h : source.EndpointsAreValid) :
    target.EndpointsAreValid := by
  unfold Diagram.EndpointsAreValid at h ⊢
...
theorem endpointsAreNodup_transport {source target : Diagram}
    (iso : Iso source target) (h : source.EndpointsAreNodup) :
    target.EndpointsAreNodup := by
  unfold Diagram.EndpointsAreNodup at h ⊢
  intro wire
...
theorem wireEndpointsAreDisjoint_transport {source target : Diagram}
    (iso : Iso source target) (h : source.WireEndpointsAreDisjoint) :
    target.WireEndpointsAreDisjoint := by
  unfold Diagram.WireEndpointsAreDisjoint at h ⊢
...
theorem requiredPortsAreCovered_transport {source target : Diagram}
    (iso : Iso source target) (h : source.RequiredPortsAreCovered) :
    target.RequiredPortsAreCovered := by
  unfold Diagram.RequiredPortsAreCovered at h ⊢
  intro node
  ...
  | identity region arity =>
      ...
      intro index
      exact covered (hsource index)
theorem wireScopesEnclose_transport {source target : Diagram}
    (iso : Iso source target) (h : source.WireScopesEnclose) :
    target.WireScopesEnclose := by
  unfold Diagram.WireScopesEnclose at h ⊢
  intro wire endpoint hmem
  let sourceWire := iso.wires.symm wire
  let sourceEndpoint := endpoint.rename iso.nodes.symm
...
def wellFormed_transport {source target : Diagram}
    (iso : Iso source target) (h : source.WellFormed ) : target.WellFormed  where
...
def checked_transport {source target : Diagram}
    (iso : Iso source target) (checked : Checked ) (hsource : checked.val = source) :
    Checked  := by
  subst source
