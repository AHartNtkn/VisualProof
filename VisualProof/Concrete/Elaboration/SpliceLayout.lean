...
@[simp] theorem frameRegion_eq_frameRegion_iff (layout : PlugLayout input)
    ...
    layout.frameRegion left = layout.frameRegion right ↔ left = right := by
  constructor
  · intro equality
    apply Fin.ext
    simpa [PlugLayout.frameRegion] using congrArg Fin.val equality
...
theorem mapPatternRegion_parent_frameRegion_site_eq
  ...
  | sheet =>
      ...
      exact (layout.frameRegion_eq_frameRegion_iff _ _).1 parentEq
  | cut parent =>
      simp only [PlugLayout.mapPatternRegion, CRegion.parent?,
        Option.some.injEq] at parentEq
...
theorem map_localOccurrences_frame (layout : PlugLayout input)
  ...
  unfold localOccurrences localNodeOccurrences localChildOccurrences
    mapFrameOccurrence frameNodeOccurrences frameChildOccurrences
...
theorem map_localOccurrences_body (layout : PlugLayout input) :
  ...
  unfold localOccurrences localNodeOccurrences localChildOccurrences
    mapPatternOccurrence bodyNodeOccurrences
  simp only [List.map_append, List.map_map]
  change
    (filterFin fun node : Fin input.pattern.val.diagram.nodeCount =>
      decide ((input.pattern.val.diagram.nodes node).region =
...
/-- The target node block at a retained frame region is exactly the retained
frame-node block followed by the inserted pattern-node block. -/
theorem localNodeOccurrences_frameRegion (layout : PlugLayout input)
    (region : Fin input.frame.val.regionCount) :
    localNodeOccurrences layout.plugRaw (layout.frameRegion region) =
      layout.frameNodeOccurrences region ++
        layout.patternNodeOccurrences region := by
  unfold localNodeOccurrences frameNodeOccurrences patternNodeOccurrences
  ...
  exact append_congr frameNodeBlock patternNodeBlock

/-- The target child block at a retained frame region is exactly the retained
frame-child block followed by the inserted material-child block. -/
theorem localChildOccurrences_frameRegion (layout : PlugLayout input)
    (region : Fin input.frame.val.regionCount) :
    localChildOccurrences layout.plugRaw (layout.frameRegion region) =
      layout.frameChildOccurrences region ++
        layout.materialChildOccurrences region := by
  unfold localChildOccurrences frameChildOccurrences materialChildOccurrences
  ...
  exact append_congr frameChildBlock materialChildBlock
...
theorem localOccurrences_frameRegion (layout : PlugLayout input)
    ...
    localOccurrences layout.plugRaw (layout.frameRegion region) =
      ...
      (layout.frameChildOccurrences region ++
        layout.materialChildOccurrences region) := by
  unfold localOccurrences localNodeOccurrences localChildOccurrences
    frameNodeOccurrences patternNodeOccurrences frameChildOccurrences
    materialChildOccurrences
