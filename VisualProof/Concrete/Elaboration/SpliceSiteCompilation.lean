...
private theorem terminalBinderMapped
  ...
  have binderEq : binder = input.binderSpine.proxy
    ...
    have owner := (CompiledSite.endpoint_binder_enumeration
...
theorem compileSpliceSite
    ...
    ∃ targetBody : CompiledRegion layout.plugRaw
      ...
      targetBody.nodeItems.erase =
        ItemSeq.append
          (ItemSeq.renameRelations (fun relation => relation)
            (ItemSeq.renameWires
              (frameSiteFullMap layout consistent admissible.terminal_body
                sourceOuter) sourceBody.nodeItems.erase))
          (ItemSeq.renameRelations relationMap
            (ItemSeq.renameWires
              (patternSiteFullMap layout consistent admissible sourceOuter
                sourceExact (CompiledSite.endpoint_fullContext_exact
                  (State.ofOpen input.pattern)
                  input.binderSpine.bodyContainer))
              (CompiledSite.directNodeItems (State.ofOpen input.pattern)
                input.binderSpine.bodyContainer).erase)) ∧
      targetBody.childItems.erase =
        ItemSeq.append
          (ItemSeq.renameRelations (fun relation => relation)
            (ItemSeq.renameWires
              (frameSiteFullMap layout consistent admissible.terminal_body
                sourceOuter) sourceBody.childItems.erase))
          (ItemSeq.renameRelations relationMap
            (ItemSeq.renameWires
              (patternSiteFullMap layout consistent admissible sourceOuter
                sourceExact (CompiledSite.endpoint_fullContext_exact
                  (State.ofOpen input.pattern)
                  input.binderSpine.bodyContainer))
              (CompiledSite.directChildItems (State.ofOpen input.pattern)
                input.binderSpine.bodyContainer).erase)) := by
  ...
  obtain ⟨sourceNodesCompiled, sourceChildrenCompiled⟩ :=
    CompilerCall.compile?_blocks input.frame.property
      (.nested input.site sourceOuter sourceRels sourceBinders)
      sourceCompiled
  have patternCompiled : patternCall.compile?
    ...
    exact CompiledSite.endpoint_computation patternState
      input.binderSpine.bodyContainer
  obtain ⟨patternNodesCompiled, patternChildrenCompiled⟩ :=
    CompilerCall.compile?_blocks input.pattern.property.diagram_well_formed
      patternCall patternCompiled
  ...
  let targetBody : CompiledRegion layout.plugRaw
      (.nested (layout.frameRegion input.site) targetOuter sourceRels
        targetBinders) :=
    .mk (frameNodes.append patternNodes)
      (frameChildren.append patternChildren)
  refine ⟨targetBody, ?_, ?_, ?_⟩
  ...
  · dsimp only [targetBody]
    change (frameNodes.append patternNodes).erase = _
    rw [CompiledItems.erase_append, frameNodesErase, patternNodesErase]
    rfl
  · dsimp only [targetBody]
    change (frameChildren.append patternChildren).erase = _
    rw [CompiledItems.erase_append, frameChildrenErase,
      patternChildrenErase]
    rfl
