...
private noncomputable def spliceCompilerRootSiteComputationCore
  ...
  rcases host with ⟨path, witness, siteRels, siteContext, route,
    ...
    siteBody_localCount, binder_covers, binder_enumeration, focus_wires,
    focus_rels, focus_body⟩
  cases derivation with
  | root =>
      let rootHost : CompiledSite source normalized.site := {
        ...
        fullWires := fullWires
        fullWires_eq := fullWires_eq
        fullWires_exact := fullWires_exact
        siteBody_localCount := siteBody_localCount
