...
abbrev WireContext (d : Diagram) := List (Fin d.wireCount)
...
theorem exactScopeWires_disjoint {d : Diagram}
...
theorem climb_add {d : Diagram}
  ...
  | succ first ih =>
      ...
      | some parent =>
          ...
          simpa [Nat.succ_add, Diagram.climb, hparent] using
            ih htail

theorem checked_encloses_trans {d : Diagram}
    (hwf : d.WellFormed )
...
theorem exists_direct_child_enclosing {d : Diagram}
    (hwf : d.WellFormed )
      ...
      | some parent =>
          ...
          · subst parent
            exact ⟨descendant, hparent,
              Diagram.Encloses.refl d descendant⟩
...
theorem checked_encloses_antisymm {d : Diagram}
    (hwf : d.WellFormed )
  ...
  have : second = first := by
    simpa [hzero, Diagram.climb] using hfirst
  exact this.symm
...
theorem encloses_direct_child {d : Diagram}
  ...
  | zero =>
      left
      simpa [Diagram.climb] using hsteps.symm
  | succ steps =>
      right
      refine ⟨⟨steps, by omega⟩, ?_⟩
      simpa [Diagram.climb, hparent] using hsteps

theorem encloses_sheet_eq {d : Diagram}
  ...
  rcases steps with ⟨steps, hbound⟩
  cases steps with
  | zero => simpa [Diagram.climb] using hsteps.symm
  | succ steps => simp [Diagram.climb, hsheet, CRegion.parent?] at hsteps

namespace WireContext

theorem root_exact {d : Diagram} (hwf : d.WellFormed ) :
  ...
  · intro wire
    ...
    · intro hscope
      rw [hscope]
      exact Diagram.Encloses.refl d d.root
    · exact encloses_sheet_eq hwf.root_is_sheet

theorem Exact.extend_child {d : Diagram}
  ...
  · intro wire
    ...
    · intro hwire
      ...
      · have hscope := (mem_exactScopeWires d child wire).mp hlocal
        rw [hscope]
        exact Diagram.Encloses.refl d child
    · intro hencloses
...
abbrev BinderContext (d : Diagram) (rels : RelCtx) :=
...
theorem empty_covers_root {d : Diagram} (hwf : d.WellFormed ) :
...
theorem checked_atom_binder_is_bubble {d : Diagram}
    (hwf : d.WellFormed )
  ...
  simpa [Diagram.AtomBindersAreBubbles, hnode]
    using hwf.atom_binders_are_bubbles node

theorem checked_atom_binder_available {d : Diagram}
    (hwf : d.WellFormed )
  ...
  simpa [Diagram.AtomBindersEnclose, hnode]
  ...
  encloses := by
    ...
    have parentChild : diagram.Encloses parent child := by
      refine ⟨⟨1, by
        have := child.isLt
        omega⟩, ?_⟩
      simp [Diagram.climb, hchild, CRegion.parent?]
    exact checked_encloses_trans hwf
...
def Enumeration.bubbleChild
  ...
  encloses := by
    intro index
    refine Fin.cases ?_ (fun tail => ?_) index
    · exact Diagram.Encloses.refl diagram child
    · have parentChild : diagram.Encloses parent child := by
        refine ⟨⟨1, by
          have := child.isLt
          omega⟩, ?_⟩
...
theorem endpointOwner?_complete {d : Diagram}
    {endpoint : CEndpoint d.nodeCount} {wire : Fin d.wireCount}
...
theorem endpoint_wire_unique {d : Diagram}
...
theorem endpointOwner?_unique {d : Diagram}
    (hdisjoint : d.WireEndpointsAreDisjoint)
...
def resolvePort? (d : Diagram) (context : WireContext d)
    (node : Fin d.nodeCount) (port : CPort) :
    Option (Fin context.length) := do
  let wire <- endpointOwner? d ⟨node, port⟩
  context.lookup? wire

def resolvePorts? (d : Diagram) (context : WireContext d)
...
theorem required_port_is_covered {d : Diagram}
  ...
  | atom region binder =>
      ...
      | sheet => simp [Diagram.RequiresPort, hnode, hbinder] at hrequired
      | cut parent => simp [Diagram.RequiresPort, hnode, hbinder] at hrequired
      | bubble parent arity =>
          simp only [hnode, hbinder] at hcovered
          rw [Diagram.requiresPort_atom_bubble_iff d node port region binder
            parent arity hnode hbinder] at hrequired
          obtain ⟨index, rfl⟩ := hrequired
          exact hcovered index
  | identity region arity =>
      simp only [hnode] at hcovered
      rw [Diagram.requiresPort_identity_iff d node port region arity hnode]
        at hrequired
      obtain ⟨index, rfl⟩ := hrequired
      exact hcovered index
