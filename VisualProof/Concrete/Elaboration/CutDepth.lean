...
private theorem concreteCutDepthAux_eq_of_climb_to_root
  ...
  | zero =>
      ...
      | succ fuel =>
          ...
          rw [show diagram.regions diagram.root = .sheet from
...
/-- Once the parent climb has reached the root, additional concrete depth
fuel cannot change the accumulated number of enclosing cuts. -/
...
/-- Entering a direct cut child adds exactly one to concrete cut depth. -/
theorem concreteCutDepth_cut_child
    ...
    concreteCutDepth diagram child =
      concreteCutDepth diagram parent + 1 := by
...
/-- Entering a direct bubble child preserves concrete cut depth. -/
