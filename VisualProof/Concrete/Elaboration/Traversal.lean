...
theorem exactScopeWires_nodup (d : Diagram)
...
theorem climb_succ_root_eq_none (d : Diagram)
    (hroot : d.RootIsSheet) (steps : Nat) :
    d.climb (steps + 1) d.root = none := by
  unfold Diagram.RootIsSheet at hroot
  simp [Diagram.climb, hroot, CRegion.parent?]

theorem climb_to_root_steps_unique (d : Diagram)
  ...
  | zero =>
      simp only [Diagram.climb_zero] at h₁
  ...
  | succ steps₁ ih =>
      ...
      | succ steps₂ =>
          ...
          | none => simp [Diagram.climb, hparent] at h₁
          | some parent =>
              have hparent₁ :
                  d.climb steps₁ parent = some d.root := by
                simpa [Diagram.climb, hparent] using h₁
              have hparent₂ :
                  d.climb steps₂ parent = some d.root := by
                simpa [Diagram.climb, hparent] using h₂
              exact congrArg Nat.succ (ih hparent₁ hparent₂)

theorem climb_to_root_steps_le_regionCount (d : Diagram)
