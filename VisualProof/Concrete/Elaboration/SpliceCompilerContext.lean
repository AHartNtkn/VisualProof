...
noncomputable def mapFrameContext (layout : PlugLayout input)
  ...
  context.map (layout.frameWireEmbedding consistent)

/-- The position map induced by stable frame-context embedding. -/
noncomputable def mapFrameContextEquiv (layout : PlugLayout input)
    (consistent : input.AttachmentConsistent)
    (context : WireContext input.frame.val) :
    FiniteEquiv (Fin context.length)
      (Fin (layout.mapFrameContext consistent context).length) :=
  FiniteEquiv.finCast (List.length_map (layout.frameWireEmbedding consistent)).symm

@[simp] theorem mapFrameContextEquiv_val
    ...
    (layout.mapFrameContextEquiv consistent context index).val = index.val := rfl

theorem mapFrameContext_get
    (layout : PlugLayout input) (consistent : input.AttachmentConsistent)
    (context : WireContext input.frame.val) (index : Fin context.length) :
    (layout.mapFrameContext consistent context).get
        (layout.mapFrameContextEquiv consistent context index) =
      layout.frameWireEmbedding consistent (context.get index) := by
...
theorem mapFrameContext_append
    ...
    layout.mapFrameContext consistent (first ++ second) =
      layout.mapFrameContext consistent first ++
        layout.mapFrameContext consistent second := by
...
theorem mapFrameContext_exactScopeWires
    ...
    layout.mapFrameContext consistent
      ...
      layout.frameLocalWires consistent region := rfl
...
theorem mapFrameBinders_push
  ...
  · subst candidate
    ...
    · have targetNe : layout.frameRegion frame ≠
        ...
        atBinder (layout.frameRegion_injective equality)
