...
of the interface, while endpoint order remains nonsemantic through `Iso`.
-/
structure OpenIso
    (source target : OpenDiagram) where
  diagram : Iso source.diagram target.diagram
  boundary : source.boundary.map diagram.wires = target.boundary

namespace OpenIso

def refl (diagram : OpenDiagram) :
    OpenIso diagram diagram where
  diagram := Iso.refl diagram.diagram
  boundary := by
...
def ofEq {source target : OpenDiagram}
    (equality : source = target) : OpenIso source target := by
  subst target
  exact refl source

def symm {source target : OpenDiagram}
    (iso : OpenIso source target) : OpenIso target source where
...
def trans {source middle target : OpenDiagram}
    (first : OpenIso source middle)
    (second : OpenIso middle target) :
    OpenIso source target where
...
theorem boundary_length_eq {source target : OpenDiagram}
...
theorem mem_exposedWires_iff {source target : OpenDiagram}
    (iso : OpenIso source target)
...
def exposedWiresEquiv {source target : OpenDiagram}
    (iso : OpenIso source target) :
...
theorem exposedWiresEquiv_spec {source target : OpenDiagram}
    (iso : OpenIso source target)
...
theorem mem_hiddenWires_iff {source target : OpenDiagram}
    (iso : OpenIso source target)
    (wire : Fin source.diagram.wireCount) :
...
def hiddenWiresEquiv {source target : OpenDiagram}
    (iso : OpenIso source target) :
...
theorem hiddenWiresEquiv_spec {source target : OpenDiagram}
    (iso : OpenIso source target)
...
def wellFormed_transport {source target : OpenDiagram}
    (iso : OpenIso source target) (h : source.WellFormed ) : target.WellFormed  where
...
def checked_transport {source target : OpenDiagram}
    (iso : OpenIso source target) (checked : CheckedOpen ) (hsource : checked.val = source) :
    CheckedOpen  := by
  subst source
