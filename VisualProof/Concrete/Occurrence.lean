...
ports. -/
structure OccurrenceEquiv (source target : Diagram) where
...
def ofConcreteIso {source target : Diagram}
    (iso : Iso source target) : OccurrenceEquiv source target where
...
def transOfRenameEq {source middle target : Diagram}
    (first : OccurrenceEquiv source middle)
    (second : OccurrenceEquiv middle target)
    (nodes_eq : ∀ node,
      (source.nodes node).rename first.regions =
        middle.nodes (first.nodes node)) :
    OccurrenceEquiv source target where
...
theorem node_region_eq {source target : Diagram}
...
structure OpenOccurrenceEquiv
  ...
  diagram : OccurrenceEquiv source.diagram target.diagram
  boundary : source.boundary.map diagram.wires = target.boundary

namespace OpenOccurrenceEquiv

def ofOpenConcreteIso {source target : OpenDiagram}
    (iso : OpenIso source target) : OpenOccurrenceEquiv source target where
  diagram := OccurrenceEquiv.ofConcreteIso iso.diagram
...
def transOfRenameEq {source middle target : OpenDiagram}
