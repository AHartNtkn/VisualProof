import VisualProof.Refinement.Implementation.DeiterationSpliceMap
...
theorem deiterationCoalescedScope_eq
  ...
  rw [IterationQuotient.coalescedScope_eq,
...
theorem deiterationCoalescedEndpoints_eq
  ...
  rw [IterationQuotient.coalescedEndpoints_eq,
...
@[simp] theorem deiterationQuotientEquiv_quotientWire
  ...
  exact IterationQuotient.quotientWireEquiv_quotientWire
...
theorem deiterationExposedAttachment_eq
  ...
  have sourceWire : source.pattern.val.boundary.get sourcePosition =
      source.pattern.val.exposedWires.get sourceExternal := by
    apply indexOf?_sound
    dsimp [sourcePosition, Splice.Input.PlugLayout.exposedPosition]
    exact (Option.some_get _).symm
  have targetWire : target.pattern.val.boundary.get targetPosition =
      target.pattern.val.exposedWires.get targetExternal := by
    apply indexOf?_sound
    dsimp [targetPosition, Splice.Input.PlugLayout.exposedPosition]
    exact (Option.some_get _).symm
