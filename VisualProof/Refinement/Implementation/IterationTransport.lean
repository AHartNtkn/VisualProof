import VisualProof.Refinement.Implementation.IterationAnchor
...
namespace VisualProof.Refinement.Implementation.IterationTransport

open VisualProof.Concrete
open VisualProof.Refinement.Implementation.IterationAnchor
...
def extractionContextRelation
    ...
    (hostIndex : Fin hostContext.length) : Prop :=
...
theorem iterationExposedAttachment_eq_fragmentOrigin
  ...
  have found : indexOf? spliceInput.pattern.val.boundary
      (spliceInput.pattern.val.exposedWires.get external) = some position := by
    unfold position Concrete.Splice.Input.PlugLayout.exposedPosition
    exact (Option.some_get (by
      rw [indexOf?_isSome_iff]
      exact (Concrete.OpenDiagram.mem_exposedWires _ _).1
        (List.get_mem _ _))).symm
  have exposedAtPosition := indexOf?_sound found
  ...
  have boundaryGet : spliceInput.pattern.val.boundary.get position =
    ...
    apply Fin.ext
    rfl
  have boundaryGetElem := boundaryGet
  simp only [List.get_eq_getElem] at boundaryGetElem
  have boundaryGetTarget : spliceInput.pattern.val.boundary[position] =
      layout.boundaryWire
        (Fin.cast (input.val.extractBoundaryRaw_length selection layout)
          position) := by
    exact boundaryGetElem
  rw [← exposedAtPosition]
  rw [boundaryGetTarget]
...
theorem iterationRootWire_sameWire
  ...
  have quotientSource : spliceInput.quotientWire
    ...
    change IterationQuotient.quotientWireEquiv input selection target
        (spliceInput.quotientWire (targetContext.get hostIndex)) =
      IterationQuotient.quotientWireEquiv input selection target
...
theorem iterationRootWireFactor
  ...
  apply compiledRouteTerminal_hostIndex_eq
    ...
    ((FiniteEquiv.finCast (List.length_map
      (IterationQuotient.coalescedFrameIso input selection target).wires).symm).symm
        hostIndex)
...
end VisualProof.Refinement.Implementation.IterationTransport
