...
theorem Internal.assigned_encloses
    (diagram : ConcreteDiagram definitionCount)
    (owner : CEndpoint diagram.nodeCount → diagram.WireId)
...
theorem Internal.replacementSkeleton_local_wire_signature
    {source : CheckedDiagram definitions}
    {wire : source.val.WireId}
    {sites : AllAppliedSites source wire}
    {spec : ReplacementSpec source wire sites}
...
theorem Internal.replacementSkeleton_encloses
    {source : CheckedDiagram definitions}
    {wire : source.val.WireId}
    {sites : AllAppliedSites source wire}
    {spec : ReplacementSpec source wire sites}
    ...
    | arg index =>
        ...
        | «local» fresh =>
            have ownerExact :
              ...
              rw [List.getElem?_eq_getElem bound, reference]
            rw [ownerExact]
            rw [Internal.replacementSkeleton_local_wire_signature]
            have localSignature :
...
private theorem replacementOwner_visible
  ...
  · intro retainedNode portRequired
    ...
    exact (Internal.replacementSkeleton_encloses (spec := spec) plan _ _).mpr <|
    ...
    | arg index =>
        ...
        | existing sourceWire =>
            ...
            change (replacementSkeleton plan).Encloses
              (retainedRegion source (source.val.wires sourceWire).scope)
              (retainedRegion source (sites.sites.get site).region)
            apply (Internal.replacementSkeleton_encloses (spec := spec) plan _ _).mpr
...
theorem Internal.checkedReplacementHead_sites_complete
    {source : CheckedDiagram definitions}
    {wire : source.val.WireId}
    {sites : AllAppliedSites source wire}
    {spec : ReplacementSpec source wire sites}
...
theorem Internal.replacementCandidate_wellFormed
    {source : CheckedDiagram definitions}
    {wire : source.val.WireId}
    {sites : AllAppliedSites source wire}
    {spec : ReplacementSpec source wire sites}
