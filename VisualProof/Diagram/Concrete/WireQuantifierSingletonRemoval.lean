...
/--
The checked base region image is the direct cast of the canonical raw
singleton-erasure region. This bridges erasure provenance to insertion without
exposing the checker owner's private cast helper.
-/
...
/--
The checked base wire image is the direct cast of the canonical raw
singleton-erasure wire. This is the wire-index analogue of
`rawTargetRegion_eq_baseRegionImage`.
-/
...
/--
Project an accepted prior context-above fact through the step-owned raw
singleton erasure. The checked base transport remains owned by the consuming
semantic receipt, so the dependent frame and this fact move together.
-/
...
@[simp] theorem rawTargetSite_eq_site
    ...
    Fin.cast
      ...
      step.site := by
  rw [rawTargetRegion_eq_baseRegionImage step,
      step.siteExact]
...
private theorem argumentOrigins_eq
    ...
    ∀ (args : List Sig) (position : Nat)
      ...
      ConcreteElaboration.ArgumentOrigins prior.val context node position
        ...
        ConcreteElaboration.variableOrigins prior.val context arguments =
          wires
  | [], position, .nil, wires, _origins, accepted => by
  ...
  | sig :: rest, position, .cons head tail, wires, origins, accepted => by
      ...
      simp only [relationArgumentWires?, headOwner, Option.bind_some] at accepted
      by_cases signature :
          (prior.val.wires
      ...
      · cases tailAccepted :
        ...
        | none => simp [signature, tailAccepted] at accepted
        | some tailWires =>
            have wiresExact :
                ConcreteElaboration.WireContext.origin prior.val context.ids
                    head ::
...
/--
Project the accepted site compilation to its relation-application singleton.
No compiler completion or target traversal is performed.
-/
