...
private def wireOfVar (diagram : ConcreteDiagram definitionCount) :
    {ids : List diagram.WireId} → {sig : Sig} →
      Var (ids.map fun id => (diagram.wires id).sig) sig →
      diagram.WireId
  | [], _, value => nomatch value
  | head :: _, _, .here => head
  | _ :: tail, _, .there value => wireOfVar diagram value
...
private theorem wireOfVar_member
  ...
  | cons head tail ih =>
      ...
      | here => simp [wireOfVar]
      | there value =>
          exact List.mem_cons_of_mem head (ih value)
...
private theorem resolveWireIn?_wireOfVar
  ...
  | cons head tail induction =>
      cases value with
      | here => simp [resolveWireIn?, wireOfVar]
      | there value =>
          ...
          simp only [resolveWireIn?, wireOfVar, notHead, ↓reduceDIte,
            tailResolved, Option.map_some]
          congr 1
...
private theorem encloses_iff_exists
    (diagram : ConcreteDiagram definitionCount)
...
private theorem compileNodes?_complete
    (definitions : List (List Sig))
    (diagram : ConcreteDiagram definitions.length)
    (wellFormed : diagram.WellFormed definitions)
    (context : WireContext diagram) (region : diagram.RegionId)
