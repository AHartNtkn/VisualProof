...
- [x] **Step 1: failing acceptance test** — the user's reported case as a fixture: a cut holding FOUR refs star-wired on their args (a heavy cohesive obstacle blob), wedged between two wired root refs. (Amended 2026-07-25 twice, both by measurement: a childless cut has NO trap — its packed layout is the global optimum, single bodies were always hop-reachable; and a 2-atom cut is ESCAPABLE by the old searcher (ratio 0.197) — only the 4-ref version isolates the subtree-move capability.) Committed numbers: trapped 909; old searcher stuck at ratio 1.000; annealer 110.5 (ratio 0.122); refDist 91→24 vs cut-circle diameter ~58; escape ~6 s of a 15 s budget. Fails against the current searcher.
