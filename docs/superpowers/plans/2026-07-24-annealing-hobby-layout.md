...
- [ ] **Step 1: failing acceptance test** — the user's reported case as a fixture: a CONTENT-BEARING cut (two atoms wired together inside it) between two wired ref nodes. (Amended 2026-07-25: the originally specified childless cut is a single anchor body the old searcher's hops can already move — measured to have NO trap, its packed layout IS the global optimum. The bug class is coordinated SUBTREE displacement, which requires a populated cut: measured trap 930 vs escaped basin ~233.) Seeded annealer, 60 s wall budget, worker-free: assert final best ≤ 0.6× the trapped score AND the wired pair's distance ends below the cut REGION circle's diameter (trapped 79 > ~50 > escaped 24). Fails against the current searcher.
