...
- **Lazy authoring mandate (user ruling 2026-07-23):** the rewritten frege keeps every defined node FOLDED until the proof step that actually needs its content — the old proof unfolded everything early and produced unreadable spaghetti regardless of layout. Unfold-latest is both the readable style and the natural one under drawn definitions (refs stay refs; K appears only at use). The re-authored plusComm becomes the gallery's real-diagram scene, replacing the unjudgeable old one.
- **Higher-order substitution demonstrations (user ruling 2026-07-23):** the library currently has NO example substituting into a quantifier of depth-2 sort. Add demonstration theorems exercising it end-to-end: at minimum (a) an explicit Eq_σ⃗ swap-lemma application chain (Eq's own quantifiers are rel(rel σ⃗, rel σ⃗) — its instantiations ARE depth-2 substitutions), and (b) one small named example over a predicate-transformer sort, e.g. a Knaster–Tarski-style μ-introduction instance (∀Φ at rel(rel(ι),ι)) from the library roadmap. These double as the first real tests of depth-2 comprehension in anger.
