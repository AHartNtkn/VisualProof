...
function paint(ctx: CanvasRenderingContext2D, e: Engine, w: number, h: number, st: Style, wireMode: WireMode): void {
  ...
  for (const [rid, g] of rs) {
    ...
    if (st.insetCuts) {
      ...
      grad.addColorStop(1, 'rgba(58,48,32,0.13)')
      ctx.fillStyle = grad
      ctx.fill()
    }
  ...
  for (const b of e.bodies.values()) {
    ...
    for (const a of g.arcs) {
      ...
      ctx.strokeStyle = atomHue ?? st.ink
      ctx.lineWidth = atomHue !== null ? 2.2 : a.kind === 'lam' ? 1.8 : 1.4
      ctx.stroke()
    }
    for (const r of g.radials) {
      ...
      ctx.strokeStyle = atomHue ?? st.ink
      ctx.lineWidth = atomHue !== null ? 2.0 : 1.4
      ctx.stroke()
    }
    if (node.kind === 'term') {
      if (g.exitArc !== null) {
    ...
    for (const s of b.satellites) {
      ...
      ctx.strokeStyle = st.ink; ctx.lineWidth = 1.2; ctx.stroke()
    }
    ctx.restore()
    for (const s of b.satellites) {
...
const NEUTRAL: Style = {
  name: 'neutral', canvas: '#e8e4d8', paper: '#faf7ee', ink: '#2a2118', frame: '#7a7263',
  wire: '#1a3a3f', wireW: 2.4, negFill: 'rgba(90, 78, 58, 0.11)', rimW: 1.3,
  discFill: '#fffdf6', discText: '#2a2118', font: 'Georgia, serif', sansLabels: false,
  insetCuts: false, discShadow: false, wireGlow: false, bubbleLightness: 46, junctionRing: false,
}

const MODES: WireMode[] = ['W1 Hobby', 'W2 Escape-arc', 'W3 Elastic chain', 'W4 Straight+fillet']
...
for (const [name, d, boundary] of cases) {
  ...
  for (const mode of MODES) {
    addCell(`${name} — ${mode}`, (ctx) => paint(ctx, e, CELL_W, CELL_H, NEUTRAL, mode))
  }
}

document.getElementById('status')!.textContent =
  'render lab — round 6: wire GEOMETRY candidates on corrected base (minimal circles, rest-length rhythm, tangent-continuous junctions)'
