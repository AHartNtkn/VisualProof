...
**Authority:** The reference implementations are the inert archived modules under `docs/superpowers/plans/2026-07-04-ui-lab-final/ui-lab/` — `shared.ts` (brush, LabCtx, physics handles), `composite.ts` (EDIT gestures), `spawn.ts` (cascade), `verdict.ts` (PROVE moves + citation), `session5.ts` (cursor-based TrackLab), `chrome.ts` (mode machine + minimal chrome), `history.ts` (scrubber + zoom thumbnails), `round7.ts` (motion layers), plus production `src/view/morph.ts`. The verdict record is `2026-07-03-plan-19-interface-overhaul.md`; standing laws live in the memory files (`port-names-are-not-semantic`, `rendering-laws`, `loose-ends-are-bodies`, `backward-is-flipped-polarity`, `named-defs-never-inside-terms`). NO DUAL SYSTEMS: the archived lab is evidence only, and production under `src` is the sole executable interaction authority.
...
**Files:** `src/app/interact/moves.ts` (laws 13–17, 19 from the archived `verdict.ts`, corrected by `docs/superpowers/specs/2026-07-10-proof-interaction-integration-design.md`: contextual Delete with orphan-riding erasure, drag-iterate, dbl-click normalize chain, pointer-local refusals, no duplicate feedback, folded instantiate — steps emitted through one orientation-aware sink over the real session), `src/app/interact/cite.ts` (law 18: occurrence matcher over the context's theorems, Tab-cycling). The existing `src/app/session.ts` track/dual application is retained as the sole step-history authority; the shell's two-phase proof pickers, `BackwardEntry`/`commitBackward`, manual citation-wire picking, click-target iteration, and any remaining directional special-casing are DELETED. Cursor-based history and declare-at-cursor remain Task 4.
...
**Files:** `src/app/interact/motion.ts` (law 26 from the archived `round7.ts`: playConversion over mkGridMorph with the pinned-v1 toggle, ghosts via the engine mutate hook, hover ease through the brush's motionPrefs, the ⚙ panel). Input guard while playing; commit deferred.
...
- [x] **Executable lab retired and complete design evidence archived (2026-07-11):** all 132 lab files, five demo-only browser checks, and both displaced source prototypes now live under `docs/superpowers/plans/2026-07-04-ui-lab-final/` with an explicit evidence-only README. The root `ui-lab/`, source prototype paths, lab Playwright server, and production/test imports of lab authority are absent. The one reusable external-reference presentation derivation moved into the authoritative comprehension-draft module; production themes own their selection colors directly.
- [x] **Adversarial review passed (2026-07-11):** one combined temporary mutation run introduced the seven prohibited regressions and produced eight targeted failures across all six guard files: ring-band admission, Shift precedence, citation occurrence filtering, verbatim refusal copy, orphan-rider erasure, folded instantiation, and motion semantic immutability. After restoration, the focused set passed 87/87, including shared paint/hit geometry and deterministic prefix replay. The review also found and repaired two production integration bugs: Edit Ctrl+Z was advertised but excluded by the shell, and entering fixed-side proving did not reset main-surface pins/selection/pending state.
- [x] **Plan synchronized and integrated directly on authorized `main` (2026-07-11):** no feature branch or worktree remained to merge or delete. TypeScript passed and all 29 production Playwright scenarios passed using only the production app server. Dedicated physics suites were excluded because no physics source changed.
