...
**Goal:** Make TypeScript and Lean `headStrip` replace a self-contained binary rigid-head equation with corresponding nontrivial argument equations, discharge trivial/nullary matches, and refuse ordinary or existential additional attachments.
...
- The equation has exactly the two selected term attachments. In the concrete
  encoding this requires exactly two explicit output endpoints and a local
  existential attachment; otherwise refuse and require severing or deiteration
  first.
