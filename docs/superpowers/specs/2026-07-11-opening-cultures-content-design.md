...
- authored teacher interventions for opening, completion, or exact recognized
  unwinnable proof states;
- broken-seal reference eligibility;
- professional name, curator shorthand, and provenance metadata.

Teacher instruction uses one or more single-paragraph passive pages while control
always remains with the player. Optional Next actions turn pages without changing
proof state. No artifact requires an intentional mistake, a prescribed solution
order where alternatives are legal, or a mandatory timeline detour.
...
recognize a valid transition into an anticipated trap and present passive recovery
guidance. Each exact-state intervention includes a legal
...
clauses become ordered knowledge points; correction copy remains performance
metadata rather than runtime hints or invalid-action thoughts.
...
- **Opening teacher intervention:** a passive multi-page basic-interaction
  tutorial. Each page is one short paragraph and page turning is optional. Its
  first pages teach hover highlighting, selection, deselection by clicking the
  selected mark again, and clearing selection by clicking the empty field. Only
  the final page explains that selecting the paired boundary and choosing the
  discovered elimination action or pressing Delete/Backspace lifts both veils
  and completes the proof. The
  logical move remains available on every page; reading the tutorial is never
  required.
...
- **Teacher interventions:** none. There is no opening lecture or generic
  stalled-play hint.
...
- No unapproved visual culture direction, character voice, or lore is invented.
  The first-puzzle interaction tutorial uses direct interface language only.
