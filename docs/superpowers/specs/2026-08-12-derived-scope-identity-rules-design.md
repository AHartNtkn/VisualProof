...
|---|---|---|
| erasure (`erasure.ts`) | (b) | `removeSubgraph` keeps touching wires with endpoints filtered (`splice.ts:49`); losing an endpoint can drop a DCA. New precondition: every surviving touching wire's derived scope is unchanged. Pin first otherwise. A wire wholly inside the selection dies with it — no condition. |
...
| deiteration (`iteration.ts:185`) | (b) | Removes the copy's endpoints; same precondition as erasure. |
