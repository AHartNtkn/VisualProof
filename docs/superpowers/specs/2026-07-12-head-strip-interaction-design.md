...
Players can author `headStrip` directly on the diagram by dragging from one term output leg to another term output leg on the same equality wire. The interaction uses the established green connection preview, identifies an exact node pair on a multi-output wire, retains the original equation, and never requires the proof menu.
...
- Dragging from a term output endpoint leg to a distinct term output endpoint leg on the same wire applies `headStrip` to those two nodes.
...
With three or more term outputs on one equality wire, the source and target endpoint legs select exactly one pair. All other nodes and the original equality wire remain untouched.
...
1. emits `headStrip` for distinct term-output endpoints on the same wire;
