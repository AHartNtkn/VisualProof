...
- **Polarity coloring.** Branches stroke by cut-nesting parity (USER ruling 2026-08-16): even depth (the sheet's polarity) keeps the strong line color, odd depth (cut polarity) goes gray (the theme's frame color). Every cut boundary is therefore visible as a color change — including a single cut continuing collinearly — and no marker dots are drawn. A childed region with no nodes still shows a visible stretch of line, so the sheet's segment never vanishes.
- **Identity nodes** draw a small pip over their line (USER ruling 2026-08-15), stroked in their owner wire's hue; their wires meet the line at that point via short perpendicular stubs.
...
- **Wires** are 3D splines through free space, touching the tree only at node anchors, and NEVER running parallel-and-overlapping with a line (USER law 2026-08-15). Every terminal connects through a short straight stub along its departure normal — the outward radial at a ring (the branch-facing side of an application circle is its interior and never receives a connection), a perpendicular at an identity — so the routed web meets the tree only at stub ends, and a wire between nodes on one line arches out and back. A wire with three or more ends is drawn as a branched network — junction vertices meeting its curves with the length-minimizing (120°-style) geometry, the 3D counterpart of the 2D routed-network wires.
- **Dark theme:** black background (`#0e1013`), white glowing sheet-parity branch lines with odd-polarity (cut) lines in the theme's frame gray, wires in the existing per-relation hues with bloom. **Light theme:** the existing manuscript tan (`#e8e4d8`), near-black ink lines, darkened relation hues, no bloom. Both derive from the existing `Theme` object via its `mode`.
