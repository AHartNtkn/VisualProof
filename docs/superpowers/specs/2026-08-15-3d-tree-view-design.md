...
Every region is a straight segment. Its contents — the region's nodes plus one branch point per child cut — are placed along it in canonical diagram order. Each item's spacing scales with its demands: a node's spacing grows with its port count (rim anchors need room), and segment length is the sum of spacings.

Each subtree is summarized bottom-up by a bounding cone: apex at its branch point, axis along its segment, half-angle and length just large enough to contain the subtree's own corridor and all of its children's cones. The **corridor** is the segment inflated by a clearance radius that grows with the number of wire curves that must escape the subtree — each escaping wire needs its own tube of routing room — so wire-dense regions get physically wider berths.

At a branch point, the child leaves with a tilt and an azimuth. Tilt: the smallest angle at which the child's cone clears the parent's corridor (corridor half-angle + child half-angle + one global clearance angle). Azimuth: the circle around the parent axis is partitioned into disjoint arcs proportional to each child's angular demand **plus a term for the wires crossing between it and other subtrees**, so heavily webbed subtrees get wider gaps between them for wires to pass through. If demands exceed the circle, tilts increase or attachment points spread along the parent — a closed-form adjustment, not a search. Sibling cones are disjoint by the arc partition; non-adjacent subtrees are inside disjoint ancestor cones by induction. This budgets room; it is not the wire-separation mechanism.
