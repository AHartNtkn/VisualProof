...
- **Polarity coloring.** Branches stroke by cut-nesting parity (USER ruling 2026-08-16): odd depth (cut polarity) keeps the strong line color, even depth (the sheet's polarity) goes gray (the theme's frame color). Every cut boundary is therefore visible as a color change — including a single cut continuing collinearly — and no marker dots are drawn.
...
- **Dark theme:** black background (`#0e1013`), white glowing odd-polarity (cut) branch lines with sheet-parity lines in the theme's frame gray, wires in the existing per-relation hues with bloom. **Light theme:** the existing manuscript tan (`#e8e4d8`), near-black ink lines, darkened relation hues, no bloom. Both derive from the existing `Theme` object via its `mode`.
