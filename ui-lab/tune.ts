...
boot('Wire-physics tuning board', 'every live constant on a slider — dial the feel, then COPY VALUES and report the numbers; drag nodes / kick to test settling', (lab: LabCtx) => {
  ...
  const knobs: (Knob | string)[] = [
    'WIRE',
    { label: 'tension (wants short)', obj: WIREP, key: 'tension', min: 0.2, max: 3, step: 0.05 },
    { label: 'bend (wants straight)', obj: WIREP, key: 'bend', min: 0, max: 2.5, step: 0.05 },
    { label: 'barrier (pushes discs)', obj: WIREP, key: 'barrierSlope', min: 12, max: 45, step: 1 },
    ...
    'PACING',
    { label: 'wire step (responsiveness)', obj: PACE, key: 'chainStep', min: 0.02, max: 0.4, step: 0.01 },
    { label: '∃-dot step', obj: PACE, key: 'homedStep', min: 0.01, max: 0.25, step: 0.01 },
