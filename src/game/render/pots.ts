...
function hologramMaterials(
  materials: TreeMaterialSource,
  trackLineMaterial: (material: LineMaterial) => () => void,
  untrack: (() => void)[],
): TreeMaterialSource {
  return {
    line(entity, width) {
      const material = ownedMaterial(materials.line(entity, width))
      untrack.push(trackLineMaterial(material))
      return material
    },
...
export function makePotObject(
  ...
  const hologram = makeRawTreeObject(asset, {
    ...
    z: 0,
    yaw: 0,
  }, hologramMaterials(materials, trackLineMaterial, untrack))
  ...
  return {
    ...
    dispose() {
      ...
      for (const release of untrack) release()
