...
function decodeSlotEntry(value: unknown, index: number): SlotListEntry {
  const entry = record(value, `slot ${index}`)
  const allowed = new Set(['slotId', 'displayName', 'updatedAtMs', 'error'])
  for (const key of Object.keys(entry)) {
    if (!allowed.has(key)) throw new Error(`slot ${index} has unknown field '${key}'`)
  }
  if (typeof entry.slotId !== 'string') throw new Error(`slot ${index}.slotId must be a string`)
  if (typeof entry.displayName !== 'string') {
    throw new Error(`slot ${index}.displayName must be a string`)
  }
  if (typeof entry.updatedAtMs !== 'number' || !Number.isSafeInteger(entry.updatedAtMs)) {
    throw new Error(`slot ${index}.updatedAtMs must be a safe integer`)
  }
  if (entry.error !== null && typeof entry.error !== 'string') {
    throw new Error(`slot ${index}.error must be a string or null`)
  }
  return {
    slotId: entry.slotId,
...
export function decodeSlotList(value: unknown): readonly SlotListEntry[] {
  ...
  return value.map((entry, index) => {
    const decoded = decodeSlotEntry(entry, index)
