...
import { applyLambdaFission } from '../../kernel/rules/lambda/fission'
import type { Body, Engine } from '../../view/engine'
...
export function fissionHit(
  ...
  if (!Number.isFinite(viewScale) || viewScale <= 0) {
    throw new RangeError('fission view scale must be positive')
  }
  ...
  for (const body of engine.bodies.values()) {
    ...
    for (const occurrence of body.geometry.occurrences) {
      ...
      if (distance <= HIT_RADIUS_PX / viewScale) {
        candidates.push({ body, occurrence, distance })
      }
    }
  }
  candidates.sort((left, right) =>
    ...
    || left.occurrence.path.join('/').localeCompare(right.occurrence.path.join('/'))
    || left.body.id.localeCompare(right.body.id))
  ...
  return {
    ...
    ...fissionValidity(diagram, found.body.id, found.occurrence.path),
  }
}
...
function segmentDistance(point: Vec2, a: Vec2, b: Vec2): number {
  ...
  const amount = lengthSquared === 0
    ? 0
    : Math.max(0, Math.min(1, (ap.x * ab.x + ap.y * ab.y) / lengthSquared))
  return length(sub(point, {
    x: a.x + ab.x * amount,
    y: a.y + ab.y * amount,
  }))
}

function occurrenceDistance(
  engine: Engine,
  body: Body,
  occurrence: TermOccurrenceGeometry,
  point: Vec2,
): number {
  ...
  if (occurrence.hit.kind === 'exit') {
    ...
    return segmentDistance(
      point,
      localToWorld(engine, body, a),
      localToWorld(engine, body, b),
    )
  }
...
function anatomyShapes(
  ...
  for (const index of occurrence.arcIndices) {
    ...
    if (arc !== undefined) shapes.push({
      ...
      center: body.pos,
      r: arc.r * scale,
      a0: arc.a0 + body.theta,
      a1: arc.a1 + body.theta,
      stroke: color,
      width: 4.5,
      glow: null,
    })
  ...
  if (occurrence.includeExit) {
    if (geometry.exitArc !== null) shapes.push({
      kind: 'arc',
      center: body.pos,
      r: geometry.exitArc.r * scale,
      a0: geometry.exitArc.a0 + body.theta,
      a1: geometry.exitArc.a1 + body.theta,
      stroke: color,
      width: 4.5,
      glow: null,
    })
    if (geometry.exitLine !== null) shapes.push({
      kind: 'segment',
      from: localToWorld(engine, body, geometry.exitLine[0]),
      to: localToWorld(engine, body, geometry.exitLine[1]),
      stroke: color,
      width: 4.5,
      glow: null,
    })
...
function hitPoint(
  ...
  if (occurrence.hit.kind === 'arcPoint') {
    return localToWorld(engine, body, occurrence.hit.point)
  }
  if (occurrence.hit.kind === 'exit') {
    return localToWorld(engine, body, body.geometry!.exitLine![0])
  }
  const radial = body.geometry!.radials[occurrence.hit.radialIndex]!
  return localToWorld(
    engine,
    body,
    polar(radial.angle, (radial.r0 + radial.r1) / 2),
  )
}

export function fissionTargetPoint(
  engine: Engine,
  node: NodeId,
  path: readonly PathSeg[],
): Vec2 | null {
...
function geometryShapesAt(
  ...
  for (const radial of geometry.radials) shapes.push({
    kind: 'segment',
    from: {
      x: at.x + Math.cos(radial.angle) * radial.r0 * scale,
      y: at.y + Math.sin(radial.angle) * radial.r0 * scale,
    },
    to: {
      x: at.x + Math.cos(radial.angle) * radial.r1 * scale,
      y: at.y + Math.sin(radial.angle) * radial.r1 * scale,
    },
    stroke: color,
    width: 2.3,
    glow: null,
  ...
  if (geometry.exitLine !== null) shapes.push({
    kind: 'segment',
    from: {
      x: at.x + geometry.exitLine[0].x * scale,
      y: at.y + geometry.exitLine[0].y * scale,
    },
    to: {
      x: at.x + geometry.exitLine[1].x * scale,
      y: at.y + geometry.exitLine[1].y * scale,
    },
    stroke: color,
    width: 2.3,
    glow: null,
...
export function fissionDropPoint(
  engine: Engine,
  diagram: Diagram,
  nodeId: NodeId,
): Vec2 | null {
...
export class FissionDragController {
  ...
  claim(sample: PointerSample): PointerClaim | null {
    ...
    return {
      ...
      release: (next, moved) => {
        ...
        this.#generation++
        if (!moved) return
        const validity = fissionValidity(
          this.#options.diagram(),
          target.node,
          target.path,
        )
        if (!validity.valid) {
          this.#options.refuse(
            validity.reason ?? 'this subterm cannot be split out',
            next.client,
          )
          return
        }
        if (!placementValid(
          ...
          this.#options.refuse(
            'pull outside the node and release in its current region to fission',
            next.client,
          )
          return
        }
  ...
  overlay(): readonly Shape[] {
    ...
    const valid = validity.valid
      && (preview === null || !preview.moved || preview.placementValid)
    const color = valid
      ? this.#options.theme().interaction.valid
      : this.#options.theme().interaction.refusal
    const shapes = anatomyShapes(
      this.#options.engine(),
      body,
      target.occurrence,
      color,
    ...
    if (preview !== null && preview.moved) {
      shapes.push({
        kind: 'segment',
        from: hitPoint(this.#options.engine(), body, target.occurrence),
        to: preview.at,
        stroke: color,
        width: 1.8,
        glow: null,
      ...
      if (validity.valid && node?.kind === 'term') {
        try {
          ...
          shapes.push(...geometryShapesAt(
            this.#options.engine(),
            bendGrid(trompGrid(subterm)),
            preview.at,
            color,
          ))
        } catch {
          // Live graph validation remains authoritative for stale paths.
        }
      }
    }
    return shapes
  ...
  #placementValid(preview: DragPreview): boolean {
    return preview.target.valid && placementValid(
      this.#options.engine(),
      this.#options.diagram(),
      preview.target.node,
      preview.at,
