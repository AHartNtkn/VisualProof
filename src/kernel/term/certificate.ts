import { applyStepAt, stepEta, stepNormalOrder, type ReductionStep } from './reduce'
...
export function checkConversion(
  ...
  const leftResult = replayPath(left, certificate.leftSteps, 'left')
  if (!leftResult.ok) return leftResult
  const rightResult = replayPath(right, certificate.rightSteps, 'right')
