...
/** Resolve one node and require a whole Lambda term. */
export function termNodeAt(
  diagram: Diagram,
  nodeId: NodeId,
): Extract<DiagramNode, { kind: 'term' }> {
  const node = diagram.nodes[nodeId]
  if (node === undefined) throw new DiagramError(`unknown node '${nodeId}'`)
  if (node.kind !== 'term') {
    throw new RuleError(
      `this rule applies to term nodes; '${nodeId}' has kind '${node.kind}'`,
    )
  }
  return node
}
