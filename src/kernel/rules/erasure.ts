 ...
 * surviving wire the erased material touched is CAPPED by the shared
 * removal residue (capRemovedSupport; Lean twin Erasure.residue): no scope
 * moves and no wire drops below two ends. */
export function applyErasure(
  ...
  capRemovedSupport(
    d, sel.region, contents.allNodes, new Set(contents.internalWires),
    parts, 'erasure', reservation?.nodes,
  )
