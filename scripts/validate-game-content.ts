...
import { GameDomainError, puzzleId, type GameStep, type PuzzleId } from '../src/game/types'
...
export function validateGameContent(contentRoot = resolve(process.cwd(), 'content')): ContentValidationReceipt {
  ...
  const coveragePath = String(manifestRecord.coverage)
  ...
  const coverageValue = parseJson(resolve(contentRoot, coveragePath))
  requireSchema(validators.coverage, coverageValue, coveragePath)
  const coverage = parseCoverage(coverageValue, coveragePath)

  const seyricIds = catalog.puzzlesInCulture('seyric-horizon' as never)
  const seyricSet = new Set(seyricIds)
  const coverageByPuzzle = new Map<PuzzleId, CoverageRow[]>()
  for (const row of coverage.puzzles) {
    coverageByPuzzle.set(row.puzzle, [...(coverageByPuzzle.get(row.puzzle) ?? []), row])
  }
  for (const id of seyricIds) {
    const rows = coverageByPuzzle.get(id) ?? []
    if (rows.length === 0) throw new GameDomainError(`Seyric puzzle '${id}' has no coverage row`)
    if (rows.length > 1) throw new GameDomainError(`Seyric puzzle '${id}' must have exactly one coverage row`)
  }
  for (const row of coverage.puzzles) {
    if (!seyricSet.has(row.puzzle)) {
      throw new GameDomainError(`coverage row names non-Seyric puzzle '${row.puzzle}'`)
    }
  }
  ...
  for (const obligation of coverage.obligations) {
    if (knownObligations.has(obligation.id)) {
      throw new GameDomainError(`duplicate coverage obligation '${obligation.id}'`)
    }
  ...
  for (const row of coverage.puzzles) {
    for (const obligation of row.obligations) {
      if (!knownObligations.has(obligation)) {
        throw new GameDomainError(`coverage for '${row.puzzle}' names unknown obligation '${obligation}'`)
      }
      coveredObligations.add(obligation)
    }
    for (const neighbor of row.experientialNeighbors) {
      if (!puzzleIds.has(neighbor)) {
        throw new GameDomainError(`coverage for '${row.puzzle}' names missing experiential neighbor '${neighbor}'`)
      }
    }
  }
  for (const obligation of knownObligations) {
    if (!coveredObligations.has(obligation)) {
      throw new GameDomainError(`uncovered obligation '${obligation}'`)
    }
  }
  ...
  for (const id of seyricIds) {
    ...
    for (const [regionId, region] of Object.entries(diagram.regions)) {
      ...
      if (region.arity !== 0) seyricLanguageViolations.push(`${id}: bubble '${regionId}' has arity ${region.arity}`)
      if (cutDepth(diagram, regionId) % 2 === 0) seyricLanguageViolations.push(`${id}: bubble '${regionId}' is existential`)
    }
    for (const [nodeId, node] of Object.entries(diagram.nodes)) {
      if (node.kind !== 'atom') seyricLanguageViolations.push(`${id}: node '${nodeId}' has kind '${node.kind}'`)
    }
    if (Object.keys(diagram.wires).length > 0) seyricLanguageViolations.push(`${id}: contains individual wires`)
  }
  if (seyricLanguageViolations.length > 0) {
    throw new GameDomainError(`Seyric puzzles exceed the propositional culture boundary: ${seyricLanguageViolations.join('; ')}`)
  }
  ...
  while (pending.size > 0) {
    ...
    for (const step of sidecar.solution) solution = applyGameStep(solution, step, authority).session
    ...
    for (const demonstration of sidecar.recognizedStates) {
      ...
      for (const step of demonstration.demonstration) reached = applyGameStep(reached, step, authority).session
