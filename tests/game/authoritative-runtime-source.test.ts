...
describe('authoritative renderer source boundary', () => {
  it('contains no prototype authority, renderer persistence fallback, app import, or obsolete asset consumer', () => {
    ...
    expect(mount).toContain('mountGamePresentationView')
    expect(mount).not.toMatch(/emptyPresentation|CursebreakerPresentationPort|presentation\?\s*:/)
    expect(`${mount}\n${readFileSync('src/game/controller.ts', 'utf8')}\n${readFileSync('src/game/controller-state.ts', 'utf8')}`)
      .not.toMatch(/modalInstruction|openTeacher|acknowledgeTeacher|closeTeacher|acknowledgedTeachers/)
