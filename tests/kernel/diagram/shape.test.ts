...
describe('termShapeKey', () => {
  it('is invariant under free-variable renaming (positional relation identity)', () => {
    expect(termShapeKey(p('y z'), ['y', 'z'])).toBe(termShapeKey(p('a b'), ['a', 'b']))
    expect(termShapeKey(p('y (z y)'), ['y', 'z'])).toBe(termShapeKey(p('z (y z)'), ['z', 'y']))
    expect(termShapeKey(p('\\x. y x'), ['y'])).toBe(termShapeKey(p('\\q. w q'), ['w']))
  })

  it('distinguishes different positional relations', () => {
    // one free var used twice vs two distinct free vars
    expect(termShapeKey(p('y y'), ['y'])).not.toBe(termShapeKey(p('y z'), ['y', 'z']))
    // structure matters
    expect(termShapeKey(p('\\x. x'), [])).not.toBe(termShapeKey(p('\\x. \\y. x'), []))
  })

  it('renames ports as p0, p1 in first-occurrence order', () => {
    expect(termShapeKey(p('y z'), ['y', 'z'])).toBe('2:A(P("p0"),P("p1"))')
    expect(termShapeKey(p('z y'), ['z', 'y'])).toBe('2:A(P("p0"),P("p1"))') // same positional relation
  })
...
describe('positionalPortKey', () => {
  it('maps output to out, freeVar names to v{first-occurrence index}, args to a{index}', () => {
    ...
    const ports = freePorts(t)
    expect(positionalPortKey(t, { kind: 'output' }, ports)).toBe('out')
    expect(positionalPortKey(t, { kind: 'freeVar', name: 'y' }, ports)).toBe('v0')
    expect(positionalPortKey(t, { kind: 'freeVar', name: 'z' }, ports)).toBe('v1')
    expect(positionalPortKey(t, { kind: 'arg', index: 3 }, ports)).toBe('a3')
  })

  it('throws for a freeVar name outside the declared interface', () => {
    expect(() => positionalPortKey(p('y'), { kind: 'freeVar', name: 'zz' }, ['y']))
