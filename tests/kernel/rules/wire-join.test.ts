...
describe('wire join: signature gate', () => {
  it('refuses to join a TERM wire with a relSig([TERM]) wire, naming both sigs', () => {
    const h = new DiagramBuilder()
    const cut = h.cut(h.root)
    const a = h.wire(cut, [], TERM)
    const b = h.wire(cut, [], relSig([TERM]))
    const d = h.build()
    expect(() => applyWireJoin(d, a, b))
      .toThrowError(/cannot join wires of different signatures: 't' vs '\(t\)'/)
  ...
  it('refuses to join two structurally distinct relational wires', () => {
    const h = new DiagramBuilder()
    const cut = h.cut(h.root)
    const a = h.wire(cut, [], relSig([TERM]))
    const b = h.wire(cut, [], relSig([TERM, TERM]))
    const d = h.build()
    expect(() => applyWireJoin(d, a, b))
      .toThrowError(/cannot join wires of different signatures: '\(t\)' vs '\(t,t\)'/)
  })
  ...
  it('the sig gate fires before scope comparability, so an incomparable-scope pair with mismatched sigs reports the sig mismatch', () => {
    ...
    const a = h.wire(cutA, [], TERM)
    const b = h.wire(cutB, [], relSig([TERM]))
    const d = h.build()
    expect(() => applyWireJoin(d, a, b))
  ...
  it('same-sig wires with comparable scopes still succeed (gate does not over-refuse)', () => {
    const h = new DiagramBuilder()
    const cut = h.cut(h.root)
    const a = h.wire(h.root, [], TERM)
    const b = h.wire(cut, [], TERM)
    const d = h.build()
    ...
    expect(out.wires[a]!.sig).toEqual(TERM)
  })
})
