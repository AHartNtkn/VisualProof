...
The exact legacy semantic declaration names `SemanticMode`,
`DirectedImplication`, and `DirectedEntailment` are absent. Existing
per-operation proof towers compile against Concrete operation modules and use
operation-scoped raw evidence; their later refinement-layer relocation remains
outside this task.
...
- Source scans found no old concrete/rule/step-tag import paths, occurrence
  discovery declarations or language, obsolete declaration aliases, `sorry`,
